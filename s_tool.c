...
void sigquit3();
void sigquit4();
int ss_round (int i, int j);
int ss_trunc (int i, int j);
...
# endif
  ...
  rounded_samplerepeat = ss_round ((global_rate * attacktime),1000);
  ...
  if (samples != 0)
    {
      ...
      line = 1;
      roundedsample = ss_round (samples,pixels);
      correctionfactor = (float) roundedsample - rawsample;
      repeat = ss_round (ss_round((global_rate * pixels), samples), 50);
...
#                 endif
          ...
          if (sampletally >= roundedsample)
      {
        ...
        overload = ss_trunc (sampletally,roundedsample) - 1;
