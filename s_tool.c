...
void sigquit3();
void sigquit4();
int round_div (int i, int j);
int trunc_div (int i, int j);
...
# endif
  ...
  rounded_samplerepeat = round_div((global_rate * attacktime), 1000);
  ...
  if (samples != 0)
    {
      ...
      line = 1;
      roundedsample = round_div(samples, pixels);
      correctionfactor = (float) roundedsample - rawsample;
      repeat = round_div(round_div((global_rate * pixels), samples), 50);
...
#                 endif
          ...
          if (sampletally >= roundedsample)
      {
        ...
        overload = trunc_div(sampletally, roundedsample) - 1;
