*BEGIN WORDS VERSION=442/320 ENCODING=7BIT
<Applix Words>
<Globals levelIndent:0 hyphMethod:0 headerMargin:500 footerMargin:500 changeBar\
 Pos:0>
<start_styles>
<style "Normal" nextStyle "Normal" no-pageBreak no-keepWith no-block justifyFul\
 l indentToLevel spellcheck firstIndent:0 leftIndent:0 rightIndent:0 lineSpacin\
 g:0 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpaci\
 ngSep:0 preParaSpacing:0 postParaSpacing:83 level:0 hyphZone:0 hyphMinFrag:0  \
 no-bold no-italic no-strikethru no-hidden no-caps no-underline hyphenate color\
 :"Black" face:"Arial" size:12 position:0 tag:""  lB:0:0:"" rB:0:0:"" tB:0:0:""\
  bB:0:0:"" hB:0:0:"" vB:0:0:"" shading:18:"":"":"" horizontalMargin:0 vertical\
 Margin:0 dropShadow:0  localTabs  noFrame  >
<style "toc 3" parent "Normal" nextStyle "Normal" indentToLevel firstIndent:985\
  leftIndent:985 rightIndent:590 lineSpacingMode2:6 lineSpacingMode:0 lineSpaci\
 ngCountHeight:1000 lineSpacingSep:0 level:0  size:10  localTabs lT:5906:"." rT\
 :6000  >
<style "toc 2" parent "Normal" nextStyle "Normal" indentToLevel firstIndent:500\
  leftIndent:500 rightIndent:500 lineSpacingMode2:6 lineSpacingMode:0 lineSpaci\
 ngCountHeight:1000 lineSpacingSep:0 level:0  size:10  localTabs lT:5750:"." rT\
 :6000  >
<style "toc 1" parent "Normal" nextStyle "Normal" indentToLevel rightIndent:590\
  lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingS\
 ep:0 level:0  bold italic capitals size:13  localTabs lT:5906:"." rT:6000  >
<style "index 7" parent "Normal" nextStyle "Normal" indentToLevel firstIndent:1\
 500 leftIndent:1500 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeigh\
 t:1000 lineSpacingSep:0 level:0  size:10  >
<style "index 6" parent "Normal" nextStyle "Normal" indentToLevel firstIndent:1\
 250 leftIndent:1250 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeigh\
 t:1000 lineSpacingSep:0 level:0  size:10  >
<style "index 5" parent "Normal" nextStyle "Normal" indentToLevel firstIndent:1\
 000 leftIndent:1000 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeigh\
 t:1000 lineSpacingSep:0 level:0  size:10  >
<style "index 4" parent "Normal" nextStyle "Normal" indentToLevel firstIndent:7\
 50 leftIndent:750 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:\
 1000 lineSpacingSep:0 level:0  size:10  >
<style "index 3" parent "Normal" nextStyle "Normal" indentToLevel firstIndent:5\
 00 leftIndent:500 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:\
 1000 lineSpacingSep:0 level:0  size:10  >
<style "index 2" parent "Normal" nextStyle "Normal" indentToLevel firstIndent:2\
 50 leftIndent:250 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:\
 1000 lineSpacingSep:0 level:0  size:10  >
<style "index 1" parent "Normal" nextStyle "Normal" indentToLevel lineSpacingMo\
 de2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep:0 level:0  \
 size:10  >
<style "line number" parent "Normal" nextStyle "Normal" indentToLevel lineSpaci\
 ngMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep:0 level\
 :0  size:10  >
<style "index heading" parent "Normal" nextStyle "index 1" indentToLevel lineSp\
 acingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep:0 le\
 vel:0  size:10  >
<style "footer" parent "Normal" nextStyle "footer" indentToLevel lineSpacingMod\
 e2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep:0 level:0  l\
 ocalTabs cT:3000 rT:6000  >
<style "header" parent "Normal" nextStyle "header" indentToLevel lineSpacingMod\
 e2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep:0 level:0  l\
 ocalTabs cT:3000 rT:6000  >
<style "footnote reference" parent "Normal" nextStyle "Normal" indentToLevel li\
 neSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep:\
 0 level:0  size:8 position:3  >
<style "footnote text" parent "Normal" nextStyle "footnote text" indentToLevel \
 leftIndent:78 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000\
  lineSpacingSep:0 level:0  size:10  >
<style "heading 9" parent "Normal" nextStyle "Normal indent" indentToLevel firs\
 tIndent:500 leftIndent:500 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCou\
 ntHeight:1000 lineSpacingSep:0 level:0  italic size:10  >
<style "heading 8" parent "Normal" nextStyle "Normal indent" indentToLevel firs\
 tIndent:500 leftIndent:500 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCou\
 ntHeight:1000 lineSpacingSep:0 level:0  italic size:10  >
<style "heading 7" parent "Normal" nextStyle "Normal indent" indentToLevel firs\
 tIndent:500 leftIndent:500 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCou\
 ntHeight:1000 lineSpacingSep:0 level:0  italic size:10  >
<style "heading 6" parent "Normal" nextStyle "Normal indent" indentToLevel firs\
 tIndent:500 leftIndent:500 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCou\
 ntHeight:1000 lineSpacingSep:0 level:0  underline  >
<style "heading 2" parent "Normal" nextStyle "Normal" keepWith justifyLeft inde\
 ntToLevel lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lin\
 eSpacingSep:0 preParaSpacing:333 postParaSpacing:250 level:0  bold capitals si\
 ze:24  >
<style "heading 3" parent "heading 2" nextStyle "Normal" keepWith indentToLevel\
  lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingS\
 ep:0 preParaSpacing:250 postParaSpacing:167 level:0  double-underline size:16 \
  >
<style "heading 4" parent "heading 3" nextStyle "Normal indent" indentToLevel l\
 ineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep\
 :0 preParaSpacing:167 level:0  underline size:14  >
<style "heading 5" parent "heading 4" nextStyle "Normal indent" indentToLevel l\
 ineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep\
 :0 postParaSpacing:83 level:0  italic capitals size:13  >
<style "heading 1" parent "Normal" nextStyle "Normal" justifyLeft indentToLevel\
  lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingS\
 ep:0 preParaSpacing:333 postParaSpacing:250 level:0  bold capitals size:48  bB\
 :31:1:"" verticalMargin:14  jumpOver xposColumnRelative xpos:0 yposParaRelativ\
 e ypos:1 leftFrameMargin:126 rightFrameMargin:126 topFrameMargin:0 bottomFrame\
 Margin:0  >
<style "Normal indent" parent "Normal" nextStyle "Normal indent" indentToLevel \
 firstIndent:500 leftIndent:500 lineSpacingMode2:6 lineSpacingMode:0 lineSpacin\
 gCountHeight:1000 lineSpacingSep:0 level:0  size:10  >
<style "Title" parent "heading 1" nextStyle "Title" justifyCenter indentToLevel\
  lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingS\
 ep:0 level:0  color:"Black"  >
<style "Bullet" parent "Normal" nextStyle "Bullet" indentToLevel firstIndent:39\
 4 leftIndent:788 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1\
 000 lineSpacingSep:0 postParaSpacing:0 level:0  color:"Black"  >
<style "Bullet1" parent "heading 3" nextStyle "Bullet1" indentToLevel lineSpaci\
 ngMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep:0 postP\
 araSpacing:83 level:0  capitals color:"Black" size:12  >
<style "Chapter" parent "Normal indent" nextStyle "Chapter" justifyLeft indentT\
 oLevel lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSp\
 acingSep:0 level:0  color:"Black"  >
<style "heading 0" parent "Normal indent" nextStyle "heading 0" indentToLevel l\
 ineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep\
 :0 level:0  color:"Black"  >
<style "toc ps" parent "toc 1" nextStyle "toc ps" indentToLevel lineSpacingMode\
 2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep:0 level:0  bo\
 ld italic color:"Black" size:12  >
<style "subtitle" parent "heading 3" nextStyle "subtitle" indentToLevel lineSpa\
 cingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep:0 lev\
 el:0  capitals color:"Black"  >
<style "Table" parent "Normal" nextStyle "Table" indentToLevel lineSpacingMode2\
 :6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep:0 postParaSpac\
 ing:0 level:0  color:"Black"  >
<style "commline" parent "Normal" nextStyle "commline" indentToLevel lineSpacin\
 gMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSep:0 level:\
 0  color:"Black" face:"Courier New" size:10  >
<style "listing" parent "Normal" nextStyle "listing" justifyLeft indentToLevel \
 lineSpacingMode2:6 lineSpacingMode:0 lineSpacingCountHeight:1000 lineSpacingSe\
 p:0 postParaSpacing:0 level:0  color:"Black" size:8  >
<S_G "FootnoteBar@" >
<T "_________________________">
<P "Normal" lineSpacing:0 postParaSpacing:100 >
<E_G>
<S_G "FootnoteContBar@" >
<T "__________________________________________________">
<P "Normal" lineSpacing:0 postParaSpacing:100 >
<E_G>
<S_G "EndnoteBar@" >
<T "_________________________">
<P "Normal" lineSpacing:0 postParaSpacing:100 >
<E_G>
<color "Black0":0:0:0:255>
<color "Black":0:0:0:255>
<color "Blue":255:255:0:0>
<color "Cyan":255:0:0:0>
<color "Green":255:0:255:0>
<color "Magenta":0:255:0:0>
<color "Red":0:255:255:0>
<color "Yellow":0:0:255:0>
<color "White":0:0:0:0>
<color "Dark Blue":127:127:0:128>
<color "Dark Cyan":127:0:0:128>
<color "Dark Green":127:0:127:128>
<color "Dark Magenta":0:127:0:128>
<color "Dark Red":0:127:127:128>
<color "Dark Yellow":0:0:127:128>
<color "Dark Gray":0:0:0:128>
<color "Light Gray":0:0:0:63>
<color "HtmlLinkDefault@":255:255:0:0>
<end_styles>
<start_flow>
<WP400 "This file must be filtered to be read in WP 3.11">
<P "Normal" justifyLeft lineSpacing:0 postParaSpacing:0 >
<T "Programmer's Guide">
<P "heading 1" lineSpacing:0 >
<T "Purpose of this Guide">
<P "heading 2" lineSpacing:0 >
<T "The purpose of this section is to give detail of the way in which ">
<T "Sound Studio" italic >
<T " has been implemented. It aims to provide sufficient information to the pro\
 grammer, so that the source code is accessible. This section does not propose \
 to give details of individual functions and procedures, these are well documen\
 ted in the source code. A directory of procedures and functions will be provid\
 ed as a map to the listing. A knowledge of TCL/Tk is assumed.">
<P "Normal" lineSpacing:0 >
<T "Architecture">
<P "heading 2" lineSpacing:0 >
<T "The architecture of ">
<T "Studio" italic >
<T " is a patchwork of software bound together by a TCL/tk shell script. The TC\
 L/tk script sets up the user interface and defines the call-back routines. The\
 se call-back routines make use of command-line applications including system c\
 ommands to perform their function. The overall structure is illustrated in fig\
 ure ">
<S_F fieldType:10>
<T "xref \"fig:struct\"">
<FV>
<T "1">
<E_F>
<T ". ">
<P "Normal" lineSpacing:0 >
<T "Data Flow">
<P "heading 3" lineSpacing:0 >
<T "There are two types of data flow in ">
<T "studio" italic >
<T ", internal and external. The internal data flow of ">
<T "Studio" italic >
<T " is provided by global variables. This is really the only sensible way to h\
 andle data flow in a TCL/Tk application. The external form of data flow is tha\
 t of obtaining data from external applications. This is provided by ensuring t\
 hat the application provides information to the standard output. This can then\
  be stored in a TCL variable, in the following manner.">
<P "Normal" lineSpacing:0 >
<T "set ">
<T "TCL_Variable" italic >
<T " [ exec ">
<T "APP_Name" italic >
<T " -">
<T "App_Options" italic >
<T "]">
<P "Table" lineSpacing:0 >
<S_F fieldType:7 docType:201 appType:2>
<T "local_inset \"object1\" -docType \"201\" -appType \"Graphics_\" -clipMode c\
 lipToFit -picture">
<FV>
<picture "object1" appId:2 clipMode:1 xScale:0.865000 yScale:0.865000 width:567\
 4 height:4136 picOrigXoffset:0 picOrigYoffset:7 picOrigWidth:6559 picOrigHeigh\
 t:4782>
<E_F>
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1292 yposParaRelative ypos:1\
  width:5717 minHeight:591 leftFrameMargin:126 rightFrameMargin:126 topFrameMar\
 gin:0 bottomFrameMargin:0 >
<T "figure " italic >
<S_F fieldType:11 editable
<start_eval_value>
<T "--field--">
<end_eval_value>
>
<T "xref_source \"fig:struct\" -live">
<FV>
<S_F fieldType:3
<start_eval_value>
<T "1" italic >
<end_eval_value>
>
<T "series pfigure">
<FV>
<T "1">
<E_F>
<E_F>
<T " - The overall structure of Studio" italic >
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1292 yposParaRelative ypos:1\
  width:5717 minHeight:591 leftFrameMargin:126 rightFrameMargin:126 topFrameMar\
 gin:0 bottomFrameMargin:0 >
<T "USING TCL ARRAYS">
<P "heading 4" lineSpacing:0 >
<T "As has been explained the data flow in this application is largely provided\
  by global variables. The number of global variables required is large thus ma\
 king it difficult to oversee and remember all the global names.">
<P "Normal" lineSpacing:0  localTabs lT:1871 >
<T "To overcome this problem TCL arrays were used. Using TCL array has two main\
  advantages. Firstly, it allows the data organised into groups of similar data\
 . Secondly, it reduced the number of ">
<T "global" face:"Courier New" >
<T " declarations required within each procedure, since only the array name nee\
 ds to be declared.">
<P "Normal" lineSpacing:0  localTabs lT:1969 >
<T "An additional advantage is that the indices of TCL arrays may be text, thus\
  keeping the purpose of each array item clear. Performing operations on a comp\
 lete array is easy using the ">
<T "array names" face:"Courier New" >
<T " command within a ">
<T "foreach" face:"Courier New" >
<T " statement.">
<P "Normal" lineSpacing:0 >
<T "Table ">
<S_F fieldType:10
<start_eval_value>
<T "tab:groups: No such cross-reference source field">
<end_eval_value>
>
<T "xref \"tab:groups\"">
<FV>
<T "1">
<E_F>
<T " contains an overview of the groups of data in ">
<T "Studio" italic >
<T ". A detailed list of all global variables used is found in table ">
<S_F fieldType:10>
<T "xref \"tab:globals\"">
<FV>
<T "3">
<E_F>
<T " in the listing directory.">
<P "Normal" lineSpacing:0 >
<P "Normal" lineSpacing:0 >
<RS leftIndent:44>
<T "Array Name" bold >
<P "Normal" lineSpacing:0  bold >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Content" bold >
<P "Normal" lineSpacing:0  bold >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "CARD">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The diagnostic data of the sound card." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "ConfigInfo ">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Descriptive strings for configuration file and use in the user interface fo\
 r setting colours etc." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "ConfigSet ">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:21:0:"" >
<T "The configuration options which affect the look of the GUI, e.g. colours an\
 d fonts." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:21:0:"" >
<RS leftIndent:44>
<T "FILE">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "File names and paths for components of " size:10 >
<T "Studio" italic size:10 >
<T "." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "FLAG">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The state flags." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "FX">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The effect parameter values and application switches." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "HELP ">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The global variables belonging to the on-line help module." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "INFO">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Formatted strings for display in the InfoBar." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "LINE">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Information about play-position line" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "LOAD">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameters of the working sample file." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:40>
<T "MACHINE">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Machine-dependent (Solaris or Linux) information" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:40>
<T "MIXER">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Variables for the mixer board, including volume settings for the mixer devi\
 ces." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "PACK">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Information for the packing order of sections." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "PLAY">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Global variables required for the functioning of the estimated time." size:\
 10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "PLOT">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Global variables required for editing plot manipulation. Including position\
  of insertion points." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "SAMPLE">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameters of the original sample file." size:10 >
<P "Normal" lineSpacing:0 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS leftIndent:44>
<T "VU">
<P "Normal" lineSpacing:0 >
<CE width:1200 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Pertaining to the appearance and function of the VU meters." size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:4075 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<T "table " italic >
<S_F fieldType:11 editable
<start_eval_value>
<T "--field--">
<end_eval_value>
>
<T "xref_source \"tab:groups\" -live">
<FV>
<S_F fieldType:3>
<T "series ptable">
<FV>
<T "1">
<E_F>
<E_F>
<T " - Arrays in Studio" italic >
<P "Normal" justifyCenter lineSpacing:0 >
<T "Temporary Files">
<P "heading 3" lineSpacing:0 >
<T "Studio" italic >
<T " makes use of temporary files to handle the sample data. This decision aros\
 e out of the necessity to perform ">
<T "Sox " italic >
<T "operations on the sample data in the form of a UNIX pipe. There are five te\
 mporary files. These are stored in the directory specified in the ">
<T "studio" italic >
<T " start-up script. The directory /tmp is ideal for this purpose. The tempora\
 ry files are created with a long file name, which contains the system clock ti\
 me in seconds in it's name. Thus a set of temporary files exists for each inst\
 ance of ">
<T "Studio" italic >
<T " running.">
<P "Normal" lineSpacing:0 >
<T "The temporary files are:">
<P "Normal" lineSpacing:0 >
<T "1. ^ajTMP1. This is the ">
<T "working file." italic >
<T " The formatted sample file is piped though ">
<T "Sox " italic >
<T "upon loading and the raw sample data is stored in this file. When the sampl\
 e data is edited it is generally stored here.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "2.^ajTMP2. This is the ">
<T "clipboard file" italic >
<T ". This contains the data that has  been copied to the clipboard.  It is als\
 o used to hold a silence of the appropriate length when the Edit">
<T "^ko" face:"Symbol" >
<T "Insert Silence menu option is invoked." face:"Arial" >
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "3.^ajTMP3. This is the ">
<T "Sox error file" italic >
<T ". The sample data that is extracted by ">
<T "Sox " italic >
<T "is sent to the standard error file, not standard out. Thus it seems that th\
 e only way to get to this information is to store it in this file and then rea\
 d it into TCL.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "4.^ajTMP4. This is the ">
<T "undo file" italic >
<T ". A copy of TMP1 is stored here when an editing operation is about be perfo\
 rmed, so that the changes may be reversed by copying this file back to TMP1.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "5.^ajTMP5. This is the ">
<T "overflow file" italic >
<T ". There is often need for a ">
<T "temporary" italic >
<T " temporary file. This is especially the case when using the preview mode.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "6.^ajTMP6.  This is the ">
<T "status-scan file. " italic >
<T "This file was introduced at version 0.2.6, and makes possible the avoidance\
  of the ">
<T "-e stat" face:"Courier" >
<T " flags of the " face:"Arial" >
<T "sox" bold italic face:"Arial" >
<T " program.  Using these flags causes some problems with more recent versions\
  of " no-bold no-italic face:"Arial" >
<T "sox" bold italic face:"Arial" >
<T ".  Having required the use of this file, it is also used to break up the su\
 sequent invocations enabling extra functionality when saving to " no-bold no-i\
 talic face:"Arial" >
<T "raw" no-bold no-italic face:"Courier" >
<T "/" no-bold no-italic face:"Arial" >
<T "wav" no-bold no-italic face:"Courier" >
<T " file types." no-bold no-italic face:"Arial" >
<P "Normal indent" leftIndent:750 lineSpacing:0 >
<T "7.^ajTMP7 is a miscellaneous scratch file.  It is used for grabbing outputs\
  from, eg, sox for later parsing, and as the record destination when the set-l\
 evel operation (middle click on the Record button) is being performed.  It is \
 also used to hold silence of the correct length when the \"Edit">
<T "^ko" face:"Symbol" >
<T "Insert Silence\" menu option is used." face:"Arial" >
<P "Normal indent" leftIndent:750 lineSpacing:0 >
<T "Editing Method">
<P "heading 3" lineSpacing:0 >
<T "Studio" italic >
<T " edits the sample file by manipulating the above temporary files.">
<P "Normal" lineSpacing:0 >
<T "Generating the Plot">
<P "heading 4" lineSpacing:0 >
<T "The plot is drawn on a canvas widget. It consists of one line per horizonta\
 l point in the canvas co-ordinate system. The vertical co-ordinates are determ\
 ined by the maximum and minimum points of a block of the sample. The program "\
 >
<T "maxmin.c " bold >
<T "was written solely for the purpose dividing the sample file into blocks and\
  scanning each block for the maxima and minima. It then prints the results to \
 stdout in TCL list format. This output is normalised and inverted due to the r\
 everse nature of the X- co-ordinate system (see figure ">
<S_F fieldType:10>
<T "xref \"fig:coords\"">
<FV>
<T "2">
<E_F>
<T "). The control of the block size is left to the caller to maxmin. Thus the \
 resolution of the plot may be adjusted, as in the Zoom function. It should be \
 noted that drawing into the canvas is time-consuming. It is for this purpose t\
 hat ">
<T "Studio" italic >
<T " has restricted the size of the canvas to not extend beyond its physical si\
 ze.">
<P "Normal" lineSpacing:0 >
<T "Maxmin" bold >
<T " requires bit resolution and number of channels in the sample, since theses\
  vary the size of the samples.">
<P "Normal" lineSpacing:0 >
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1287 yposParaRelative ypos:2\
 77 width:5708 minHeight:575 leftFrameMargin:125 rightFrameMargin:125 topFrameM\
 argin:0 bottomFrameMargin:0 >
<S_F fieldType:7 docType:201 appType:2>
<T "local_inset \"object2\" -docType \"201\" -appType \"Graphics_\" -clipMode c\
 lipToFit -picture">
<FV>
<picture "object2" appId:2 clipMode:1 width:1893 height:1269 picOrigXoffset:6 p\
 icOrigYoffset:25 picOrigWidth:1893 picOrigHeight:1269>
<E_F>
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1287 yposParaRelative ypos:2\
 77 width:5708 minHeight:575 leftFrameMargin:125 rightFrameMargin:125 topFrameM\
 argin:0 bottomFrameMargin:0 >
<T "figure " italic >
<S_F fieldType:11 editable
<start_eval_value>
<T "--field--">
<end_eval_value>
>
<T "xref_source \"fig:coords\" -live">
<FV>
<S_F fieldType:3>
<T "series pfigure">
<FV>
<T "2">
<E_F>
<E_F>
<T " -X Co-ordinate system" italic >
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1287 yposParaRelative ypos:2\
 77 width:5708 minHeight:575 leftFrameMargin:125 rightFrameMargin:125 topFrameM\
 argin:0 bottomFrameMargin:0 >
<T "Graphical editing">
<P "heading 4" lineSpacing:0 >
<T "All the graphical editing is performed in terms of the canvas. Each line in\
  the plot represents a block of sample data. Upon loading the complete sample \
 is fit within the boundaries of the canvas. Thus the initial resolution is det\
 ermined by the size of the sample file.">
<P "Normal" lineSpacing:0 >
<T "When marking, the boundary positions of the marked area are stored as canva\
 s co-ordinates [">
<T "PLOT(Startx), Plot(Endx)" size:10 >
<T "]. These are then used to determine the amount of data to be skipped at the\
  head and tail of the file to perform the editing operations of ">
<T "copy" italic >
<T ", ">
<T "delete" italic >
<T " and ">
<T "paste" italic >
<T ". These editing operations operate on the working file using the programs "\
 >
<T "head" bold italic >
<T ", or ">
<T "tail" bold italic >
<T ".">
<P "Normal" lineSpacing:0 >
<T "Zoom Levels">
<P "heading 5" lineSpacing:0 >
<T "The ">
<T "Zoom" italic >
<T " operation complicates the editing concept, since the blocks size (bytes pe\
 r vertical line) changes as the resolution is increased.">
<P "Normal" lineSpacing:0 >
<T "To keep track of each zoom operation the context of the current zoom level \
 is stored when a ">
<T "Zoom-Up" italic >
<T " operation is performed. By context is meant the postions of the insertion \
 points and the block size. This is stored in a global called ">
<T "PLOT(Zoom" size:10 >
<T "ZOOM_LEVEL" italic capitals size:10 >
<T ")" size:10 >
<T ", where ">
<T "ZOOM_LEVEL" italic capitals size:10 >
<T " " italic capitals >
<T "is a discrete number ">
<T ">" underline >
<T " 0.">
<P "Normal" lineSpacing:0 >
<S_F fieldType:7 docType:201 appType:2>
<T "local_inset \"object3\" -docType \"201\" -appType \"Graphics_\" -clipMode c\
 lipToFit -picture">
<FV>
<picture "object3" appId:2 clipMode:1 width:4320 height:2663 picOrigXoffset:5 p\
 icOrigYoffset:0 picOrigWidth:4320 picOrigHeight:2663>
<E_F>
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1287 yposParaRelative ypos:7\
 92 width:5676 minHeight:1444 leftFrameMargin:125 rightFrameMargin:125 topFrame\
 Margin:0 bottomFrameMargin:0 >
<T "figure " italic >
<S_F fieldType:11 editable
<start_eval_value>
<T "--field--">
<end_eval_value>
>
<T "xref_source \"fig:graphedit\" -live">
<FV>
<S_F fieldType:3>
<T "series pfigure">
<FV>
<T "3">
<E_F>
<E_F>
<T " - Graphical editing" italic >
<T "." size:10 >
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1287 yposParaRelative ypos:7\
 92 width:5676 minHeight:1444 leftFrameMargin:125 rightFrameMargin:125 topFrame\
 Margin:0 bottomFrameMargin:0 >
<T "With each context change the co-ordinate system is shifted; the left insert\
 ion point is adjusted to the x=0 position. Thus it is necessary to record this\
  offset [">
<T "Plot(Offset)" size:10 >
<T "]. Figure ">
<S_F fieldType:10>
<T "xref \"fig:graphedit\"">
<FV>
<T "3">
<E_F>
<T " illustrates this point.">
<P "Normal" lineSpacing:0 >
<T "Furher, as of version 0.8, it is possible to scale the view of the editing \
 canvas.  This facility is available in addition to the zooming function descri\
 bed above.  When working on a large file, it is always advisable to zoom to th\
 e area of interest, as the updating of the canvas and application of effects i\
 s slower when the whole file is considered rather than a zoomed section.  Magn\
 ification does not section the file in the way that zoom does; it is purely a \
 graphical convenience.">
<P "Normal" lineSpacing:0 >
<T "The selection of magnification level is by pull down menu, which permits th\
 e user to jump several magnification steps at once.  This avoids ">
<T "studio" italic >
<T " having to rescan the file as the user repeatedly presses the \"magnify\" b\
 utton, as would be the case if it were implemented with a similar user interfa\
 ce to the zoom-in and zoom-out functions." no-italic >
<P "Normal" lineSpacing:0 >
<T "Using head and tail">
<P "heading 5" lineSpacing:0 >
<T "The way in which ">
<T "head" no-bold face:"Courier" >
<T " and ">
<T "tail" face:"Courier" >
<T " are used in the context of manipulating the working file is illustrated wi\
 th the examples of the ">
<T "Delete" italic >
<T " and ">
<T "Paste" italic >
<T " operations in figure ">
<S_F fieldType:10>
<T "xref \"fig:cutandpaste\"">
<FV>
<T "4">
<E_F>
<T ".">
<P "Normal" lineSpacing:0 >
<S_F fieldType:7 docType:201 appType:2>
<T "local_inset \"object4\" -docType \"201\" -appType \"Graphics_\" -clipMode c\
 lipToFit -picture">
<FV>
<picture "object4" appId:2 clipMode:1 xScale:0.600000 yScale:0.700000 width:261\
 4 height:1819 picOrigXoffset:85 picOrigYoffset:7 picOrigWidth:4357 picOrigHeig\
 ht:2598>
<E_F>
<T " ">
<S_F fieldType:7 docType:201 appType:2>
<T "local_inset \"object5\" -docType \"201\" -appType \"Graphics_\" -clipMode c\
 lipToFit -picture">
<FV>
<picture "object5" appId:2 clipMode:1 xScale:0.600000 yScale:0.726000 width:282\
 4 height:1813 picOrigXoffset:2 picOrigYoffset:0 picOrigWidth:4706 picOrigHeigh\
 t:2497>
<E_F>
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1307 yposParaRelative ypos:1\
 4 width:5660 minHeight:558 leftFrameMargin:125 rightFrameMargin:125 topFrameMa\
 rgin:0 bottomFrameMargin:0 >
<T "^aj^aj">
<T "(a) Delete^aj^aj^aj^aj(b) Paste" size:10 >
<P "Normal" justifyLeft lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0\
 :"" horizontalMargin:14 verticalMargin:14  xpos:1307 yposParaRelative ypos:14 \
 width:5660 minHeight:558 leftFrameMargin:125 rightFrameMargin:125 topFrameMarg\
 in:0 bottomFrameMargin:0 >
<T "figure " italic >
<S_F fieldType:11 editable
<start_eval_value>
<T "--field--">
<end_eval_value>
>
<T "xref_source \"fig:cutandpaste\" -live">
<FV>
<S_F fieldType:3>
<T "series pfigure">
<FV>
<T "4">
<E_F>
<E_F>
<T " - Deleting and Pasting using head and tail" italic >
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1307 yposParaRelative ypos:1\
 4 width:5660 minHeight:558 leftFrameMargin:125 rightFrameMargin:125 topFrameMa\
 rgin:0 bottomFrameMargin:0 >
<P "Normal" lineSpacing:0 >
<T "Sound Card Interface">
<P "heading 3" lineSpacing:0 >
<T "The sound card interface is a straightforward C-implementation of the conte\
 nt of the VoxWare Hacker's Guide. There are two programs that have been writte\
 n for the purpose of the sound card interface;">
<P "Normal" lineSpacing:0 >
<T "1.^aj">
<T "studio_tool" italic >
<T " (s_tool.c), the dsp interface and diagnostics.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "2.^aj">
<T "studio_mixer" italic >
<T " (s_mixer.c), the mixer interface.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "Both these have an interface that resembles the form of a TCL command. This\
  is because they consist of a number of activities. A syntax guide is shown in\
  figure ">
<S_F fieldType:10>
<T "xref \"fig:syntax\"">
<FV>
<T "5">
<E_F>
<T ".">
<P "Normal" lineSpacing:0 >
<S_F fieldType:7 docType:201 appType:2>
<T "local_inset \"object6\" -docType \"201\" -appType \"Graphics_\" -clipMode c\
 lipToFit -picture">
<FV>
<picture "object6" appId:2 clipMode:1 xScale:0.969000 yScale:0.969000 width:568\
 8 height:2077 picOrigXoffset:7 picOrigYoffset:7 picOrigWidth:5904 picOrigHeigh\
 t:2143>
<E_F>
<T "(a) " size:10 >
<T "studio" italic size:10 >
<T "_mixer" size:10 >
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1267 yposParaRelative ypos:1\
 2 width:5688 minHeight:956 leftFrameMargin:125 rightFrameMargin:125 topFrameMa\
 rgin:0 bottomFrameMargin:0 >
<S_F fieldType:7 docType:201 appType:2>
<T "local_inset \"object7\" -docType \"201\" -appType \"Graphics_\" -clipMode c\
 lipToFit -picture">
<FV>
<picture "object7" appId:2 clipMode:1 width:4678 height:1430 picOrigXoffset:7 p\
 icOrigYoffset:7 picOrigWidth:4678 picOrigHeight:1430>
<E_F>
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1267 yposParaRelative ypos:1\
 2 width:5688 minHeight:956 leftFrameMargin:125 rightFrameMargin:125 topFrameMa\
 rgin:0 bottomFrameMargin:0 >
<T "(b) " size:10 >
<T "studio" italic size:10 >
<T "_tool" size:10 >
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1267 yposParaRelative ypos:1\
 2 width:5688 minHeight:956 leftFrameMargin:125 rightFrameMargin:125 topFrameMa\
 rgin:0 bottomFrameMargin:0 >
<T "figure " italic >
<S_F fieldType:11 editable
<start_eval_value>
<T "--field--">
<end_eval_value>
>
<T "xref_source \"fig:syntax\" -live">
<FV>
<S_F fieldType:3>
<T "series pfigure">
<FV>
<T "5">
<E_F>
<E_F>
<T " - Syntax of Sound Card Applications." italic >
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1267 yposParaRelative ypos:1\
 2 width:5688 minHeight:956 leftFrameMargin:125 rightFrameMargin:125 topFrameMa\
 rgin:0 bottomFrameMargin:0 >
<T "The diagnostic routine simply loops through the sampling rates, channels an\
 d bit resolutions, to determine the limits of the card. This is useful for set\
 ting the parameter limits in the interface.">
<P "Normal" lineSpacing:0 >
<T "Module overview">
<P "heading 3" lineSpacing:0 >
<T "Studio" italic >
<T " is split into several files. The C-code listings are naturally in separate\
  files. The TCL/Tk listings are divided into files, or modules. These modules \
 are organised to make it relatively easy to find the required code segments. "\
 >
<P "Normal" lineSpacing:0 >
<page_break>
<T "There is a module file for:">
<P "Normal" lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajeach menu button in the menu bar.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajeach ">
<T "Studio" italic >
<T " section.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "The exception to this rule is the effect module, which combines the effect \
 bar and effect menu button into one, since they are both very similar in funct\
 ion.">
<P "Normal" lineSpacing:0 >
<T "Additional modules are ">
<P "Normal" lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajthe initialisation module. This ties all the modules together.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajthe miscellaneous module. This consists of routines that don't clearly f\
 it into other modules, or are procedures that are self-sufficient, and thus re\
 -usable.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "Table ">
<S_F fieldType:10>
<T "xref \"tab:modules\"">
<FV>
<T "2">
<E_F>
<T " gives a summary of the ">
<T "Studio" italic >
<T " modules.">
<P "Normal" lineSpacing:0 >
<RS>
<T "Module">
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:21:0:"" bB:21:0:"" >
<T "File Name">
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:21:0:"" bB:21:0:"" >
<T "Purpose">
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:21:0:"" bB:21:0:"" >
<RS>
<T "Edit menu" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" bB:10:0:"" >
<T "Provide call-backs to menu items" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" bB:10:0:"" >
<RS>
<T "Effect" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effect.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Provide call-backs to menu items" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "File menu" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "file.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Provide call-backs to menu items" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Help menu" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "help.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Provide call-backs to menu items" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Information Bar " size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "info.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Create Information bar and associated procedures." size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Initialisation" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "init.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Set-up initial values and bind modules together." size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Menu Bar" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "menu.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Create Menu Bar and menus." size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Miscellaneous" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "\"Global\" routines." size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Mixer Board" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "mixer.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Create and set-up the mixer board. Provide the mixer routines." size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Options menu" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Provide call-backs to menu items" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Control Panel" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "playpanel.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Create and set-up the control panel. Provide the sound card routines." size\
 :10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Editing Plot" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "plot.tk" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Create and set-up the canvas and associated procedures." size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Start-up" size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:1297 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:21:0:"" >
<T "studio" italic size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:947 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:21:0:"" >
<T "To allow entry of required paths, and call initialisation." size:10 >
<P "Normal" justifyLeft lineSpacing:0  xpos:1333 yposParaRelative ypos:18 width\
 :6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0\
  bottomFrameMargin:0 >
<CE width:2739 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:21:0:"" >
<T "table " italic >
<S_F fieldType:11 editable
<start_eval_value>
<T "--field--">
<end_eval_value>
>
<T "xref_source \"tab:modules\" -live">
<FV>
<S_F fieldType:3>
<T "series ptable">
<FV>
<T "2">
<E_F>
<E_F>
<T " - Module Summary" italic >
<P "Normal" justifyCenter lineSpacing:0  xpos:1333 yposParaRelative ypos:18 wid\
 th:6454 minHeight:4583 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin\
 :0 bottomFrameMargin:0 >
<T "Additionally, the Solais version of Studio has its own mixer module which c\
 ommunicates via small C programs with Sun's audio pseudo-device.  The programm\
 er is referred to the appropriate source files for further information." size:\
 12 >
<P "Normal indent" firstIndent:0 leftIndent:0 lineSpacing:0 >
<T "Procedure Categories">
<P "heading 4" lineSpacing:0 >
<T "Since ">
<T "Studio" italic >
<T " is not a single sequential program it is difficult to show how all the dif\
 ferent procedures interact one with another. Thus to get a general idea, the p\
 rocedures contained in these modules are categorised into four  groups. The in\
 teraction between these types of procedure is illustrated in figure ">
<S_F fieldType:10>
<T "xref \"fig:procinteraction\"">
<FV>
<T "6">
<E_F>
<T ".">
<P "Normal" lineSpacing:0 >
<P "Normal" lineSpacing:0 >
<S_F fieldType:7 docType:201 appType:2>
<T "local_inset \"object8\" -docType \"201\" -appType \"Graphics_\" -clipMode c\
 lipToFit -picture">
<FV>
<picture "object8" appId:2 clipMode:1 xScale:0.969000 yScale:0.969000 width:572\
 4 height:2991 picOrigXoffset:0 picOrigYoffset:16 picOrigWidth:5907 picOrigHeig\
 ht:3087>
<E_F>
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1304 yposParaRelative ypos:-\
 183 width:5760 minHeight:633 leftFrameMargin:125 rightFrameMargin:125 topFrame\
 Margin:0 bottomFrameMargin:0 >
<T "figure " italic >
<S_F fieldType:11 editable
<start_eval_value>
<T "--field--">
<end_eval_value>
>
<T "xref_source \"fig:procinteraction\" -live">
<FV>
<S_F fieldType:3>
<T "series pfigure">
<FV>
<T "6">
<E_F>
<E_F>
<T " - Interaction between procedure categories" italic >
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1304 yposParaRelative ypos:-\
 183 width:5760 minHeight:633 leftFrameMargin:125 rightFrameMargin:125 topFrame\
 Margin:0 bottomFrameMargin:0 >
<T "1.^ajWidget Creation Routines (WCR)">
<P "Normal" leftIndent:250 lineSpacing:0 >
<T "Each sound ">
<T "studio" italic >
<T " section is created by a widget creation routine. This is a procedure that \
 performs one of three tasks. These tasks are,">
<P "Normal indent" lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^aj">
<T "Initialisation and configuration" bold >
<T ". If the section does not yet exist, this task creates the section widgets,\
  and attaches call-backs and variables to them. The widgets are always configu\
 red with the current contents of the ConfigSet array, when this task is called\
 . It does not display the section.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^aj">
<T "Show the section" bold >
<T ". This packs the section window into the root window of ">
<T "studio" italic >
<T ". It uses the contents of the PACK array to decide where its position in th\
 e packing order.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^aj">
<T "Remove the section" bold >
<T ". This simply unpacks the section widgets from the root window.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "Some WCRs divided their initialisation task into subroutines, particularly \
 when numerous similar widgets are to be created, e.g. ">
<T "MenuBar" italic >
<T " and ">
<T "Mixer" italic >
<T ".">
<P "Normal indent" firstIndent:492 leftIndent:492 lineSpacing:0 >
<T "2.^ajCall-backs">
<P "Normal" leftIndent:250 lineSpacing:0 >
<T "These are the top level of the sequential programs that are attached to wid\
 get events defined in the WCR. It is in these procedures that the actual proce\
 ssing of ">
<T "Studio " italic >
<T "lies.">
<P "Normal indent" lineSpacing:0 >
<T "3.  Sub-routines">
<P "Normal" leftIndent:250 lineSpacing:0 >
<T "As is usual in sequential programs, often repeated tasks are assigned to su\
 broutines. This category of procedures refers mainly to those subroutines that\
  are used by more than one call-back.">
<P "Normal indent" lineSpacing:0 >
<T "4.^ajDual-purpose routines.">
<P "Normal" leftIndent:250 lineSpacing:0 >
<T "Dual purpose routines are a procedures that serve as a call-back and a subr\
 outine. Examples of these are apparent in the edit module, where the Copy and \
 Delete call-backs are used as a subroutine for the Cut call-back. Dual purpose\
  routines usually have some parameter which switches the procedure from its ca\
 ll-back function to the subroutine function, since these can differ minutely."\
 >
<P "Normal indent" lineSpacing:0 >
<T "5.^ajLocal subroutines.">
<P "Normal" leftIndent:250 lineSpacing:0 >
<T "These subroutines are those which ideally only have scope in one single cal\
 l-back or within a subroutine. However in practise it appears that procedures \
 in TCL/Tk do not have scope like in C-programming; all procedures are global!"\
 >
<P "Normal indent" lineSpacing:0 >
<T "A list of all procedures, their purpose and category is found in table ">
<S_F fieldType:10>
<T "xref \"tab:procs\"">
<FV>
<T "4">
<E_F>
<T " in the listing index.">
<P "Normal" lineSpacing:0 >
<T "Techniques">
<P "heading 2" lineSpacing:0 >
<T "Using TCL lists">
<P "heading 3" lineSpacing:0 >
<T "TCL has an excellent facility for using lists. Thus in the case where many \
 similar widgets were to be created, all it took was to create a list containin\
 g the features that are unique to each widget, and then loop using the ">
<T "foreach" face:"Courier New" >
<T " command and use ">
<T "lindex " face:"Courier New" >
<T "to access compound list items using command or variable substitution.">
<P "Normal" lineSpacing:0 >
<T "Here is a simple example from the procedure which sets up the effect bar se\
 ction;">
<P "Normal" lineSpacing:0 >
<T "# Make the list" italic size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "set effects {Filter Reverse Echo Vibrato}" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "# Loop through the list." italic size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "foreach Effect $effects {" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "^aj" size:10 >
<T "# Convert to lower case for widget path name." italic size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "^ajset effect [string tolower $Effect]" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "^aj" size:10 >
<T "# Create a checkbutton for the current list item" italic size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "^ajcheckbutton .effect.top.$effect -text $Effect" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "^ajpack .effect.top.$effect -side left \\" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "^aj^aj^aj^aj^aj-padx 1" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "^aj" size:10 >
<T "#Bind button 3 to pop up the window to set the settings" italic size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "^ajbind .effect.top.$effect <Button-3> \"s$Effect\"" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  size:10 >
<T "^aj}" size:10 >
<P "listing" lineSpacing:0  size:10 >
<P "Normal" lineSpacing:0 >
<T "Note that, in this case, the list is not complex and thus ">
<T "lindex " face:"Courier New" >
<T "was not used.">
<P "Normal" lineSpacing:0 >
<T "This technique was particularly useful in creating the menu bar and associa\
 ted menus. Thus to modify a menu, it simply requires a list being changed. It \
 also removed the tedium and enabled the automation of creating the mixer board\
  sliders.">
<P "Normal" lineSpacing:0 >
<T "Keeping Track of States">
<P "heading 3" lineSpacing:0 >
<T "Studio" italic >
<T " consists of many states. These many states are flagged in an array of vari\
 ables titled FLAG. The number of states makes programming difficult, because t\
 here are so many ">
<T "if" face:"Courier New" >
<T " statements to keep track of. Of particular importance is ensuring that the\
  user doesn't perform an action that isn't valid for a given state. For exampl\
 e, the user should not try to record whilst ">
<T "Studio" italic >
<T " is playing a sample. The sound card cannot do both at the same time! ">
<P "Normal" lineSpacing:0 >
<T "The method used in ">
<T "studio" italic >
<T " to avoid this kind of conflict is by disabling the relevant widgets. The f\
 ollowing are the techniques used, which ensure that the states of the relevant\
  widgets change at the appropriate time.">
<P "Normal" lineSpacing:0 >
<T "Pop-up Windows">
<P "heading 4" lineSpacing:0 >
<T "There are a number of pop-up window requiring the user to provide some info\
 rmation. When ">
<T "Studio" italic >
<T " is in this state, it is not desirable that another function is called, or \
 even worse the same window is generated again, causing a proliferation of wind\
 ows. To avoid this each dialogue window is created as a transient window and t\
 hen requests the 'grab'. The grab is essentially an X flag which give all the \
 focus of the application to this window. Thus in effect the parent window beco\
 mes disabled, until the pop-up window is destroyed. For example,">
<P "Normal" lineSpacing:0 >
<T "   " face:"Courier New" size:10 >
<T "# Create toplevel window." face:"Courier" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  face:"Courier" size:\
 10 >
<T "   toplevel .echo -bg $ConfigSet(cBG1)" face:"Courier" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  face:"Courier" size:\
 10 >
<T "   wm title .echo \"Echo Settings\"" face:"Courier" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  face:"Courier" size:\
 10 >
<T "   # Set window as a pop-up child of the root window." face:"Courier" size:\
 10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  face:"Courier" size:\
 10 >
<T "   wm transient .echo ." face:"Courier" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  face:"Courier" size:\
 10 >
<T "   # Stop entry from the main window. (grab)." face:"Courier" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  face:"Courier" size:\
 10 >
<T "   catch { grab .echo }" face:"Courier" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  face:"Courier" size:\
 10 >
<T "Passive and Active State control">
<P "heading 4" lineSpacing:0 >
<T "The other methods of state control actually change the state of specific wi\
 dgets. This is somewhat messy since it requires the path name of the widget. T\
 his is particularly awkward because of the automated method in which many of t\
 he widgets are created. Fortunately there are only a few widgets that need to \
 change state. These are, ">
<P "Normal" lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajThe control panel buttons (play, stop and record), depending on the play\
  status.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajThe file menu items, depending on whether a file is loaded.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajThe edit menu items, depending on whether there is clipboard data and wh\
 ether a section of the plot is marked.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "In all these cases there is a procedure which changes the state of the corr\
 esponding widgets according to the status of the flags. The paths of these wid\
 gets are hard coded into these procedures. The way in which these procedures a\
 re called vary.">
<P "Normal" lineSpacing:0 >
<T "The procedure to change the control panel states is called by a code entry \
 after the flag has been changed in the following ">
<T "passive" italic >
<T " manner:">
<P "Normal" lineSpacing:0 >
<T "set FLAG(Playing) 1" face:"Courier" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  face:"Courier" >
<T "# Change Play Panel button states" italic face:"Courier" size:10 >
<P "listing" firstIndent:500 leftIndent:500 lineSpacing:0  face:"Courier" >
<T "  ^ajPP_ButtonAble" face:"Courier" size:10 >
<P "listing" lineSpacing:0  face:"Courier" >
<P "listing" lineSpacing:0  face:"Courier" >
<T "In the latter cases, a more ">
<T "active" italic >
<T " method of doing this same task was discovered , using a variable trace. Th\
 e ">
<T "trace" face:"Courier New" >
<T " command allows a procedure to be called every time a particular variable i\
 s written to.  For example,">
<P "Normal" lineSpacing:0 >
<T "^aj">
<T "trace variable FLAG(Loaded) w FileMenuStates" face:"Courier" >
<P "commline" lineSpacing:0 >
<T "This proves to be an efficient and effective way of tracking the states. Th\
 e passive method is not really required and should be done away with in future\
  modifications. A trace call-back provides information about the variable that\
  has been accessed and in what way is has been accessed (read, write or kill).\
  Thus a single state control mechanism may actually be written. Also for futur\
 e work would be to over come the inflexibility of having to ">
<T "hard code" italic >
<T " the widgets' path names.">
<P "Normal" lineSpacing:0 >
<T "Generating Pipes">
<P "heading 3" lineSpacing:0 >
<T "Studio" italic >
<T " makes use of a number of UNIX system commands, plus the use of Sox. The ">
<T "grep, awk, dd" italic >
<T ", ">
<T "mv" italic >
<T " and ">
<T "cp" italic >
<T " commands are all used. In many cases these are often use in pipes, just as\
  they would be from the command line. Where this is of particular importance i\
 s in using ">
<T "Sox " italic >
<T "to add effects to the sample. In it's syntax ">
<T "Sox " italic >
<T "has only scope to add one effect per command call. Multiple effects may onl\
 y be added by piping the sample data through a number of ">
<T "Sox " italic >
<T "calls. Due to TCl/Tk's simple variable substitution, it is possible to crea\
 te a pipe dynamically so that only the selected effects are applied to the sam\
 ple. A variable to contain the command string pipe is built up by adding the p\
 ipe section of each selected effect, and then appending the output file. Thus \
 the constructed pipe string might be,">
<P "Normal" lineSpacing:0 >
<T "pipe = \"" face:"Courier" size:12 >
<T "sox " italic face:"Courier" size:12 >
<T "-t .ub " face:"Courier" size:12 >
<T "infile" italic face:"Courier" size:12 >
<T " -t .ub - reverse | \\" face:"Courier" size:12 >
<P "Table" lineSpacing:0  face:"Courier" size:12 >
<T "        sox " italic face:"Courier" size:12 >
<T "-t .ub -" face:"Courier" size:12 >
<T " outfile echo .5 .5\"" italic face:"Courier" size:12 >
<P "Table" lineSpacing:0  face:"Courier" size:12 >
<P "Table" lineSpacing:0  face:"Courier" >
<T "This is then executed in the following manner,">
<P "Normal" lineSpacing:0 >
<T "eval exec $pipe" face:"Courier" >
<P "Table" justifyCenter lineSpacing:0 >
<P "Table" justifyCenter lineSpacing:0 >
<T "In addition, a small C program has been written to enable the fade-in and f\
 ade-out of samples.  ">
<T "fader.c" face:"Courier" >
<S_F fieldType:4 botPage footnote "3"
<start_eval_value>
<T "1" position:3 >
<end_eval_value>
>
<T "footnote">
<FV>
<T "1" face:"Courier" position:3 >
<E_F>
<T " has the following call syntax:">
<P "Table" justifyFull lineSpacing:0 >
<P "Table" lineSpacing:0 >
<T "fader <channels> <sample_type> <fade_up_samples> <fade_down_samples>" face:\
 "Courier" size:10 >
<P "Normal" justifyCenter lineSpacing:0  face:"Courier" size:10 >
<T "where the sample type parameter may indicate signed or unsigned bytes or wo\
 rds according to the sox syntax.  Fader imposes a liner fade up to a level of \
 -60dB, then fades exponentially to the final level.">
<P "Table" justifyFull lineSpacing:0 >
<T "Configuration File">
<P "heading 3" lineSpacing:0 >
<T "Sound Studio" italic >
<T " provides a mechanism which allows the user to set-up ">
<T "studio" italic >
<T " to his/her preference. This is through a configuration file that is set-up\
  in the user's home directory. The main motivation for doing this was my disli\
 ke for the peach colour inherent to TCL/tk applications! Thus it seemed approp\
 riate to create a mechanism allowing the user to change these colours and have\
  these colour settings remain for subsequent running of ">
<T "Studio" italic >
<T ".  Requesting a \"make default\" menu entry has the effect of updating this\
  file, provifing a user-friendly way of selecting and saving default settings.\
 ">
<P "Normal" lineSpacing:0 >
<T "The idea is simple:">
<P "Normal" lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajAll the WCRs in ">
<T "studio" italic >
<T " use the contents of the ConfigSet array to configure the widgets.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajA dialogue box to allow the user to change the contents of this array is\
  provided. ">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajThe creation and configuration tasks of WCR are kept distinct from each \
 other, so that widgets may be re-configured whilst ">
<T "Studio" italic >
<T " is running.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajThe contents of the array may be stored in a configuration file upon use\
 r request.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "This idea was extended to allow the user to store the view options, i.e. wh\
 at sections of ">
<T "Studio" italic >
<T " are to be displayed and also the mixer sliders to be contained in the Mixe\
 r Board. However these options already had a place in the FLAG and MIXER array\
 . This complicated the saving procedure slightly since now there are the conte\
 nts of three arrays to be saved and it must be possible to save these independ\
 ently of the others into the configuration file.">
<P "Normal" lineSpacing:0 >
<T "The general mechanism for doing this is illustrated in figure ">
<S_F fieldType:10>
<T "xref \"fig:fileconfig\"">
<FV>
<T "7">
<E_F>
<T ". As illustrated the configuration file is essentially a table with three c\
 olumns. The index and description correspond to the ConfigInfo array. The firs\
 t letter of these indices indicates which array the data belongs to. The chara\
 cters after the first letter represent the index of the array to which the dat\
 a belongs. This is all sorted out in the procedures ">
<T "LoadSettings" face:"Courier New" >
<T " and ">
<T "SaveSettings" face:"Courier New" >
<T " in the options module.">
<P "Normal" lineSpacing:0 >
<S_F fieldType:7 docType:201 appType:2>
<T "local_inset \"object9\" -docType \"201\" -appType \"Graphics_\" -clipMode c\
 lipToFit -picture">
<FV>
<picture "object9" appId:2 clipMode:1 xScale:0.895000 yScale:0.895000 width:569\
 3 height:2410 picOrigXoffset:0 picOrigYoffset:0 picOrigWidth:6361 picOrigHeigh\
 t:2693>
<E_F>
<P "Normal" lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" horizont\
 alMargin:14 verticalMargin:14  xpos:1301 yposParaRelative ypos:474 width:5735 \
 minHeight:683 leftFrameMargin:125 rightFrameMargin:125 topFrameMargin:0 bottom\
 FrameMargin:0 >
<T "figure " italic >
<S_F fieldType:11 editable
<start_eval_value>
<T "--field--">
<end_eval_value>
>
<T "xref_source \"fig:fileconfig\" -live">
<FV>
<S_F fieldType:3>
<T "series pfigure">
<FV>
<T "7">
<E_F>
<E_F>
<T " - Configuration file Mechanism" italic >
<P "Normal" justifyCenter lineSpacing:0  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10\
 :0:"" horizontalMargin:14 verticalMargin:14  xpos:1301 yposParaRelative ypos:4\
 74 width:5735 minHeight:683 leftFrameMargin:125 rightFrameMargin:125 topFrameM\
 argin:0 bottomFrameMargin:0 >
<T "The description is provided   to provide useful text for dialogue boxes">
<S_F fieldType:4 botPage footnote "1">
<T "footnote">
<FV>
<T "2" position:3 >
<E_F>
<T " and also to make the configuration file readable.">
<P "Normal" lineSpacing:0 >
<T "It is hoped that this mechanism is sufficient for future extensions to the \
 preference options. Unfortunately his ">
<T "simple" italic >
<T " job was a bigger task than anticipated and was therefore rushed!">
<P "Normal" lineSpacing:0 >
<T "Icons">
<P "heading 3" lineSpacing:0 >
<T "The icons used in ">
<T "Studio" italic >
<T " were generated by the X application called ">
<T "bitmap." italic >
<T " There are two sizes of bitmap used in ">
<T "Studio" italic >
<T ".">
<P "Normal" lineSpacing:0 >
<T "^lh" face:"Symbol" >
<T "^aj64x64, for the iconified version of ">
<T "Studio" italic >
<T " and the ">
<T "User Manual" italic >
<T ".">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" >
<T "^aj16x16, for the button icons in the control panel.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" >
<T "^aj32x32, for the magnification zoom buttons.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "Each of these bitmaps are stored in files with the extension .ico, and are \
 read into ">
<T "Studio" italic >
<T " using the @">
<T "filename" italic >
<T " method">
<T ". " italic >
<T "A way to combined all these bitmaps into one file was not found.">
<P "Normal" lineSpacing:0 >
<T "The Playline and VU Meters">
<P "heading 2" lineSpacing:0 >
<T "As of version 1.0, Studio supports a playline which gives visual feedback a\
 s to the approximate play position on the plot panel, and VU meters with peak \
 hold to assist in setting recording level and assessing playback volume.">
<P "Normal" lineSpacing:0 >
<T "Since TCL is merely a scripting language, it became clear very early on tha\
 t it would not be feasible to implement either the playline or the VU meters i\
 n TCL alone, especially when dealing with the real time issues that both would\
  raise. The most feasible solution was to have a C application do all the ``nu\
 mber crunching'' and pass information to studio which would then be used direc\
 tly to produce the desired results. It seemed logical to modify the studio_too\
 l application to not only play or record the required soundfile, but also to d\
 o the necessary working and send the required data back to studio.">
<P "Normal" lineSpacing:0 >
<T "This necessitated a change to the data flow, as previously studio only sent\
  data out to the studio_tool application by actually executing it, with the co\
 mmand line parameters set within studio, and then possibly setting an internal\
  variable equal to the output of studio_tool after it had finished executing. \
 In this way, there was no communication between studio and studio_tool, as stu\
 dio_tool was actually running. To implement both the playline and the VU meter\
 s a communication channel was needed to be established between studio and stud\
 io_tool.  The responsible code reads as follows:">
<P "Normal" lineSpacing:0 >
<T "# execute studio_tool and open a channel" italic face:"Courier Swa" size:10\
  >
<P "listing" lineSpacing:0  face:"Courier Swa" size:10  noFrame >
<T "set pipe [open \"|./$FILE(Tool) play $source $LOAD(bit) $LOAD(chnl)\\" face\
 :"Courier Swa" size:10 >
<P "listing" lineSpacing:0  face:"Courier Swa" size:10  noFrame >
<T "                           $LOAD(rate) meter 75 1 $linelist\" r]" face:"Cou\
 rier Swa" size:10 >
<P "listing" lineSpacing:0  face:"Courier Swa" size:10  noFrame >
<T "# Call function Reader whenever new data comes in the channel" italic face:\
 "Courier Swa" size:10 >
<P "listing" lineSpacing:0  face:"Courier Swa" size:10  noFrame >
<T "fileevent $pipe readable [list Reader $pipe]" face:"Courier Swa" size:10 >
<P "listing" lineSpacing:0  face:"Courier Swa" size:10  noFrame >
<P "Normal" lineSpacing:0 >
<T "The fileevent command forces ">
<T "Studio" italic >
<T " into an event loop, calling the function ">
<T "Reader" face:"Courier" >
<T " every time a new piece of data came in on the channel. ">
<T "Reader" face:"Courier" >
<T " then reads the data from the channel, checks whether it is a VU update req\
 uest or a play-line motion request, and performes the required task.">
<P "Normal" lineSpacing:0 >
<T "Preserving the functionality of studio_tool" underline size:12 >
<P "heading 3" lineSpacing:0 >
<T "studio_tool" italic >
<T " serves not only as the mechanism by which studio interfaces with the sound\
  card, but also as a useful stand-alone program that could play and record sou\
 nd files from the command line prompt. It was therefore required that ">
<T "studio_tool" italic >
<T " would only emit playline and VU data only if requested to do so in the com\
 mand line arguments. In this way, ">
<T "studio_tool" italic >
<T " could still be used from the command line prompt, without flooding the scr\
 een with unnecessary data. For example:">
<P "Normal" lineSpacing:0 >
<T "% studio_tool play ultravibe.au 16 1 11025" face:"Courier" size:12 >
<P "listing" lineSpacing:0  face:"Courier" size:12 >
<P "listing" lineSpacing:0  face:"Courier" size:12 >
<T "will play the soundfile with the specified rate, precision and channels. Th\
 is would be how studio_tool would be used from the command line prompt., where\
 as:">
<P "Normal" lineSpacing:0 >
<T "% studio_tool play ultravibe.au 16 11025 meter 20 1 line 499 109779" face:"\
 Courier" size:10 >
<P "Normal" lineSpacing:0 >
<T "will play the soundfile, but also emit the VU meter and play-line data. The\
  extra parameters in the command line were designed only to be used by ">
<T "studio" italic >
<T ", as their calculation is probably too cumbersome to be expected of a user.\
 " no-italic >
<P "Normal" lineSpacing:0 >
<T "Dealing with the Real Time Issues" underline size:12 >
<P "heading 3" lineSpacing:0 >
<T "Any program that deals with performing timing on audio data faces real time\
  issues, which must be addressed. For both the playline and the VU meter, the \
 program needs to know exactly how much of the soundfile had been processed at \
 all times. Both Solaris and Linux had very convenient ioctl calls that returne\
 d this information. In Solaris the call is:">
<P "Normal" lineSpacing:0 >
<T "ioctl(fd,AUDIO_GETDEV,&audio_info)" face:"Courier" >
<P "Normal" lineSpacing:0 >
<T "which fills the structure ">
<T "audio_info" face:"Courier" >
<T " with the current soundcard information. One of the fields of ">
<T "audio_info" face:"Courier" >
<T " is ">
<T "play.samples" face:"Courier" >
<T " which stores the number of samples that the soundcard had processed.">
<P "Normal" lineSpacing:0 >
<T "The ioctl command in Linux is:">
<P "Normal" lineSpacing:0 >
<T "ioctl(fd,SNDCTL_DSP_GETOPTR,&info)" face:"Courier" >
<P "Normal" lineSpacing:0 >
<T "which fills the structure ">
<T "info" face:"Courier" >
<T " with soundcard information. One of the fields of ">
<T "info" face:"Courier" >
<T " is bytes which stored the number of bytes that the soundcard had processed\
 . Using the channels and precision to work out the sample size (in bytes), thi\
 s value is easily converted to the number of samples processed.">
<P "Normal" lineSpacing:0 >
<T "In Solaris, the number of samples processed was not read from the soundcard\
 , but from the pseudo audio-device, ">
<T "/dev/audioctl" face:"Courier" >
<T ":">
<P "Normal" lineSpacing:0 >
<T "        if (PLATFORM == OS_SUNOS)">
<P "listing" lineSpacing:0 >
<T "        {">
<P "listing" lineSpacing:0 >
<T "                if ((fd2=open(\"/dev/audioctl\", O_RDONLY,0))== -1)">
<P "listing" lineSpacing:0 >
<T "                {">
<P "listing" lineSpacing:0 >
<T "                        printf(\"Read Failure\\n\");">
<P "listing" lineSpacing:0 >
<T "                        exit(-1);">
<P "listing" lineSpacing:0 >
<T "                }">
<P "listing" lineSpacing:0 >
<T "        }">
<P "listing" lineSpacing:0 >
<P "listing" lineSpacing:0 >
<T "This is a control device for the sound hardware that can be used to check o\
 r set the status of the soundcard, but read() or write() system calls to it ar\
 e ignored. The pseudo-device returns the number of samples in blocks of 8 samp\
 les, whereas using the soundcard the blocksize is 512 samples. Using the Pseud\
 o device, ">
<T "studio_tool" italic >
<T " is able to track the number of samples processed more accurately.">
<P "Normal" lineSpacing:0 >
<T "In Linux, though, an extra few lines of code were required in order to set \
 the fragment size on the soundcard buffer to 64 samples:">
<P "Normal" lineSpacing:0 >
<T "if (PLATFORM == OS_LINUX)">
<P "listing" lineSpacing:0 >
<T "        {">
<P "listing" lineSpacing:0 >
<T "                if (samplesize == 1) fragsize = 0x7fff0006;">
<P "listing" lineSpacing:0 >
<T "                if (samplesize == 2) fragsize = 0x7fff0007;">
<P "listing" lineSpacing:0 >
<T "                if (samplesize == 4) fragsize = 0x7fff0008;">
<P "listing" lineSpacing:0 >
<T "                if (samplesize == 8) fragsize = 0x7fff0009;">
<P "listing" lineSpacing:0 >
<T "                if (samplesize == 16) fragsize = 0x7fff0010;">
<P "listing" lineSpacing:0 >
<T "                ioctl(fd, SNDCTL_DSP_SETFRAGMENT, &fragsize);">
<P "listing" lineSpacing:0 >
<T "        }">
<P "listing" lineSpacing:0 >
<P "Normal" lineSpacing:0 >
<T "Knowing the number of samples processed at all times proved to be vital in \
 solving the real-time issues, as the soundcard itself (or its pseudo control d\
 evice) is used to regulate the flow of output data.">
<P "Normal" lineSpacing:0 >
<T "Separate Processes Vs POSIX Threads" underline size:12 >
<P "heading 3" lineSpacing:0 >
<T "The play function in the previous version of ">
<T "studio_tool" italic >
<T " only performed one function: it played the soundfile. To generate the nece\
 ssary output, the play function had to be modified to perform multiple tasks. \
 These were (in execute-initiation order):">
<P "Normal" lineSpacing:0 >
<T "1.^ajTo play the soundfile">
<P "Normal" firstIndent:0 leftIndent:312 lineSpacing:0 >
<T "2.^ajTo calculate and output the play line data">
<P "Normal" firstIndent:0 leftIndent:312 lineSpacing:0 >
<T "3.^ajTo calculate and output the VU data">
<P "Normal" firstIndent:0 leftIndent:312 lineSpacing:0 >
<T "4.^ajTo calculate and output a regular decay on the VU signal">
<P "Normal" firstIndent:0 leftIndent:312 lineSpacing:0 >
<T "Obviously, these multiple processes would need to be run concurrently withi\
 n the play function in order for the program to work as desired. This posed th\
 e question of whether to use separate processes or POSIX threads. At first it \
 seemed that threads would have been the best solution, as the VU meter uses tw\
 o separate processes that would be required to share certain variables. They a\
 re also much more lightweight, since a whole new process is not created, there\
 fore threads do not require to be killed off, they are automatically killed wh\
 en the main process terminates. ">
<P "Normal" lineSpacing:0 >
<T "Unfortunately, the use of threads did not allocate processor time fairly to\
  each of the processes; the busier tasks would \"hog\" the processor, and this\
  would be wholly unsuitable for processes with real time issues.  Subsequent i\
 mprovements of the control algorithms prior to version^ka1.0 have enormously r\
 educted the computational overhead by the introduction of appropriate sleep()s\
  within the poll loops, so it is likely that this reason has been removed.  Ho\
 wever, these improvements were achieved only after the structure of the new ">
<T "studio_tool" italic >
<T " had matured." no-italic >
<P "Normal" lineSpacing:0 >
<T "Forking Hell" underline size:12 >
<P "heading 3" lineSpacing:0 >
<T "Separate processes are used to perform the multiple tasking. This raises tw\
 o issues that need to be resolved. Firstly when a ">
<T "fork" face:"Courier" >
<T "() system call was performed, the child takes a copy of all the parent's va\
 riables, but that is exactly what they are: copies.  They are not shared. Chan\
 ges made to a certain variable in the child do not appear in the same variable\
  in the parent. To enable processes to communicate it is necessary to set-up a\
 s a shared structure. This is achieved by the following code:">
<P "Normal" lineSpacing:0 >
<T "/*  get unique key for shared memory block  */">
<P "listing" lineSpacing:0 >
<T "if ((VUmemid = shmget (IPC_PRIVATE,">
<P "listing" lineSpacing:0 >
<T "                                              sizeof(SharedVUVariables),066\
 0)) == -1) {">
<P "listing" lineSpacing:0 >
<T "           fprintf(stderr,\"Shared Memory Failure\");">
<P "listing" lineSpacing:0 >
<T "           exit (-1);">
<P "listing" lineSpacing:0 >
<T "}">
<P "listing" lineSpacing:0 >
<T "  /* attach the variable VUdata to shared memory block */">
<P "listing" lineSpacing:0 >
<T "VUdata  = (char *)shmat (VUmemid,(char *)0,0);">
<P "listing" lineSpacing:0 >
<T "/* setup shared memory block as transient */">
<P "listing" lineSpacing:0 >
<T "shmctl (VUmemid,IPC_RMID,NULL);">
<P "listing" lineSpacing:0 >
<P "Normal" lineSpacing:0 >
<T "The ">
<T "shmget" face:"Courier" >
<T "() call sets up a unique key (">
<T "VUmemid" face:"Courier" >
<T ") for a block of shared memory, which was attached to the variable VUdata b\
 y the ">
<T "shmat" face:"Courier" >
<T "() call. Finally, the ">
<T "shmctl" face:"Courier" >
<T "() call sets up the shared memory block as transient, which is used to prev\
 ent memory leaks.">
<P "Normal" lineSpacing:0 >
<T "The second issue is that of terminating the processes. When ">
<T "studio_tool" italic >
<T " is used by ">
<T "studio" italic >
<T ", it could be terminated in two ways. Firstly, it terminates when the sound\
 file had finished, but it could also be terminated by executing a system kill(\
 ) command within studio (which happens when the stop button is pressed). This \
 action actually sent ">
<T "studio_tool" italic face:"Arial" >
<T " a SIGTERM signal, so ">
<T "studio_tool" italic >
<T " needs to be able to catch this signal and kill off all the child processes\
  before quitting. This is achieved by the use of a ">
<T "signal" face:"Courier" >
<T "() call directly after each ">
<T "fork" face:"Courier" >
<T "() call, which nominates a quitting function to be executed when the signal\
  SIGTERM was received by each process:">
<P "Normal" lineSpacing:0 >
<T " signal(SIGTERM,sigquit);" face:"Courier" >
<P "Normal" lineSpacing:0  face:"Courier" >
<T "Also, the last command of each process is a kill() command, by which the pr\
 ocess sent itself a SIGTERM signal on its natural completion:">
<P "Normal" lineSpacing:0 >
<T " kill(pid_number,SIGTERM);" face:"Courier" >
<P "Normal" lineSpacing:0  face:"Courier" >
<T "The play function uses four ">
<T "fork" face:"Courier" >
<T "() calls to execute each of its four tasks. The processes were dealt with i\
 n a last come-first served manner, so the code for the first process to be dea\
 lt with, which was the actual playing of the sound file, appears in the child \
 of the last fork().">
<P "Normal" lineSpacing:0 >
<T "Working Out the Data for the Playline" underline >
<P "heading 3" lineSpacing:0 >
<T "After loading a sound file into studio, it displays a graphical representat\
 ion of the sound file in the PLOT canvas. The basic function of the playline i\
 s to be a visual indicator of how much of the sound file has been played, as i\
 t is being played. In operation this is simply a vertical line that moves from\
  left to right in the PLOT canvas at a speed that ensures that the location of\
  the line is at all time representative of the current position in the sound f\
 ile.">
<P "Normal" lineSpacing:0 >
<T "In order to calculate the required data for the play line, ">
<T "studio_tool" italic >
<T " needs to be passed two extra pieces of data by ">
<T "studio" italic >
<T ". These are the number of pixels in the PLOT canvas, and the total number o\
 f samples in the soundfile to be played.">
<P "Normal" lineSpacing:0 >
<T "The code which emits the output data for the playline is in the third fork(\
 ), so that it was dealt with directly after the soundfile was played.  The out\
 put from this process is \"l.n\" (a letter l, a dot, and a number correspondin\
 g to the number of display pixels that the play-line should be moved).  The pl\
 ay status is polled roughtly every 15ms, when the current audio sample number \
 is read from the sound device.">
<P "Normal" lineSpacing:0 >
<T "Data required to Simulate the VU meter" underline >
<P "heading 3" lineSpacing:0 >
<T "A VU meter displays the average volume level of an audio signal. Its readin\
 g corresponds closely to the level sensing mechanism of the human ear, and so \
 provides a very useful indication of the subjective loudness of an audio signa\
 l.">
<P "Normal" lineSpacing:0 >
<T "A VU meter has two attributes which determine its behaviour, the sample att\
 ack time, and its decay rate. In the real world a VU meter is a peak meter wit\
 h very slow ballistics, therefore it is not very good at tracking fast changes\
  in audio peak levels, as the meter is forced to \"sample\" the signal over a \
 longer time period than a peak meter. This means that the needle is less activ\
 e than that of a peak meter and in this way an average of the audio signal lev\
 els if displayed.">
<P "Normal" lineSpacing:0 >
<T "In order to calculate the data required to simulate a VU meter, ">
<T "studio_tool" italic >
<T " required two extra pieces of data, namely the sample attack time, and the \
 decay rate, which were passed to it by ">
<T "studio" italic >
<T " as command line arguments. From the meter attack time, and the rate of the\
  sound file the number of samples per attack period are calculated.">
<P "Normal" lineSpacing:0 >
<T "Since there are two aspects of simulating the VU meter, namely the actual a\
 mplitude output and the meter decay, two separate processes were instantiated.\
  The first process is quite similar to the playline process in that, while the\
  value of outputnow was zero, it checks the soundcard or control device on pla\
 yback or the recorded file size on record.">
<P "Normal" lineSpacing:0 >
<T "The amplitude data in value, was converted to decibels by taking the log">
<T "10" size:10 position:-3 >
<T " of the normalised amplitude and multiplying by 20.  The result is stored i\
 f it exceeds the current VU reading and is emitted to ">
<T "stdout" face:"Courier" >
<T " in the format \"v.n\".  Sumultaneously, every time a VU decay period elaps\
 es, the decay process awakens and reduces the stored VU value by an appropriat\
 e amount.">
<P "Normal" lineSpacing:0 >
<T "Changes to the tk Script" underline size:14 >
<P "heading 2" lineSpacing:0 >
<T "Reading from the channel" size:12 >
<P "heading 4" lineSpacing:0 >
<T "When playing or recording, the version^ka<^ka1.0 of ">
<T "studio" italic >
<T " interfaced with the soundcard by executing ">
<T "studio_tool" italic >
<T " from within its script. This command is replaced by an open command, which\
  executes ">
<T "studio_tool" italic >
<T ", and set up a pipe and a fileevent command, which ran the TCL procedure ">
<T "Reader" face:"Courier" >
<T " every time a new piece of data came in on the channel.">
<P "Normal" lineSpacing:0 >
<T "The function of the ">
<T "Reader" face:"Courier" >
<T " procedure was to read a line of data from ">
<T "studio_tool" italic >
<T "'s output, parse it to decide what the purpose of the data was, and to use \
 that data to perform the required task.">
<P "Normal" lineSpacing:0 >
<T "Moving the Play Line" size:12 >
<P "heading 4" lineSpacing:0 >
<T "The data that studio_tool passed to Studio relating to the play line was a \
 series of numbers. These represented the number of pixels that the line needed\
  to be moved in the PLOT canvas. The implementation of the line was very simpl\
 e, as Tk has the ability to create a line (or any shape) in a canvas and move \
 it with one command:">
<P "Normal" lineSpacing:0 >
<T ".canvasname move itemid x-pixels y-pixels" face:"Courier" >
<P "Normal" lineSpacing:0 >
<T "This means that Studio needs to create a line during its the PLOT canvas in\
 itialisation, and set a global variable equal to the line-id thus:set LINE(Id)\
 ">
<P "Normal" lineSpacing:0  face:"Courier" >
<T " ">
<T "[.plotbox.cs.canvas create line 0 0 0 50 -fill red]" face:"Courier" >
<P "Normal" lineSpacing:0  face:"Courier" >
<T "Two counters of the number of pixels moved were also maintained. One may be\
  used in resetting the line after playing has finished. The other is used to t\
 rack the line at higher magnifications, by switching to the next portion of th\
 e PLOT canvas when the line approached the right hand side of the PLOT window.\
 ">
<P "Normal" lineSpacing:0 >
<T "Simulating the VU Meter" size:12 >
<P "heading 4" lineSpacing:0 >
<T "This is achieved with the help of two procedures. ">
<T "SetupVU_Display" face:"Courier" >
<T " {}, creates the VU meter toplevel window, with the required number of chan\
 nels, and ">
<T "light" face:"Courier" >
<T "{} displays the required number of lit LEDs.">
<P "Normal" lineSpacing:0 >
<T "The LED lights were defined as movable objects, in the same way as playline\
 , except they were created OFF the canvas, and moved on to the canvas as requi\
 red. ">
<P "Normal" lineSpacing:0 >
<T "Sox Version 15 upgrades" size:12 >
<P "heading 4" lineSpacing:0 >
<T "Implementing the support for the new version of ">
<T "Sox" italic >
<T " simply involved Studio executing \"sox --h\" during the initial load-up. "\
 >
<T "Sox" italic >
<T " version 12.15 returns \"SOX version 12.15\" in the first line of the outpu\
 t, which Studio is able to catch and determine whether of not ">
<T "Sox" italic >
<T " was version 12.15 or not. If so, the global flag SOXVersion15 is set to 1,\
  otherwise it was set to 0.">
<P "Normal" lineSpacing:0 >
<T "If SOXVersion15 was set to 1, Studio adds the extra-effect selectors to the\
  Effects bar, and also to the Effects menu. The echo effect, which was already\
  present in the older versions of ">
<T "Sox" italic >
<T ", are bound to a different callback function, due to the echo effect arguma\
 nts being changed in Sox-12.15. Extra callback functions were also needed for \
 each of the other new effects, which were almost identical to the callback fun\
 ctions for the older ">
<T "Sox" italic >
<T " effects.">
<P "Normal" lineSpacing:0 >
<T "When using the new 12.15 effects, ">
<T "Sox" italic >
<T " would possibly produce warning messages, indicating that the output soundf\
 ile had clipped in parts. Studio caught the output from the use of ">
<T "Sox" italic >
<T ", and produced a dialog box, that contained a text widget, which displayed \
 these warnings. The global variable dh was set to ``don't know'' and a vwait c\
 ommand was used to halt everything until this variable had been changed.">
<P "Normal" lineSpacing:0 >
<T "Known BUGS">
<P "heading 2" lineSpacing:0 >
<T "Format Conversion">
<P "heading 3" lineSpacing:0 >
<T "Studio" italic >
<T " is not yet fully compatible with all ">
<T "Sox " italic >
<T "format conversions. This applies mainly to converting the raw data into the\
  sample format requested of the user. There are a number of conversion that fa\
 il. These are,">
<P "Normal" lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajcdr. This needs the sampling parameters of cd.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajdat. This is very time consuming and fails.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajhcom. Requires sampling rates of, 5512,7350,11025, 20500 only.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajsf. Saves data successfully but format doesn't.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "^lh" face:"Symbol" size:10 >
<T "^ajsmp. Saves incorrectly.">
<P "Normal indent" firstIndent:500 leftIndent:750 lineSpacing:0 >
<T "Another bug, or even maybe the same bug, is due to the way that ">
<T "Studio" italic >
<T " uses ">
<T "Sox " italic >
<T "to down-convert the loaded sample, if it's sampling parameters exceeds that\
  of the sound card. This is to ensure that playback is possible. However, this\
  proves a problem when loading a 2-channel sample on a 1-channel set-up. The c\
 onversion into ">
<T "Studio" italic >
<T " is fine. The sample, however, cannot be saved back into the 2-channel form\
 , this is because the individual channel data is lost through averaging. This \
 is really a problem of having this conversion in the wrong place. The averagin\
 g and down-conversion should really happen upon play-back and not upon loading\
 . The flag, ">
<T "FLAG(Converted)" size:10 >
<T ", is already in place to make this possible. ">
<P "Normal" lineSpacing:0 >
<T "Studio" italic >
<T " does not provide any error detection of ">
<T "Sox " italic >
<T "errors, they are simply ignored. The reason for this is that ">
<T "Sox " italic >
<T "provides information through the standard error file. This information is n\
 ot always error information. Thus handling the exception using ">
<T "catch" face:"Courier New" >
<T " is awkward. Currently the only compromise is to print the error informatio\
 n to standard out, until all the errors are ironed out.">
<P "Normal" lineSpacing:0 >
<T "From this it is clear that a review of the whole loading and saving mechani\
 sm is required.">
<P "Normal" lineSpacing:0 >
<T "Zoom">
<P "heading 3" lineSpacing:0 >
<T "Currently ">
<T "Studio" italic >
<T " reverts to the root zoom level if an editing operation has been performed \
 or effects added. This is because of the ">
<T "FillPlot" italic >
<T " call that is performed at the end of all these operations. This call is re\
 quired to update the contents of the plot as well as recalculating the sample \
 size. It is an awkward side effect that can be annoying to the user. It may be\
  possible to overcome this problem at least in part if the zoom context where \
 used in the ">
<T "FillPlot" italic >
<T " operation.">
<P "Normal" lineSpacing:0 >
<T "Playline in preview-mode">
<P "heading 3" lineSpacing:0 >
<T "The playline does not traverse the audio plot in preview mode.  This is sim\
 ply because the variety of effects which Studio can accommodate, via ">
<T "Sox" italic >
<T " and otherwise, has too great an influence on the behaviour of the playline\
  to make a general solution feasilble.  For example, " no-italic >
<T "Sox" italic >
<T " can change the \"tempo\" of a file, or even cause it to be played backward\
 s! Because the sound-processing program is not tightly bound to studio develop\
 ment, it was felt the best course was to suppress the playline appearance.  Th\
 is is really more of a limitation than a bug." no-italic >
<P "Normal" lineSpacing:0 >
<T "SOUND BLASTER 8">
<P "heading 3" lineSpacing:0 >
<T "In testing, an 8-bit Sound Blaster card would record successfully at all sa\
 mpling rates, but would only play-back, correctly, recordings sampled with a s\
 ampling rate of 12kHz or less. This appears to be a driver problem. The driver\
  is set correctly to the correct sampling rate and responds that is set to the\
  sampling rate, however it just doesn't playback correctly. It cannot be a sou\
 nd card problem since recording and playback is successful at all available sa\
 mpling rates in DOS sound sample applications.">
<P "Normal" lineSpacing:0 >
<T "Tests on a 16-bit Sound Blaster did not present this problem.">
<P "Normal" lineSpacing:0 >
<T "VoxWare Manual Error">
<P "heading 3" lineSpacing:0 >
<T "In programming the mixer interface it was found that it is not possible to \
 access the mixer though the dsp device as explained in the VoxWare Hacker's gu\
 ide">
<S_F fieldType:4 botPage footnote "2">
<T "footnote">
<FV>
<T "3" position:3 >
<E_F>
<T ". Hours of frustration resulted because of this error. The mixer functions \
 seem only available by ioctl calls to the open mixer device.">
<P "Normal" lineSpacing:0 >
<T "Playline does not move smoothly (Linux)">
<P "heading 3" lineSpacing:0 >
<T "studio_tool" italic >
<T " relies on an ioctrl() call to the sound drivers to obtain information abou\
 t the current output sample.  This is unavailable on some hardware, and under \
 these circumstatnces the kernel falls back to making a \"best guess\" based on\
  the number of buffer fragments written to the card so far.  Unfortuately, the\
  best guess is not always good enough to ensure the playline moves smoothly, e\
 specially on small files." no-italic >
<P "Normal" lineSpacing:0 >
<T "Playline stalls on moving the Mixer Controls (Solaris)">
<P "heading 3" lineSpacing:0 >
<T "The hardware on which the program has been tested (Ultra workstations) supp\
 orts high resolution sample counts (see above), but any access to the audio de\
 vice resets the counter.  This invalidates the current-play-position variables\
  within ">
<T "Studio" italic >
<T " and causes the playline to malfunction for the rest of the playback period\
 .  Since Solaris is not our primary development platform, this bug is unlikely\
  to be fixed in the near future." no-italic >
<P "Normal" lineSpacing:0 >
<T "Listing INDEX">
<P "heading 2" lineSpacing:0 >
<T "As a further guide into the source code, the following lists provide inform\
 ation about the global variables and procedures.">
<P "Normal" lineSpacing:0 >
<T "global Variable Index">
<P "heading 3" lineSpacing:0 >
<T "Table ">
<S_F fieldType:10>
<T "xref \"tab:globals\"">
<FV>
<T "3">
<E_F>
<T " shows the global variables used in ">
<T "Studio" italic >
<T " and their purpose.">
<P "Normal" lineSpacing:0 >
<RS>
<T "Global Variable">
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:21:0:"" bB:10:0:"" >
<T "Type">
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:21:0:"" bB:10:0:"" >
<T "Description">
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:21:0:"" bB:10:0:"" >
<RS>
<T "CARD(Bits)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Bit resolutions available from sound card" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "CARD(Chnnls)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Channels (mono, stereo) available from sound card." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "CARD(Driver)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VoxWare driver version." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "CARD(HW" size:10 >
<T "n" bold italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Name of sound card hardware detected. " size:10 >
<T "n" bold italic size:10 >
<T "=0,1,2,...." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "CARD(Maxrates)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Maximum sampling rates of sound card, for each channel setting." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "CARD(Minrates)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Maximum sampling rates of sound card, for each channel setting." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(BDW)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: Borderwidth" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(FNT_L)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: LargeFont" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(FNT_S)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: SmallFont" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(REL)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: Relief for Sections." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(cABG)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: ActiveBackGround" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(cAFG)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: ActiveForeGround" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(cBG1)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: BackGround1" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(cBG2)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: BackGround2" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(cBG3)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: BackGround3" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(cDFG)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: DisabledForeGround" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(cFG1)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: ForeGround1" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(cFG2)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: ForeGround2" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(cFG3)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: ForeGround3" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(fVw_" size:10 >
<T "Section_WCR" bold italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: View Flag for a section. " size:10 >
<T "Section_WCR" bold italic size:10 >
<T "=EffectBar,MenuBar,PlayPanel,Mixer, InfoBar, PlotBox" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigInfo(mnoshow)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Description of configuration option: List of mixer devices not to be shown.\
  [see MIXER(noshow)]" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(BDW)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: Borderwidth" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(FNT_L)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: LargeFont" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(FNT_S)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: SmallFont" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(REL)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: Relief for Sections." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(cABG)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: ActiveBackGround" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(cAFG)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: ActiveForeGround" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(cBG1)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: BackGround1" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(cBG2)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: BackGround2" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(cBG3)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: BackGround3" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(cDFG)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: DisabledForeGround" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(cFG1)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: ForeGround1" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(cFG2)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: ForeGround2" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ConfigSet(cFG3)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Value of configuration option: ForeGround3" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "DialogCfg " size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Configuration list for " size:10 >
<T "dialog" italic size:10 >
<T " and " size:10 >
<T "filebrowser" italic size:10 >
<T " calls." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(CFG)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename for the configuration file." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(CURR)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename of the current file." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(Fader)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename of the " size:10 >
<T "fader" face:"Courier" size:10 >
<T " binary" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(Mixer)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename of the mixer interface (" size:10 >
<T "studio_mixer" italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(PID)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The unique part of temporary file names. (" size:10 >
<T "System clock" italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(SRC)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path of the modules of " size:10 >
<T "studio" italic size:10 >
<T "." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(Sox)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename of " size:10 >
<T "Sox" italic size:10 >
<T "." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(TMP1)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename of temporary file 1" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(TMP2)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename of temporary file 2" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(TMP3)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename of temporary file 3" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(TMP4)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename of temporary file 4" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(TMP5)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename of temporary file 5" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FILE(Tool)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename of the sound card interface. (" size:10 >
<T "studio_tool" italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FLAG(Card_Exist)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating a sound card exists." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FLAG(Clip)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating the clipboard has contents." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FLAG(Converted)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating the sample has been converted for sound card compatibility.\
 " size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FLAG(Edited)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating the working file has been edited." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FLAG(Loaded)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating a working file exists." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FLAG(Marked)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating a marked area exists in the editing plot." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FLAG(Mixer_Exist)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating a sound card mixer is available." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FLAG(Playing)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating playback/recording in progress." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FLAG(Recording)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating the application is recording." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FLAG(Vw_" size:10 >
<T "Section_WCR" bold italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating the  " size:10 >
<T "section" bold italic size:10 >
<T " is visible. " size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<T "Section_WCR" bold italic size:10 >
<T "=EffectBar,MenuBar,PlayPanel,Mixer, InfoBar, PlotBox." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(ECHDelay)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Slider value of the echo parameter: Volume." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(ECHVolume)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Slider value of the echo parameter: Volume." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(FLTCentre)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Slider value of the filter parameter: Centre Frequency." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(FLTtype)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Slider value of the filter parameter: Filter Type" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(Preview)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating preview mode." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(VIBDepth)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Slider value of the vibrato parameter: depth." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(VIBSpeed)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Slider value of the vibrato parameter: speed." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(" size:10 >
<T "effect_name" bold italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating that " size:10 >
<T "effect" bold italic size:10 >
<T " is to be applied. " size:10 >
<T "effect_name" bold italic size:10 >
<T "=echo, filter, reverse, tempo, vibrato, volume." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(fadein)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "fade-in time for the fade effect in centiseconds" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(fadeout)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Fade-out time in centiseconds" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(list)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "List of available effects." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(tempoVal)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Slider value for the tempo effect." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FX(volumeVal)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Slider value for the volume effect.." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "HELP(AFG)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "ActiveForeGround colour for the Help window." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "HELP(BG)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BackGround colour for the Help window." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "HELP(COM)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "ForeGround colour of contents text." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "HELP(FG)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "ForeGround colour for the Help window." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "HELP(HIL)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "ForeGround colour of highlighted text." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "HELP(Log)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VLIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Stores the position in the Help widget that have been viewed via. Contents.\
 " size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "HELP(UserGuide)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path+Filename of the user guide text." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "INFO(Length)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "STR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "InfoBar string: Length of current sample." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "INFO(Rate)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "STR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "InfoBar string: Sample Rate of current sample." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "INFO(Sample)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "STR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "InfoBar string: File name of current sample." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "INFO(Stereo)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "STR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "InfoBar string: Channels in current sample." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "INFO(Type)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "STR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "InfoBar string: Bit resolution of current sample." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LINE(Id)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Play-line handle for use in graphics operations." size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LINE(On)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Whether the play-line is visible." size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LINE(PixelsMovedInPlot)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Number of pixels traversed by the playline." size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LINE(Startx)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Initial position of play-line at commencement of " size:10 >
<T "studio_tool" italic size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LINE(TotalPoxelsMoved)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Pixels moved by play-line" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LOAD(bit)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: Bit Resolution." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LOAD(chnl)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: Channels." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LOAD(format)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: Format name." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LOAD(maxvol)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: " size:10 >
<T "Sox " italic size:10 >
<T "maximum volume index." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LOAD(rate)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: Sampling Rate" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LOAD(size)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: a letter indicating the size of each sample, e.g. b=byt\
 e (see " size:10 >
<T "Sox " italic size:10 >
<T "documentation)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LOAD(style)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: a letter indicating the data style of each sample, e.g.\
  u=unsigned (see " size:10 >
<T "Sox " italic size:10 >
<T "documentation)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MACHINE(PLATFORM)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The type of host operating system under which " size:10 >
<T "studio" italic size:10 >
<T " is running.  Currenly valid strings are " no-italic size:10 >
<T "Linux" no-italic face:"Courier" size:10 >
<T " and " no-italic face:"Arial" size:10 >
<T "SunOS" no-italic face:"Courier" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MIXER(All)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Index numbers of all available Mixer devices." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MIXER(I_chnnls)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "List of input channels to be shown in Mixer board." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MIXER(O_chnnls)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "List of input channels to be shown in Mixer board." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MIXER(flag" size:10 >
<T "DevName" bold italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The flag indicating a recording source is active. " size:10 >
<T "DevName" bold italic size:10 >
<T "=an element of MIXER(labels)." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MIXER(label" size:10 >
<T "DevName" bold italic size:10 >
<T "n " italic underline size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Slider value of a mixer device channel. " size:10 >
<T "DevName" bold italic size:10 >
<T "=an element of MIXER(labels). " size:10 >
<T "n" italic underline size:10 >
<T "=0,1." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MIXER(labels)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "List of mixer device names, returned from mixer interface." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MIXER(noshow)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "List of mixer devices not to be shown." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MIXER(show" size:10 >
<T "DevName" bold italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Flag indicating a device has a slider in the Mixer Board. " size:10 >
<T "DevName" bold italic size:10 >
<T "=an element of MIXER(labels)." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PACK(" size:10 >
<T "Section_WCR" bold italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Path of the section after which " size:10 >
<T "Section" bold italic size:10 >
<T " should be packed. " size:10 >
<T "Section_WCR" bold italic size:10 >
<T "=EffectBar,MenuBar,PlayPanel,Mixer, InfoBar, PlotBox" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PACK(list)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "List of section_WCRs, for generating PACK(" size:10 >
<T "Section_WCR" bold italic size:10 >
<T ") globals." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PACK(path)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "List of section paths, for generating PACK(" size:10 >
<T "Section_WCR" bold italic size:10 >
<T ") globals." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PID" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VLIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The process id's of background process pipes." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLAY(ButtList)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "List of PlayPanel button names." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLAY(Time)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The value of the " size:10 >
<T "Play Time" italic size:10 >
<T " field, in the control panel." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLAY(TimeMark)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The value of the system clock at the time the PP_Playing routine was called\
  during playback, in order to change PLAY(Time) every second." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(Blocksize)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The number of bytes represented by a single vertical line of the editing pl\
 ot." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(End)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "STR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The position of insertion point 2: in a time format for display. " size:10 \
 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(Endx)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The position of insertion point 2: it's x-coord in the canvas." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(Height)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Height of plot canvas." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(Magnification)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Stores the current magnification factor of the plot canvas.  Currently rest\
 ricted to 1, 2, 4 or 8." size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(MarkBox)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Handle to the marking rectangle in the plot canvas." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(MaxBlockSize)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CONST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The block size which fits the currents sample entirely within the canvas bo\
 undaries." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(Offset)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Number of bytes in sample preceding the point at the x=0 point in the plot \
 canvas." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(Start)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "STR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The position of insertion point 1: in a time format for display. " size:10 \
 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(Startx)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The position of insertion point 2: it's x-coord in the canvas." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(Total)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The actual number of vertical lines drawn in the plot canvas." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(Width)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Width of plot canvas.  May be greater the the visible width if the magnific\
 ation feature is in use." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(ZoomCount)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Number of Zoom levels" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PLOT(Zoom" size:10 >
<T "n" bold italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VLIST" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Context of the last zoom level." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<T "n" bold italic size:10 >
<T "=0,1,2,3..." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SAMPLE(Size)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Number of samples in the sample file." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SAMPLE(bit)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: Bit Resolution." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SAMPLE(chnl)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: Channels." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SAMPLE(format)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: Format name." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SAMPLE(rate)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: Sampling Rate" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SAMPLE(size)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Sampling parameter: a letter indicating the size of each sample, e.g. b=byt\
 e (see " size:10 >
<T "Sox " italic size:10 >
<T "documentation)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SAMPLE(style)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:21:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:21:0:"" >
<T "Sampling parameter: a letter indicating the data style of each sample, e.g.\
  u=unsigned (see " size:10 >
<T "Sox " italic size:10 >
<T "documentation)" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:21:0:"" >
<RS>
<T "VU(On)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Whether the peak meter is enabled" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "VU(Peak" size:10 >
<T "channel" bold italic size:10 >
<T ")" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Current peak value for channel " size:10 >
<T "channel" bold italic size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "VU(PeakOn)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BOOL" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Whether the peak hold feature is visible" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "VU(channels)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "The current number of channels being displayed by the VU meter (if VU(On))"\
  size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "VU(xpos)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "X position of the VU meters' window" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "VU(ypos)" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "Y position of the VU meters' window" size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "button" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:2208 lm:39 rm:39  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "VAR" size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:592 lm:39 rm:39  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "A global of the " size:10 >
<T "dialogue" italic size:10 >
<T " subroutine." size:10 >
<P "Normal" justifyLeft lineSpacing:0 >
<CE width:3092 lm:39 rm:39 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<T "table " italic >
<S_F fieldType:11 editable
<start_eval_value>
<T "--field--">
<end_eval_value>
>
<T "xref_source \"tab:globals\" -live">
<FV>
<S_F fieldType:3>
<T "series ptable">
<FV>
<T "3">
<E_F>
<E_F>
<T " - Index of Global Variables" italic >
<P "Normal" justifyCenter lineSpacing:0 >
<T "Variable type abbreviations: CONST=Constant. VAR=Variable. STR=Formatted st\
 ring for display. LIST=a TCL list; VLIST=a dynamic list; BOOL=a flag" size:10 \
 >
<T "." italic size:10 >
<P "Normal" justifyLeft lineSpacing:0  size:10 >
<T "Procedure Index">
<P "heading 3" lineSpacing:0 >
<T "The index in table ">
<S_F fieldType:10>
<T "xref \"tab:procs\"">
<FV>
<T "4">
<E_F>
<T " is to be used as a guide to finding procedures within the source code. The\
  category is provided to give an indication of its position in the procedure h\
 ierarchy.">
<P "Normal" lineSpacing:0 >
<P "Normal" lineSpacing:0 >
<T "Category abbreviations:" size:10 >
<P "Normal" lineSpacing:0  xpos:5256 yposParaRelative ypos:31 width:2227 leftFr\
 ameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:0 >
<RS>
<T "WCR" size:8 >
<P "Normal" lineSpacing:0  size:8  xpos:5256 yposParaRelative ypos:31 width:222\
 7 leftFrameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:\
 0 >
<CE width:707 lm:100 rm:100 >
<T "Widget Creation Routines" size:8 >
<P "Normal" lineSpacing:0  size:8  xpos:5256 yposParaRelative ypos:31 width:222\
 7 leftFrameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:\
 0 >
<CE width:1507 lm:100 rm:100 last >
<RS>
<T "CB" size:8 >
<P "Normal" lineSpacing:0  size:8  xpos:5256 yposParaRelative ypos:31 width:222\
 7 leftFrameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:\
 0 >
<CE width:707 lm:100 rm:100 >
<T "Callback" size:8 >
<P "Normal" lineSpacing:0  size:8  xpos:5256 yposParaRelative ypos:31 width:222\
 7 leftFrameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:\
 0 >
<CE width:1507 lm:100 rm:100 last >
<RS>
<T "CB/SUB" size:8 >
<P "Normal" lineSpacing:0  size:8  xpos:5256 yposParaRelative ypos:31 width:222\
 7 leftFrameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:\
 0 >
<CE width:707 lm:100 rm:100 >
<T "dual purpose routines" size:8 >
<P "Normal" lineSpacing:0  size:8  xpos:5256 yposParaRelative ypos:31 width:222\
 7 leftFrameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:\
 0 >
<CE width:1507 lm:100 rm:100 last >
<RS>
<T "SUB" size:8 >
<P "Normal" lineSpacing:0  size:8  xpos:5256 yposParaRelative ypos:31 width:222\
 7 leftFrameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:\
 0 >
<CE width:707 lm:100 rm:100 >
<T "Subroutine" size:8 >
<P "Normal" lineSpacing:0  size:8  xpos:5256 yposParaRelative ypos:31 width:222\
 7 leftFrameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:\
 0 >
<CE width:1507 lm:100 rm:100 last >
<RS>
<T "LOC" size:8 >
<P "Normal" lineSpacing:0  size:8  xpos:5256 yposParaRelative ypos:31 width:222\
 7 leftFrameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:\
 0 >
<CE width:707 lm:100 rm:100 >
<T "Local Subroutine" size:8 >
<P "Normal" lineSpacing:0  size:8  xpos:5256 yposParaRelative ypos:31 width:222\
 7 leftFrameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:\
 0 >
<CE width:1507 lm:100 rm:100 last >
<P "Normal" lineSpacing:0  xpos:5256 yposParaRelative ypos:31 width:2227 leftFr\
 ameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:0 >
<T "Entries applicatble only to non-Linux implementations are italicised." size\
 :8 >
<P "Normal" lineSpacing:0  xpos:5256 yposParaRelative ypos:31 width:2227 leftFr\
 ameMargin:150 rightFrameMargin:150 topFrameMargin:0 bottomFrameMargin:0 >
<RS>
<T "Procedure name" bold >
<P "Normal" lineSpacing:0  bold >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:21:0:"" bB:10:0:"" >
<T "Category" bold >
<P "Normal" lineSpacing:0  bold >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:21:0:"" bB:10:0:"" >
<T "Module file name" bold >
<P "Normal" lineSpacing:0  bold >
<CE width:960 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:21:0:"" bB:10:0:"" >
<RS>
<T "About" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:21:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:21:0:"" bB:10:0:"" >
<T "help.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:21:0:"" bB:10:0:"" >
<RS>
<T "AddFX" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effect.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "AddSilence" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Bit2SoxSize" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC: Load" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "file.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "CARDSetter" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "CheckGlob" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:filebrowser" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ColourList" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Colours" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Convert" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "file.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Copy" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB/SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "CreateMenu" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "menu.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "CreateWidgets" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "menu.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Cut" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Delete" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB/SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "DisplayWarnings" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "WCR" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effects.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "EditMenuStates" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "TCB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "EffectBar" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "WCR" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effect.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "EndCallBack" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:Select" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Exit" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "file.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FB_Cancel" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:FileBrowser/2" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FB_Exit" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:FileBrowser/2" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FB_RefreshList" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:FileBrowser/2" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FB_Select" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:FileBrowser/2" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FXApply" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effect.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FileBrowser" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FileBrowser2" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FileMenuStates" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "TCB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "file.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "FillPlot" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "plot.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "GetArrayName" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "GetSoxVersion" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "GetWordSize" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "HoldWindow" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Info" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "file.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Info2Set" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "info.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "InfoBar" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "WCR" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "info.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "InfoFill" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "info.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Insert" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "light" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:Reader" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "lineVU.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "ListBoxFill" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Load" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB/SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "file.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "LoadSettings" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MIX_DeviceGet" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "mixer.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MIX_Reset" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "mixer.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MarkPlot" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "plot.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MatchList" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:Setter" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MenuBar" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "WCR" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "menu.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MenuGetUndl" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "menu.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MenuItemDisable" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "menu.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Mixer" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "WCR" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "mixer.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MixerSliders" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "mixer.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "MkSlider" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "mixer.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "New" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB/SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "file.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "OrderPacking" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PP_ButtonAble" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "playpanel.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PP_Recording" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "playpanel.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Paste" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Peak_Reset" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "BC" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "lineVU.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Play" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "playpanel.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PlayPanel" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "WCR" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "playpanel.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PlayPort" italic size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" italic size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUNmixer.tk" italic size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PlotBox" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "WCR" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "plot.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Preview" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "PutPeak" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "lineVU.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Reader" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "lineVU.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "RecSrcSet" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "mixer.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Record" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "playpanel.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "RecPort" italic size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUNmixer.tk" italic size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Resetter" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SETscaleLim" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:Settings" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "sChorus" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "neweffects.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "sEcho15" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "neweffects.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "sFlanger" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "neweffects.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "sPhaser" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "neweffects.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "sReverb" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "neweffects.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SetVUParamters" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "lineVU.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SetupVU_Channels" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "WCR" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "lineVU.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Save" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "file.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SaveSettings" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Saver" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "file.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Sec2Time" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Select" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SelectApply" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:Select" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SetInitially" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:SaveSettings" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Setter" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Settings" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Show_VY" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "StartCallBack" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:Select" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Stop" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "playpanel.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SoxWarnings" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effects.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SUN_GetMixInfo" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUNmixer.tk" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SUN_Mixer" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUNmixer.tk" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SUN_SetSliders" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SYBmixer.tk" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SUNPlayBalanceSetter" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUNmixer.tk" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SUNRecBalanceSetter" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUNmixer.tk" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SUNRecSetter" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUNmixer.tk" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "SUNSetter" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUNmixer.tk" italic size:10 >
<P "Normal" lineSpacing:0  italic size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Trim" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB/SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Trim_Stop" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "playpanel.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Undo" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "UserManual" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "help.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "VolSet" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "mixer.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "WaitPlease" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "Zoom" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:21:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:21:0:"" >
<T "edit.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:21:0:"" >
<RS>
<T "bindScale" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "dialog" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "misc.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "expandIt" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:CreateMenu" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "menu.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "forAllMatches" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "help.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "formatText" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "help.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "loadFile" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "help.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "parse_sox" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "file.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "sEcho" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effect.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "sFade" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effect.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "sFilter" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effect.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "sReverse" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effect.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "sVibrato" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effect.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "sVolTemp" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effect.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "scrollcb" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:plotbox" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "plot.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "setPlotMag" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB/SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "plot.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "setRanges" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "LOC:sFilter" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "effect.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "setupText" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "SUB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "help.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "vEffects" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "vInfo" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "vMixer" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<RS>
<T "vPlot" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1459 lm:75 rm:75  lB:21:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "CB" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:1409 lm:75 rm:75  lB:10:0:"" rB:10:0:"" tB:10:0:"" bB:10:0:"" >
<T "option.tk" size:10 >
<P "Normal" lineSpacing:0  size:10 >
<CE width:967 lm:75 rm:75 last  lB:10:0:"" rB:21:0:"" tB:10:0:"" bB:10:0:"" >
<T "table " italic >
<S_F fieldType:11 editable
<start_eval_value>
<T "--field--">
<end_eval_value>
>
<T "xref_source \"tab:procs\" -live">
<FV>
<S_F fieldType:3>
<T "series ptable">
<FV>
<T "4">
<E_F>
<E_F>
<T " - List of studio procedures" italic >
<P "Normal" justifyLeft lineSpacing:0 >
<section  next_column pageWidth:8267 pageHeight:11692 leftMargin:1250 rightMarg\
 in:1250 topMargin:1000 bottomMargin:1000 bindingMargin:0 paperSize:11 oddHeade\
 r:Normal "_AX_HF_3" oddFooter:None sectNumFmt:-1 sectPageSep:"-" pageNumFmt:0 \
 pageNumCtl:0>
<end_flow>
<start_footnote "2">
<S_F fieldType:5>
<T "footnote_body">
<FV>
<T "3" position:3 >
<E_F>
<T " The VoxWare manual states (p.12): \"The mixer ioctl calls are accessible t\
 hrough every device of the VoxWare driver.\"">
<P "footnote text" lineSpacing:0 >
<end_footnote>
<start_footnote "1">
<S_F fieldType:5>
<T "footnote_body">
<FV>
<T "2" position:3 >
<E_F>
<T " See the ">
<T "colour" italic >
<T " dialog box in the SnapShots section.">
<P "footnote text" lineSpacing:0 >
<end_footnote>
<start_hdrftr "_AX_HF_3">
<T "Programmer's Guide^aj^aj3-">
<S_F fieldType:19>
<T "page_number ">
<FV>
<T "19">
<E_F>
<P "header" lineSpacing:0  bB:10:0:"" verticalMargin:14 >
<end_hdrftr>
<object "object9" docType:201 appType:2>
<start_data Applix>
*BEGIN GRAPHICS VERSION=430/420 ENCODING=7BIT

COLORMAP
<"Transparent" 0 0 0 0 0 1>
<"Black" 0 0 0 0 255 0>
<"Dark gray" 0 0 0 0 127 0>
<"Dark blue" 0 128 128 0 127 0>
<"Blue" 0 255 255 0 0 0>
<"Dark cyan" 0 128 0 0 127 0>
<"Cyan" 0 255 0 0 0 0>
<"Dark green" 0 128 0 128 127 0>
<"Green" 0 255 0 255 0 0>
<"Dark magenta" 0 0 128 0 127 0>
<"Magenta" 0 0 255 0 0 0>
<"Dark red" 0 0 128 128 127 0>
<"Red" 0 0 255 255 0 0>
<"Dark yellow" 0 0 0 128 127 0>
<"Yellow" 0 0 0 255 0 0>
<"Light gray" 0 0 0 0 63 0>
<"White" 0 0 0 0 0 0>
END COLORMAP

FONT
"Arial"
END FONT

SESSION
 WIN_SIZE <7500 5500>
 VIEW <0 0 0 0 0 0>
 BACKFILL <1 0 0 0 0 0 0>
 LINEFILL <1 2 5 1000 0 0 0>
 SHADOW <6 0 15 15>
 SIZE 180000 VER_JUST TOP
 REZ <0 1000 1000 0 0>
END SESSION

PICTURE
.ELL AT (1923,256)
 BACKFILL <16 16 5 1000 0 0 0>
 LINEFILL <1 16 5 1000 0 0 0>
 SHADOW <1 0 15 15>
 THICKNESS 14
 PNTS (0,0)(2016,0)(2016,2016)(0,2016)(0,0)
.TXT AT (2352,496)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 100000 VER_JUST BASELINE MARGINS <0 0 0 0>
 TXTXYOFF <-5 101>
 T_PNTS (0,101)(0,0)(979,0)(979,131)(0,131)(0,101)(979,101)
 .STR
  "case first_char {"
.TXT AT (2523,665)
 TXTXYOFF <-9 99>
 T_PNTS (0,99)(0,0)(664,0)(664,100)(0,100)(0,99)(664,99)
 .STR
  "m : MIXER"
.TXT AT (2514,830)
 TXTXYOFF <0 101>
 T_PNTS (0,101)(0,0)(589,0)(589,104)(0,104)(0,101)(589,101)
 .STR
  " f  : FLAG"
.TXT AT (2519,996)
 TXTXYOFF <-5 101>
 T_PNTS (0,101)(0,0)(999,0)(999,131)(0,131)(0,101)(999,101)
 .STR
  "other: ConfigSet"
.TXT AT (2350,1163)
 TXTXYOFF <-3 101>
 T_PNTS (0,101)(0,0)(41,0)(41,131)(0,131)(0,101)(41,101)
 .STR
  "}"
.RECT AT (7,7)
 BACKFILL <16 16 5 1000 0 0 0>
 MARGINS <125 125 250 62>
 XYRAD <146 146>
 PNTS (0,0)(1097,0)(1097,597)(0,597)(0,0)
.RECT AT (7,840)
 PNTS (0,0)(1097,0)(1097,598)(0,598)(0,0)
.RECT AT (7,1757)
 PNTS (0,0)(1097,0)(1097,597)(0,597)(0,0)
.RECT AT (4674,174)
 XYRAD <0 0>
 PNTS (0,0)(1679,0)(1679,1930)(0,1930)(0,0)
.TXT AT (4749,248)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-9 99>
 T_PNTS (0,99)(0,0)(322,0)(322,102)(0,102)(0,99)(322,99)
 .STR
  "index"
.POL AT (5181,181)
 BACKFILL <16 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(0,1916)
.POL AT (5597,181)
 PNTS (0,0)(0,1916)
.POL AT (4681,422)
 PNTS (0,0)(1666,9)
.TXT AT (4681,498)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <0 99>
 T_PNTS (0,99)(0,0)(486,0)(486,102)(0,102)(0,99)(486,99)
 .STR
  " mindex"
.TXT AT (4681,663)
 TXTXYOFF <0 101>
 T_PNTS (0,101)(0,0)(409,0)(409,104)(0,104)(0,101)(409,101)
 .STR
  " findex"
.TXT AT (4945,1109)
 BOLD ON
 TXTXYOFF <-14 72>
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (4945,1359)
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (4945,1609)
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (4945,1859)
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (5268,496)
 BOLD OFF
 TXTXYOFF <-4 101>
 T_PNTS (0,101)(0,0)(203,0)(203,131)(0,131)(0,101)(203,101)
 .STR
  "{...}"
.TXT AT (5268,663)
 T_PNTS (0,101)(0,0)(203,0)(203,131)(0,131)(0,101)(203,101)
 .STR
  "{...}"
.TXT AT (5685,496)
 T_PNTS (0,101)(0,0)(554,0)(554,131)(0,131)(0,101)(554,101)
 .STR
  "{............}"
.TXT AT (5685,663)
 T_PNTS (0,101)(0,0)(554,0)(554,131)(0,131)(0,101)(554,101)
 .STR
  "{............}"
.TXT AT (5361,1109)
 BOLD ON
 TXTXYOFF <-14 72>
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (5361,1359)
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (5361,1609)
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (5361,1859)
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (5861,1109)
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (5861,1359)
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (5861,1609)
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (5861,1859)
 T_PNTS (0,72)(0,0)(20,0)(20,73)(0,73)(0,72)(20,72)
 .STR
  ":"
.TXT AT (4681,832)
 BOLD OFF
 TXTXYOFF <0 99>
 T_PNTS (0,99)(0,0)(370,0)(370,102)(0,102)(0,99)(370,99)
 .STR
  " index"
.TXT AT (5268,830)
 TXTXYOFF <-4 101>
 T_PNTS (0,101)(0,0)(203,0)(203,131)(0,131)(0,101)(203,101)
 .STR
  "{...}"
.TXT AT (5685,830)
 T_PNTS (0,101)(0,0)(554,0)(554,131)(0,131)(0,101)(554,101)
 .STR
  "{............}"
.POL AT (3931,1097)
 BACKFILL <16 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (666,0)(0,0)
.POL AT (1097,347)
 PNTS (0,0)(941,375)
.POL AT (1097,1181)
 PNTS (834,0)(0,0)
.POL AT (1090,1589)
 PNTS (0,442)(889,0)
.POL AT (4597,1014)
 BACKFILL <1 16 5 1000 0 0 0>
 THICKNESS 1
 PNTS (84,83)(0,167)(0,0)(84,83)
.TXT AT (107,80)
 MARGINS <0 0 0 0>
 TXTXYOFF <-10 101>
 T_PNTS (0,101)(0,0)(850,0)(850,131)(0,131)(0,101)(850,101)
 .STR
  "MIXER(index)"
.TXT AT (108,913)
 TXTXYOFF <-11 101>
 T_PNTS (0,101)(0,0)(771,0)(771,131)(0,131)(0,101)(771,101)
 .STR
  "FLAG(index)"
.TXT AT (21,1830)
 TXTXYOFF <-7 101>
 T_PNTS (0,101)(0,0)(1022,0)(1022,131)(0,131)(0,101)(1022,101)
 .STR
  "ConfigSet(index)"
.POL AT (1097,1097)
 MARGINS <125 125 250 62>
 PNTS (0,84)(84,167)(84,0)(0,84)
.POL AT (1847,1097)
 PNTS (84,84)(0,167)(0,0)(84,84)
.POL AT (3931,1014)
 PNTS (0,83)(83,0)(83,167)(0,83)
.POL AT (1955,665)
 PNTS (142,94)(0,95)(68,0)(142,94)
.POL AT (1872,1542)
 PNTS (128,29)(66,154)(0,0)(128,29)
.POL AT (1089,344)
 PNTS (0,0)(66,109)(125,5)(0,0)
.POL AT (1101,1877)
 PNTS (0,146)(109,166)(54,0)(0,146)
.TXT AT (5217,255)
 MARGINS <0 0 0 0>
 TXTXYOFF <-5 99>
 T_PNTS (0,99)(0,0)(260,0)(260,102)(0,102)(0,99)(260,99)
 .STR
  "data"
.TXT AT (5644,257)
 T_PNTS (0,99)(0,0)(657,0)(657,128)(0,128)(0,99)(657,99)
 .STR
  "description"
.TXT AT (4636,2528)
 TXTXYOFF <-7 101>
 T_PNTS (0,101)(0,0)(1077,0)(1077,131)(0,131)(0,101)(1077,101)
 .STR
  "Configuration File"
.TXT AT (2139,2561)
 TXTXYOFF <-10 101>
 T_PNTS (0,101)(0,0)(1620,0)(1620,131)(0,131)(0,101)(1620,101)
 .STR
  "Loading/Saving Procedure"
.TXT AT (104,2547)
 TXTXYOFF <0 99>
 T_PNTS (0,99)(0,0)(396,0)(396,129)(0,129)(0,99)(396,99)
 .STR
  "Arrays"
.TXT AT (2038,1330)
 TXTXYOFF <0 101>
 T_PNTS (0,101)(0,0)(1812,0)(1812,131)(0,131)(0,101)(1812,101)
 .STR
  " If Save: Selected array to file"
.TXT AT (2110,1496)
 TXTXYOFF <-13 101>
 T_PNTS (0,101)(0,0)(1632,0)(1632,131)(0,131)(0,101)(1632,101)
 .STR
  "If Load: Store data in array"
.TXT AT (439,277)
 TXTXYOFF <-8 70>
 T_PNTS (0,43)(0,0)(66,0)(66,43)(0,43)(0,43)(66,43)
 .STR
  "="
.TXT AT (439,1111)
 T_PNTS (0,43)(0,0)(66,0)(66,43)(0,43)(0,43)(66,43)
 .STR
  "="
.TXT AT (439,2027)
 T_PNTS (0,43)(0,0)(66,0)(66,43)(0,43)(0,43)(66,43)
 .STR
  "="
.TXT AT (300,422)
 TXTXYOFF <-5 99>
 T_PNTS (0,99)(0,0)(260,0)(260,102)(0,102)(0,99)(260,99)
 .STR
  "data"
.TXT AT (300,1255)
 T_PNTS (0,99)(0,0)(260,0)(260,102)(0,102)(0,99)(260,99)
 .STR
  "data"
.TXT AT (300,2172)
 T_PNTS (0,99)(0,0)(260,0)(260,102)(0,102)(0,99)(260,99)
 .STR
  "data"
END PICTURE

*END GRAPHICS
<end_data>
<object "object8" docType:201 appType:2>
<start_data Applix>
*BEGIN GRAPHICS VERSION=430/420 ENCODING=7BIT

COLORMAP
<"Transparent" 0 0 0 0 0 1>
<"Black" 0 0 0 0 255 0>
<"Dark gray" 0 0 0 0 127 0>
<"Dark blue" 0 128 128 0 127 0>
<"Blue" 0 255 255 0 0 0>
<"Dark cyan" 0 128 0 0 127 0>
<"Cyan" 0 255 0 0 0 0>
<"Dark green" 0 128 0 128 127 0>
<"Green" 0 255 0 255 0 0>
<"Dark magenta" 0 0 128 0 127 0>
<"Magenta" 0 0 255 0 0 0>
<"Dark red" 0 0 128 128 127 0>
<"Red" 0 0 255 255 0 0>
<"Dark yellow" 0 0 0 128 127 0>
<"Yellow" 0 0 0 255 0 0>
<"Light gray" 0 0 0 0 63 0>
<"White" 0 0 0 0 0 0>
END COLORMAP

FONT
"Arial"
END FONT

SESSION
 WIN_SIZE <7500 5500>
 VIEW <0 0 0 0 0 0>
 BACKFILL <1 0 0 0 0 0 0>
 LINEFILL <1 2 5 1000 0 0 0>
 SHADOW <6 0 15 15>
 SIZE 180000 VER_JUST TOP
 REZ <0 1000 1000 0 0>
END SESSION

PICTURE
.RECT AT (590,82)
 BACKFILL <16 16 5 1000 0 0 0>
 LINEFILL <1 16 5 1000 0 0 0>
 SHADOW <1 0 15 15>
 THICKNESS 14
 XYRAD <146 146>
 PNTS (0,0)(1181,0)(1181,597)(0,597)(0,0)
.RECT AT (590,998)
 PNTS (0,0)(1181,0)(1181,597)(0,597)(0,0)
.RECT AT (1674,1998)
 PNTS (0,0)(1180,0)(1180,597)(0,597)(0,0)
.RECT AT (7,1998)
 PNTS (0,0)(1180,0)(1180,597)(0,597)(0,0)
.POL AT (1181,672)
 BACKFILL <16 16 0 0 0 0 0>
 PNTS (0,0)(0,333)
.POL AT (764,1589)
 PNTS (417,0)(0,416)
.RECT AT (2840,82)
 BACKFILL <16 16 5 1000 0 0 0>
 XYRAD <-146 146>
 PNTS (0,0)(1181,0)(1181,597)(0,597)(0,0)
.RECT AT (2840,998)
 PNTS (0,0)(1181,0)(1181,597)(0,597)(0,0)
.RECT AT (3090,2498)
 PNTS (0,0)(1181,0)(1181,597)(0,597)(0,0)
.POL AT (3430,672)
 BACKFILL <16 16 0 0 0 0 0>
 PNTS (0,0)(0,333)
.POL AT (1764,1255)
 PNTS (0,0)(1083,0)
.POL AT (1764,1422)
 PNTS (0,0)(142,0)
.POL AT (1906,1422)
 PNTS (0,0)(106,583)
.POL AT (2703,1422)
 PNTS (142,0)(0,0)
.POL AT (2597,1422)
 PNTS (106,0)(0,583)
.POL AT (1118,889)
 BACKFILL <1 16 5 1000 0 0 0>
 THICKNESS 1
 PNTS (63,116)(116,0)(0,0)(63,116)
.POL AT (1931,1889)
 PNTS (83,116)(137,0)(0,21)(83,116)
.POL AT (766,1915)
 PNTS (0,92)(138,7)(12,0)(0,92)
.POL AT (2727,1189)
 PNTS (117,63)(0,117)(0,0)(117,63)
.POL AT (2564,1880)
 PNTS (37,125)(117,12)(0,0)(37,125)
.POL AT (3373,898)
 PNTS (63,116)(117,0)(0,0)(63,116)
.TXT AT (882,148)
 SIZE 100000 VER_JUST BASELINE MARGINS <0 0 0 0>
 TXTXYOFF <-2 99>
 T_PNTS (0,99)(0,0)(430,0)(430,129)(0,129)(0,99)(430,99)
 .STR
  "Widget"
.TXT AT (837,295)
 TXTXYOFF <-7 101>
 T_PNTS (0,101)(0,0)(509,0)(509,104)(0,104)(0,101)(509,101)
 .STR
  "Creation"
.TXT AT (850,455)
 TXTXYOFF <-11 99>
 T_PNTS (0,99)(0,0)(463,0)(463,102)(0,102)(0,99)(463,99)
 .STR
  "Routine"
.TXT AT (3189,156)
 TXTXYOFF <-2 99>
 T_PNTS (0,99)(0,0)(430,0)(430,129)(0,129)(0,99)(430,99)
 .STR
  "Widget"
.TXT AT (3144,304)
 TXTXYOFF <-7 101>
 T_PNTS (0,101)(0,0)(509,0)(509,104)(0,104)(0,101)(509,101)
 .STR
  "Creation"
.TXT AT (3157,463)
 TXTXYOFF <-11 99>
 T_PNTS (0,99)(0,0)(463,0)(463,102)(0,102)(0,99)(463,99)
 .STR
  "Routine"
.TXT AT (854,1211)
 TXTXYOFF <-7 101>
 T_PNTS (0,101)(0,0)(525,0)(525,104)(0,104)(0,101)(525,101)
 .STR
  "Callback"
.TXT AT (2929,1295)
 T_PNTS (0,101)(0,0)(653,0)(653,104)(0,104)(0,101)(653,101)
 .STR
  "Callback &"
.TXT AT (3016,1080)
 TXTXYOFF <-11 99>
 T_PNTS (0,99)(0,0)(825,0)(825,128)(0,128)(0,99)(825,99)
 .STR
  "Dual Purpose"
.TXT AT (1871,2170)
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(661,0)(661,104)(0,104)(0,101)(661,101)
 .STR
  "Subroutine"
.TXT AT (1829,2361)
 TXTXYOFF <-8 101>
 T_PNTS (0,101)(0,0)(844,0)(844,131)(0,131)(0,101)(844,101)
 .STR
  "(Multiple Use)"
.TXT AT (3213,1428)
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(661,0)(661,104)(0,104)(0,101)(661,101)
 .STR
  "Subroutine"
.TXT AT (355,2088)
 TXTXYOFF <-10 99>
 T_PNTS (0,99)(0,0)(312,0)(312,102)(0,102)(0,99)(312,99)
 .STR
  "Local"
.TXT AT (246,2229)
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(661,0)(661,104)(0,104)(0,101)(661,101)
 .STR
  "Subroutine"
.TXT AT (213,2420)
 TXTXYOFF <-8 101>
 T_PNTS (0,101)(0,0)(751,0)(751,131)(0,131)(0,101)(751,101)
 .STR
  "(Single Use)"
.TXT AT (1268,765)
 SIZE 80000
 TXTXYOFF <-4 80>
 T_PNTS (0,80)(0,0)(790,0)(790,103)(0,103)(0,80)(790,80)
 .STR
  "attach_to_event"
.TXT AT (3551,748)
 T_PNTS (0,80)(0,0)(790,0)(790,103)(0,103)(0,80)(790,80)
 .STR
  "attach_to_event"
.TXT AT (1970,1133)
 TXTXYOFF <-6 81>
 T_PNTS (0,81)(0,0)(628,0)(628,104)(0,104)(0,81)(628,81)
 .STR
  "Call_Routine"
.TXT AT (2695,1766)
 T_PNTS (0,81)(0,0)(628,0)(628,104)(0,104)(0,81)(628,81)
 .STR
  "Call_Routine"
.TXT AT (1327,1731)
 T_PNTS (0,81)(0,0)(628,0)(628,104)(0,104)(0,81)(628,81)
 .STR
  "Call_Routine"
.TXT AT (279,1733)
 T_PNTS (0,81)(0,0)(628,0)(628,104)(0,104)(0,81)(628,81)
 .STR
  "Call_Routine"
.POL AT (4014,339)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14 MARGINS <125 125 250 62>
 PNTS (0,0)(500,0)
.RECT AT (4590,82)
 BACKFILL <16 16 5 1000 0 0 0>
 XYRAD <146 146>
 PNTS (0,0)(1181,0)(1181,597)(0,597)(0,0)
.TXT AT (4829,312)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 100000 MARGINS <0 0 0 0>
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(661,0)(661,104)(0,104)(0,101)(661,101)
 .STR
  "Subroutine"
.POL AT (4477,273)
 THICKNESS 1 MARGINS <125 125 250 62>
 PNTS (117,62)(0,116)(0,0)(117,62)
.RECT AT (4590,998)
 BACKFILL <16 16 5 1000 0 0 0>
 THICKNESS 14
 PNTS (0,0)(1181,0)(1181,597)(0,597)(0,0)
.TXT AT (4939,1088)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-10 99>
 T_PNTS (0,99)(0,0)(312,0)(312,102)(0,102)(0,99)(312,99)
 .STR
  "Local"
.TXT AT (4829,1229)
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(661,0)(661,104)(0,104)(0,101)(661,101)
 .STR
  "Subroutine"
.TXT AT (4796,1420)
 TXTXYOFF <-8 101>
 T_PNTS (0,101)(0,0)(751,0)(751,131)(0,131)(0,101)(751,101)
 .STR
  "(Single Use)"
.POL AT (5180,672)
 BACKFILL <16 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(0,333)
.POL AT (5118,889)
 BACKFILL <1 16 5 1000 0 0 0>
 THICKNESS 1
 PNTS (62,116)(116,0)(0,0)(62,116)
.TXT AT (5278,850)
 SIZE 80000 MARGINS <0 0 0 0>
 TXTXYOFF <-6 81>
 T_PNTS (0,81)(0,0)(628,0)(628,104)(0,104)(0,81)(628,81)
 .STR
  "Call_Routine"
.TXT AT (4029,16)
 T_PNTS (0,81)(0,0)(628,0)(628,104)(0,104)(0,81)(628,81)
 .STR
  "Call_Routine"
.TXT AT (3355,2588)
 SIZE 100000
 TXTXYOFF <-10 99>
 T_PNTS (0,99)(0,0)(312,0)(312,102)(0,102)(0,99)(312,99)
 .STR
  "Local"
.TXT AT (3246,2729)
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(661,0)(661,104)(0,104)(0,101)(661,101)
 .STR
  "Subroutine"
.TXT AT (3213,2920)
 TXTXYOFF <-8 101>
 T_PNTS (0,101)(0,0)(751,0)(751,131)(0,131)(0,101)(751,101)
 .STR
  "(Single Use)"
.POL AT (2181,2588)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14 MARGINS <125 125 250 62>
 PNTS (0,0)(250,167)
.POL AT (2431,2755)
 PNTS (0,0)(666,0)
.POL AT (2977,2689)
 BACKFILL <1 16 5 1000 0 0 0>
 THICKNESS 1
 PNTS (117,63)(0,117)(0,0)(117,63)
.TXT AT (2362,2850)
 SIZE 80000 MARGINS <0 0 0 0>
 TXTXYOFF <-6 81>
 T_PNTS (0,81)(0,0)(628,0)(628,104)(0,104)(0,81)(628,81)
 .STR
  "Call_Routine"
END PICTURE

*END GRAPHICS
<end_data>
<object "object7" docType:201 appType:2>
<start_data Applix>
*BEGIN GRAPHICS VERSION=430/420 ENCODING=7BIT

COLORMAP
<"Transparent" 0 0 0 0 0 1>
<"Black" 0 0 0 0 255 0>
<"Dark gray" 0 0 0 0 127 0>
<"Dark blue" 0 128 128 0 127 0>
<"Blue" 0 255 255 0 0 0>
<"Dark cyan" 0 128 0 0 127 0>
<"Cyan" 0 255 0 0 0 0>
<"Dark green" 0 128 0 128 127 0>
<"Green" 0 255 0 255 0 0>
<"Dark magenta" 0 0 128 0 127 0>
<"Magenta" 0 0 255 0 0 0>
<"Dark red" 0 0 128 128 127 0>
<"Red" 0 0 255 255 0 0>
<"Dark yellow" 0 0 0 128 127 0>
<"Yellow" 0 0 0 255 0 0>
<"Light gray" 0 0 0 0 63 0>
<"White" 0 0 0 0 0 0>
END COLORMAP

FONT
"Arial"
END FONT

SESSION
 WIN_SIZE <7501 5501>
 VIEW <0 0 0 0 0 0>
 REZ <0 1000 1000 0 0>
END SESSION

PICTURE
.POL AT (31,724)
 BACKFILL <1 16 0 0 0 0 0>
 LINEFILL <1 16 5 1000 0 0 0>
 SHADOW <1 0 15 15>
 THICKNESS 14
 PNTS (0,0)(707,0)(707,118)(0,118)
.POL AT (1092,134)
 PNTS (0,0)(708,0)(708,118)(0,118)
.TXT AT (1337,130)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 100000 VER_JUST BASELINE MARGINS <0 0 0 0>
 TXTXYOFF <-9 99>
 T_PNTS (0,99)(0,0)(245,0)(245,129)(0,129)(0,99)(245,99)
 .STR
  "play"
.POL AT (1092,720)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(708,0)(708,119)(0,119)
.TXT AT (1336,762)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 90000 MARGINS <0 0 0 0>
 TXTXYOFF <-8 66>
 T_PNTS (0,66)(0,0)(166,0)(166,69)(0,69)(0,66)(166,66)
 .STR
  "rec"
.POL AT (1092,1311)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(708,0)(708,116)(0,116)
.TXT AT (82,735)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-4 90>
 T_PNTS (0,90)(0,0)(597,0)(597,116)(0,116)(0,90)(597,90)
 .STR
  "studio_tool"
.POL AT (738,793)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(118,0)
.POL AT (856,203)
 PNTS (0,590)(0,0)
.POL AT (856,1342)
 PNTS (0,0)(236,0)
.POL AT (1009,1300)
 PNTS (83,42)(0,0)
.POL AT (1009,1342)
 PNTS (83,0)(0,42)
.POL AT (856,797)
 PNTS (0,0)(0,562)
.TXT AT (1138,1315)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 T_PNTS (0,90)(0,0)(622,0)(622,117)(0,117)(0,90)(622,90)
 .STR
  "diagnostics"
.POL AT (849,793)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(236,0)
.POL AT (1002,752)
 PNTS (83,41)(0,0)
.POL AT (1002,793)
 PNTS (83,0)(0,42)
.POL AT (856,203)
 PNTS (0,0)(236,0)
.POL AT (1009,161)
 PNTS (83,42)(0,0)
.POL AT (1009,203)
 PNTS (83,0)(0,42)
.POL AT (2036,14)
 PNTS (0,151)(0,144)(0,135)(0,128)(0,121)(2,114)(2,109)(4,102)(4,95)
      (4,90)(6,85)(7,78)(9,73)(11,68)(13,64)(14,61)(18,55)(20,50)
      (23,47)(25,43)(28,40)(32,36)(37,33)(40,29)(46,26)(49,24)(54,22)
      (60,19)(65,17)(70,16)(77,14)(82,12)(89,10)(96,9)(103,7)(112,5)
      (120,5)(129,3)(138,3)(146,2)(155,2)(165,2)(176,0)(186,0)(198,0)
      (211,0)(223,0)(235,0)(249,0)(599,0)(612,0)(624,0)(636,0)(648,0)
      (658,0)(669,0)(679,0)(690,0)(700,2)(709,2)(718,2)(726,3)(735,5)
      (742,5)(749,7)(756,9)(763,10)(770,12)(777,14)(782,16)(787,17)
      (792,21)(797,22)(803,26)(806,28)(811,31)(815,33)(818,36)(822,40)
      (823,43)(827,47)(829,52)(832,55)(834,61)(836,66)(839,71)(839,76)
      (841,81)(843,87)(843,94)(844,99)(846,106)(846,113)(846,120)
      (846,127)(848,134)(848,142)(848,151)(848,304)(848,312)(848,319)
      (848,326)(848,333)(848,340)(846,347)(846,352)(846,359)(844,364)
      (843,371)(843,377)(839,382)(839,387)(836,391)(834,396)(832,399)
      (829,404)(827,408)(823,411)(820,415)(816,418)(813,422)(808,425)
      (804,429)(799,430)(794,434)(790,436)(783,437)(778,439)(771,441)
      (766,444)(759,444)(752,446)(745,448)(737,450)(728,451)(719,451)
      (711,451)(702,453)(693,453)(683,455)(672,455)(662,455)(650,455)
      (638,455)(626,456)(613,456)(599,456)(249,456)(237,456)(224,456)
      (212,456)(200,456)(190,456)(179,456)(169,456)(158,455)(150,455)
      (139,455)(131,453)(122,451)(113,451)(106,450)(99,448)(93,448)
      (86,446)(79,444)(73,443)(68,441)(63,437)(56,436)(53,432)(47,430)
      (42,427)(39,424)(35,422)(32,418)(28,415)(25,411)(23,408)(20,403)
      (18,399)(14,394)(13,389)(11,384)(9,378)(7,373)(7,368)(6,361)
      (4,356)(4,349)(4,342)(2,335)(2,328)(2,321)(0,312)(0,304)
.TXT AT (2076,108)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 80000 MARGINS <0 0 0 0>
 TXTXYOFF <-5 81>
 T_PNTS (0,81)(0,0)(821,0)(821,104)(0,104)(0,81)(821,81)
 .STR
  "SAMPL_PARAM"
.TXT AT (2217,228)
 TXTXYOFF <-9 80>
 T_PNTS (0,80)(0,0)(542,0)(542,81)(0,81)(0,80)(542,80)
 .STR
  "FILENAME"
.POL AT (2030,575)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,152)(0,144)(0,137)(0,130)(0,123)(1,116)(1,109)(3,104)(3,97)
      (3,92)(5,85)(6,80)(8,74)(10,69)(12,66)(13,60)(17,55)(19,52)
      (22,48)(24,45)(27,41)(31,38)(36,34)(39,31)(45,27)(48,24)(53,22)
      (59,20)(64,19)(69,17)(76,14)(81,12)(88,10)(95,10)(102,8)(111,7)
      (119,5)(128,5)(137,3)(145,3)(154,3)(164,1)(175,1)(185,1)(197,1)
      (210,1)(222,0)(234,0)(248,0)(599,0)(611,0)(623,0)(635,0)(647,0)
      (658,0)(668,0)(678,0)(689,1)(699,1)(708,3)(717,3)(725,5)(734,5)
      (741,7)(748,8)(755,10)(762,10)(769,14)(776,14)(781,17)(786,19)
      (791,20)(796,24)(802,27)(805,29)(810,31)(814,34)(817,38)(821,41)
      (822,45)(826,48)(828,52)(831,57)(833,62)(835,66)(838,73)(838,76)
      (840,83)(842,88)(842,93)(843,100)(845,107)(845,114)(845,121)
      (845,128)(847,135)(847,144)(847,152)(847,305)(847,312)(847,321)
      (847,328)(847,335)(847,342)(845,347)(845,354)(845,359)(843,366)
      (842,371)(842,376)(838,383)(838,387)(835,392)(833,396)(831,401)
      (828,404)(826,409)(822,413)(819,416)(816,420)(812,423)(807,427)
      (803,428)(798,432)(793,434)(789,435)(783,439)(777,441)(770,442)
      (765,444)(758,446)(751,448)(744,449)(736,449)(727,451)(718,453)
      (710,453)(701,453)(692,455)(682,455)(671,456)(661,456)(649,456)
      (637,456)(625,456)(612,458)(599,458)(248,458)(236,458)(224,458)
      (211,458)(199,458)(189,458)(178,456)(168,456)(158,456)(149,456)
      (138,455)(130,455)(121,453)(112,453)(105,451)(99,449)(92,448)
      (85,446)(78,444)(72,442)(67,441)(62,439)(55,435)(52,434)(46,430)
      (41,428)(38,425)(34,422)(31,418)(27,415)(24,411)(22,408)(19,404)
      (17,401)(13,396)(12,390)(10,385)(8,380)(6,375)(6,369)(5,363)
      (3,356)(3,349)(3,343)(1,336)(1,330)(1,321)(0,314)(0,305)
.TXT AT (2069,671)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-5 81>
 T_PNTS (0,81)(0,0)(821,0)(821,104)(0,104)(0,81)(821,81)
 .STR
  "SAMPL_PARAM"
.TXT AT (2210,797)
 TXTXYOFF <-9 80>
 T_PNTS (0,80)(0,0)(542,0)(542,81)(0,81)(0,80)(542,80)
 .STR
  "FILENAME"
.POL AT (1814,780)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(236,0)
.POL AT (1967,738)
 PNTS (83,42)(0,0)
.POL AT (1967,780)
 PNTS (83,0)(0,41)
.POL AT (1807,203)
 PNTS (0,0)(236,0)
.POL AT (1960,161)
 PNTS (83,42)(0,0)
.POL AT (1960,203)
 PNTS (83,0)(0,42)
.TXT AT (2976,113)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 T_PNTS (0,80)(0,0)(790,0)(790,104)(0,104)(0,80)(790,80)
 .STR
  "Play FILENAME"
.TXT AT (2967,231)
 TXTXYOFF <0 80>
 T_PNTS (0,80)(0,0)(1453,0)(1453,104)(0,104)(0,80)(1453,80)
 .STR
  "with the sampling parameters"
.TXT AT (2976,703)
 TXTXYOFF <-9 80>
 T_PNTS (0,80)(0,0)(1056,0)(1056,82)(0,82)(0,80)(1056,80)
 .STR
  "Record to FILENAME"
.TXT AT (2967,821)
 TXTXYOFF <0 80>
 T_PNTS (0,80)(0,0)(1453,0)(1453,104)(0,104)(0,80)(1453,80)
 .STR
  "with the sampling parameters"
.TXT AT (2972,938)
 TXTXYOFF <-5 81>
 T_PNTS (0,81)(0,0)(1532,0)(1532,104)(0,104)(0,81)(1532,81)
 .STR
  "SAMPL_PARAM. Emit VU info."
.TXT AT (2972,348)
 T_PNTS (0,81)(0,0)(1712,0)(1712,104)(0,104)(0,81)(1712,81)
 .STR
  "SAMPL_PARAM. Emit line/VU info"
.POL AT (14,722)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,117)(0,0)
.POL AT (1080,130)
 PNTS (0,117)(0,0)
.POL AT (1089,722)
 PNTS (0,117)(0,0)
.POL AT (1097,1313)
 PNTS (0,116)(0,0)
.POL AT (2031,123)
 PNTS (0,231)(9,0)
.POL AT (2023,722)
 PNTS (0,174)(0,0)
.TXT AT (2188,335)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-8 81>
 T_PNTS (0,81)(0,0)(659,0)(659,104)(0,104)(0,81)(659,81)
 .STR
  "[PLAYLN/VU]"
.TXT AT (2410,910)
 TXTXYOFF <-8 80>
 T_PNTS (0,80)(0,0)(202,0)(202,103)(0,103)(0,80)(202,80)
 .STR
  "[UV]"
END PICTURE

*END GRAPHICS
<end_data>
<object "object6" docType:201 appType:2>
<start_data Applix>
*BEGIN GRAPHICS VERSION=430/420 ENCODING=7BIT

COLORMAP
<"Transparent" 0 0 0 0 0 1>
<"Black" 0 0 0 0 255 0>
<"Dark gray" 0 0 0 0 127 0>
<"Dark blue" 0 128 128 0 127 0>
<"Blue" 0 255 255 0 0 0>
<"Dark cyan" 0 128 0 0 127 0>
<"Cyan" 0 255 0 0 0 0>
<"Dark green" 0 128 0 128 127 0>
<"Green" 0 255 0 255 0 0>
<"Dark magenta" 0 0 128 0 127 0>
<"Magenta" 0 0 255 0 0 0>
<"Dark red" 0 0 128 128 127 0>
<"Red" 0 0 255 255 0 0>
<"Dark yellow" 0 0 0 128 127 0>
<"Yellow" 0 0 0 255 0 0>
<"Light gray" 0 0 0 0 63 0>
<"White" 0 0 0 0 0 0>
END COLORMAP

FONT
"Arial"
END FONT

SESSION
 WIN_SIZE <7500 5500>
 VIEW <0 0 0 0 0 0>
 BACKFILL <1 0 0 0 0 0 0>
 LINEFILL <1 2 5 1000 0 0 0>
 SHADOW <6 0 15 15>
 SIZE 180000 VER_JUST TOP
 REZ <0 1000 1000 0 0>
END SESSION

PICTURE
.POL AT (14,962)
 BACKFILL <1 16 0 0 0 0 0>
 LINEFILL <1 16 5 1000 0 0 0>
 SHADOW <1 0 15 15>
 THICKNESS 14
 PNTS (0,0)(706,0)(706,118)(0,118)
.POL AT (2137,135)
 PNTS (0,0)(709,0)(709,118)(0,118)
.TXT AT (2377,153)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 90000 VER_JUST BASELINE MARGINS <0 0 0 0>
 TXTXYOFF <-4 90>
 T_PNTS (0,90)(0,0)(198,0)(198,93)(0,93)(0,90)(198,90)
 .STR
  "dev"
.POL AT (2137,608)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(709,0)(709,118)(0,118)
.TXT AT (2375,625)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-2 90>
 T_PNTS (0,90)(0,0)(151,0)(151,93)(0,93)(0,90)(151,90)
 .STR
  "vol"
.POL AT (2137,1080)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(709,0)(709,118)(0,118)
.TXT AT (2377,1122)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-4 66>
 T_PNTS (0,66)(0,0)(163,0)(163,69)(0,69)(0,66)(163,66)
 .STR
  "src"
.POL AT (1075,372)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(708,0)(708,118)(0,118)
.TXT AT (1315,391)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 100000 MARGINS <0 0 0 0>
 TXTXYOFF <-4 97>
 T_PNTS (0,97)(0,0)(189,0)(189,127)(0,127)(0,97)(189,97)
 .STR
  "get"
.POL AT (1075,1670)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(708,0)(708,118)(0,118)
.TXT AT (1315,1690)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 90000 MARGINS <0 0 0 0>
 TXTXYOFF <-4 88>
 T_PNTS (0,88)(0,0)(164,0)(164,91)(0,91)(0,88)(164,88)
 .STR
  "set"
.POL AT (2137,1434)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(709,0)(709,118)(0,118)
.TXT AT (2375,1452)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-2 90>
 T_PNTS (0,90)(0,0)(151,0)(151,93)(0,93)(0,90)(151,90)
 .STR
  "vol"
.POL AT (2137,1906)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(709,0)(709,118)(0,118)
.TXT AT (2377,1948)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-4 66>
 T_PNTS (0,66)(0,0)(163,0)(163,69)(0,69)(0,66)(163,66)
 .STR
  "src"
.POL AT (3200,24)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(708,0)(708,118)(0,118)
.TXT AT (3370,37)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-4 88>
 T_PNTS (0,88)(0,0)(342,0)(342,91)(0,91)(0,88)(342,88)
 .STR
  "stereo"
.POL AT (3200,135)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(708,0)(708,118)(0,118)
.TXT AT (3441,153)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-5 90>
 T_PNTS (0,90)(0,0)(113,0)(113,93)(0,93)(0,90)(113,90)
 .STR
  "all"
.POL AT (3200,253)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(708,0)(708,119)(0,119)
.TXT AT (3444,295)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-8 66>
 T_PNTS (0,66)(0,0)(166,0)(166,69)(0,69)(0,66)(166,66)
 .STR
  "rec"
.POL AT (3200,372)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(708,0)(708,118)(0,118)
.TXT AT (3440,391)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-4 88>
 T_PNTS (0,88)(0,0)(171,0)(171,91)(0,91)(0,88)(171,88)
 .STR
  "out"
.POL AT (3200,608)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,40)(0,36)(0,34)(0,33)(0,29)(1,29)(1,27)(3,26)(3,24)(5,22)
      (7,20)(7,19)(8,19)(10,17)(12,17)(14,15)(15,14)(19,12)(21,12)
      (24,12)(27,10)(31,8)(34,8)(38,7)(41,7)(45,7)(48,7)(54,5)(59,5)
      (64,5)(69,5)(74,5)(80,5)(86,3)(93,3)(100,1)(107,1)(114,1)(121,1)
      (130,1)(139,1)(149,1)(158,1)(166,1)(177,1)(187,1)(198,0)(208,0)
      (618,0)(628,0)(639,0)(649,0)(658,0)(668,0)(677,0)(685,0)(694,0)
      (701,0)(710,0)(717,0)(724,1)(731,1)(738,1)(743,1)(750,1)(755,1)
      (760,3)(765,3)(771,5)(776,5)(779,5)(784,7)(788,7)(791,8)(795,8)
      (797,8)(800,10)(804,10)(807,12)(809,12)(810,12)(812,14)(814,15)
      (817,17)(817,19)(819,19)(821,20)(821,22)(823,24)(824,26)(824,27)
      (824,29)(824,31)(824,33)(826,34)(826,36)(826,40)(826,78)(826,81)
      (826,83)(826,85)(826,88)(824,90)(824,92)(824,93)(823,95)(821,97)
      (819,99)(817,99)(817,100)(816,100)(814,102)(810,104)(809,106)
      (805,106)(804,106)(800,107)(797,109)(793,109)(790,111)(786,111)
      (783,111)(777,111)(774,112)(769,112)(764,112)(758,114)(753,114)
      (746,114)(741,114)(734,116)(727,116)(720,116)(713,116)(705,116)
      (696,116)(687,116)(679,116)(670,116)(661,116)(651,116)(640,118)
      (630,118)(618,118)(208,118)(198,118)(187,118)(179,118)(168,118)
      (159,118)(151,118)(142,118)(133,118)(125,118)(118,118)(109,118)
      (102,118)(95,118)(90,118)(83,118)(78,116)(73,116)(66,116)(60,114)
      (55,114)(52,112)(47,112)(43,112)(38,111)(34,111)(33,109)(29,109)
      (26,109)(24,107)(21,107)(19,106)(17,106)(14,104)(12,102)(10,100)
      (8,99)(7,99)(7,97)(7,95)(5,93)(3,92)(3,90)(1,88)(1,86)(1,85)
      (1,83)(0,81)(0,78)
.TXT AT (3444,627)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 80000 MARGINS <0 0 0 0>
 TXTXYOFF <-8 81>
 T_PNTS (0,81)(0,0)(307,0)(307,83)(0,83)(0,81)(307,81)
 .STR
  "NONE"
.POL AT (3200,726)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,40)(0,36)(0,34)(0,33)(0,29)(1,29)(1,27)(3,26)(3,24)(5,22)
      (7,21)(7,19)(8,19)(10,17)(12,17)(14,15)(15,14)(19,12)(21,12)
      (24,12)(27,10)(31,8)(34,8)(38,7)(41,7)(45,7)(48,7)(54,5)(59,5)
      (64,5)(69,5)(74,5)(80,5)(86,3)(93,3)(100,1)(107,1)(114,1)(121,1)
      (130,1)(139,1)(149,1)(158,1)(166,1)(177,1)(187,1)(198,0)(208,0)
      (618,0)(628,0)(639,0)(649,0)(658,0)(668,0)(677,0)(685,0)(694,0)
      (701,0)(710,0)(717,0)(724,1)(731,1)(738,1)(743,1)(750,1)(755,1)
      (760,3)(765,3)(771,5)(776,5)(779,5)(784,7)(788,7)(791,8)(795,8)
      (797,8)(800,10)(804,10)(807,12)(809,12)(810,12)(812,14)(814,15)
      (817,17)(817,19)(819,19)(821,21)(821,22)(823,24)(824,26)(824,27)
      (824,29)(824,31)(824,33)(826,34)(826,36)(826,40)(826,78)(826,81)
      (826,83)(826,85)(826,88)(824,90)(824,92)(824,93)(823,95)(821,97)
      (819,99)(817,99)(817,100)(816,100)(814,102)(810,104)(809,106)
      (805,106)(804,106)(800,107)(797,109)(793,109)(790,111)(786,111)
      (783,111)(777,111)(774,113)(769,113)(764,113)(758,114)(753,114)
      (746,114)(741,114)(734,116)(727,116)(720,116)(713,116)(705,116)
      (696,116)(687,116)(679,116)(670,116)(661,116)(651,116)(640,118)
      (630,118)(618,118)(208,118)(198,118)(187,118)(179,118)(168,118)
      (159,118)(151,118)(142,118)(133,118)(125,118)(118,118)(109,118)
      (102,118)(95,118)(90,118)(83,118)(78,116)(73,116)(66,116)(60,114)
      (55,114)(52,113)(47,113)(43,113)(38,111)(34,111)(33,109)(29,109)
      (26,109)(24,107)(21,107)(19,106)(17,106)(14,104)(12,102)(10,100)
      (8,99)(7,99)(7,97)(7,95)(5,93)(3,92)(3,90)(1,88)(1,87)(1,85)
      (1,83)(0,81)(0,78)
.TXT AT (3278,745)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-9 81>
 T_PNTS (0,81)(0,0)(711,0)(711,104)(0,104)(0,81)(711,81)
 .STR
  "DEVICE_NUM"
.POL AT (3200,962)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,38)(0,36)(0,35)(0,31)(0,29)(1,28)(3,26)(3,24)(5,22)(7,21)
      (7,19)(8,17)(10,17)(12,17)(14,15)(15,14)(19,12)(21,12)(24,10)
      (27,10)(31,8)(34,7)(38,7)(41,7)(45,7)(48,7)(54,5)(59,5)(64,3)
      (69,3)(74,3)(80,3)(86,3)(93,3)(100,2)(107,2)(114,2)(121,2)(130,2)
      (139,2)(149,2)(158,2)(166,2)(177,2)(187,0)(198,0)(208,0)(618,0)
      (628,0)(639,0)(649,0)(658,0)(668,0)(677,0)(685,0)(694,0)(701,0)
      (710,0)(717,0)(724,0)(731,0)(738,0)(743,0)(750,2)(755,2)(760,3)
      (765,3)(771,3)(776,5)(779,5)(784,7)(788,7)(791,7)(795,8)(797,8)
      (800,10)(804,10)(807,10)(809,12)(810,12)(812,14)(814,15)(817,17)
      (819,19)(821,21)(821,22)(823,24)(824,24)(824,28)(824,31)(824,33)
      (826,35)(826,36)(826,38)(826,78)(826,80)(826,81)(826,83)(826,85)
      (826,87)(826,88)(824,90)(824,92)(824,94)(823,95)(821,97)(819,97)
      (817,99)(817,101)(816,101)(814,102)(810,104)(809,104)(805,106)
      (804,106)(800,107)(797,107)(793,109)(790,111)(786,111)(783,111)
      (777,111)(774,111)(769,113)(764,113)(758,114)(753,114)(746,114)
      (741,114)(734,114)(727,116)(720,116)(713,116)(705,116)(696,116)
      (687,116)(679,116)(670,116)(661,116)(651,116)(640,118)(630,118)
      (618,118)(208,118)(198,118)(187,118)(179,118)(168,118)(159,118)
      (151,118)(142,118)(133,118)(125,118)(118,118)(109,118)(102,118)
      (95,118)(90,118)(83,118)(78,116)(73,116)(66,114)(60,114)(55,114)
      (52,113)(47,113)(43,111)(38,111)(34,111)(33,109)(29,109)(26,107)
      (24,107)(21,107)(19,106)(17,104)(14,104)(12,102)(10,101)(8,99)
      (7,97)(7,95)(5,94)(3,92)(3,90)(1,88)(1,87)(1,85)(1,83)(0,80)
      (0,78)
.TXT AT (3444,982)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-8 81>
 T_PNTS (0,81)(0,0)(307,0)(307,83)(0,83)(0,81)(307,81)
 .STR
  "NONE"
.POL AT (3200,1080)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,38)(0,36)(0,35)(0,31)(0,29)(1,28)(3,26)(3,24)(5,22)(7,21)
      (7,19)(8,17)(10,17)(12,17)(14,15)(15,14)(19,12)(21,12)(24,10)
      (27,10)(31,9)(34,7)(38,7)(41,7)(45,7)(48,7)(54,5)(59,5)(64,3)
      (69,3)(74,3)(80,3)(86,3)(93,3)(100,2)(107,2)(114,2)(121,2)(130,2)
      (139,2)(149,2)(158,2)(166,2)(177,2)(187,0)(198,0)(208,0)(618,0)
      (628,0)(639,0)(649,0)(658,0)(668,0)(677,0)(685,0)(694,0)(701,0)
      (710,0)(717,0)(724,0)(731,0)(738,0)(743,0)(750,2)(755,2)(760,3)
      (765,3)(771,3)(776,5)(779,5)(784,7)(788,7)(791,7)(795,9)(797,9)
      (800,10)(804,10)(807,10)(809,12)(810,12)(812,14)(814,15)(817,17)
      (819,19)(821,21)(821,22)(823,24)(824,24)(824,28)(824,31)(824,33)
      (826,35)(826,36)(826,38)(826,78)(826,80)(826,81)(826,83)(826,85)
      (826,87)(826,88)(824,90)(824,92)(824,94)(823,95)(821,97)(819,97)
      (817,99)(817,101)(816,101)(814,102)(810,104)(809,104)(805,106)
      (804,106)(800,108)(797,108)(793,109)(790,111)(786,111)(783,111)
      (777,111)(774,111)(769,113)(764,113)(758,114)(753,114)(746,114)
      (741,114)(734,114)(727,116)(720,116)(713,116)(705,116)(696,116)
      (687,116)(679,116)(670,116)(661,116)(651,116)(640,118)(630,118)
      (618,118)(208,118)(198,118)(187,118)(179,118)(168,118)(159,118)
      (151,118)(142,118)(133,118)(125,118)(118,118)(109,118)(102,118)
      (95,118)(90,118)(83,118)(78,116)(73,116)(66,114)(60,114)(55,114)
      (52,113)(47,113)(43,111)(38,111)(34,111)(33,109)(29,109)(26,108)
      (24,108)(21,108)(19,106)(17,104)(14,104)(12,102)(10,101)(8,99)
      (7,97)(7,95)(5,94)(3,92)(3,90)(1,88)(1,87)(1,85)(1,83)(0,80)
      (0,78)
.TXT AT (3278,1100)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-9 81>
 T_PNTS (0,81)(0,0)(711,0)(711,104)(0,104)(0,81)(711,81)
 .STR
  "DEVICE_NUM"
.POL AT (3200,1788)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,117)(0,110)(0,104)(0,99)(0,94)(0,87)(1,84)(1,78)(3,73)(3,70)
      (5,64)(7,61)(7,56)(8,52)(10,49)(12,45)(14,42)(15,38)(19,37)
      (21,33)(24,31)(27,28)(31,25)(34,23)(38,21)(41,18)(45,18)(48,16)
      (54,14)(59,12)(64,11)(69,9)(74,9)(80,7)(86,7)(93,5)(100,4)(107,4)
      (114,4)(121,4)(130,2)(139,2)(149,0)(158,0)(166,0)(177,0)(187,0)
      (198,0)(208,0)(618,0)(628,0)(639,0)(649,0)(658,0)(668,0)(677,0)
      (685,0)(694,0)(701,0)(710,2)(717,2)(724,4)(731,4)(738,4)(743,5)
      (750,7)(755,7)(760,9)(765,11)(771,12)(776,14)(779,16)(784,18)
      (788,21)(791,23)(795,25)(797,26)(800,30)(804,31)(807,35)(809,38)
      (810,40)(812,44)(814,47)(817,51)(817,56)(819,59)(821,63)(821,66)
      (823,71)(824,77)(824,82)(824,87)(824,92)(824,97)(826,104)(826,110)
      (826,117)(826,235)(826,242)(826,247)(826,252)(826,257)(826,264)
      (826,268)(824,273)(824,278)(824,281)(823,287)(821,290)(819,295)
      (817,299)(817,302)(816,306)(814,309)(810,313)(809,314)(805,318)
      (804,320)(800,323)(797,327)(793,328)(790,332)(786,334)(783,335)
      (777,337)(774,339)(769,340)(764,342)(758,344)(753,344)(746,344)
      (741,346)(734,347)(727,349)(720,349)(713,351)(705,351)(696,351)
      (687,351)(679,353)(670,353)(661,353)(651,353)(640,353)(630,353)
      (618,353)(208,353)(198,353)(187,353)(179,353)(168,353)(159,353)
      (151,353)(142,353)(133,353)(125,353)(118,351)(109,351)(102,351)
      (95,349)(90,349)(83,347)(78,347)(73,346)(66,344)(60,342)(55,340)
      (52,339)(47,337)(43,335)(38,332)(34,330)(33,328)(29,327)(26,323)
      (24,320)(21,318)(19,314)(17,311)(14,308)(12,304)(10,301)(8,295)
      (7,292)(7,288)(7,285)(5,280)(3,275)(3,269)(1,264)(1,259)(1,254)
      (1,247)(0,242)(0,235)
.TXT AT (3278,1808)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 T_PNTS (0,81)(0,0)(711,0)(711,104)(0,104)(0,81)(711,81)
 .STR
  "DEVICE_NUM"
.TXT AT (3323,1985)
 TXTXYOFF <-5 81>
 T_PNTS (0,81)(0,0)(507,0)(507,104)(0,104)(0,81)(507,81)
 .STR
  "ON_FLAG"
.POL AT (3200,1316)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,116)(0,109)(0,104)(0,99)(0,94)(0,89)(1,83)(1,78)(3,75)(3,69)
      (5,64)(7,61)(7,57)(8,54)(10,50)(12,47)(14,43)(15,40)(19,36)
      (21,33)(24,31)(27,28)(31,26)(34,23)(38,21)(41,19)(45,17)(48,16)
      (54,14)(59,12)(64,10)(69,9)(74,9)(80,9)(86,7)(93,5)(100,5)(107,3)
      (114,3)(121,3)(130,2)(139,2)(149,2)(158,2)(166,2)(177,2)(187,0)
      (198,0)(208,0)(618,0)(628,0)(639,0)(649,0)(658,0)(668,0)(677,0)
      (685,0)(694,0)(701,2)(710,2)(717,2)(724,3)(731,3)(738,5)(743,5)
      (750,7)(755,9)(760,9)(765,10)(771,12)(776,14)(779,16)(784,19)
      (788,21)(791,23)(795,24)(797,26)(800,30)(804,33)(807,35)(809,38)
      (810,40)(812,43)(814,47)(817,50)(817,56)(819,59)(821,62)(821,68)
      (823,71)(824,76)(824,82)(824,87)(824,92)(824,97)(826,104)(826,109)
      (826,116)(826,234)(826,241)(826,247)(826,252)(826,259)(826,264)
      (826,269)(824,273)(824,278)(824,281)(823,286)(821,290)(819,295)
      (817,299)(817,302)(816,306)(814,309)(810,312)(809,314)(805,318)
      (804,321)(800,323)(797,326)(793,328)(790,332)(786,333)(783,335)
      (777,337)(774,339)(769,340)(764,342)(758,344)(753,344)(746,345)
      (741,345)(734,347)(727,349)(720,349)(713,351)(705,351)(696,351)
      (687,352)(679,352)(670,352)(661,352)(651,352)(640,352)(630,354)
      (618,354)(208,354)(198,354)(187,354)(179,354)(168,354)(159,354)
      (151,354)(142,354)(133,352)(125,352)(118,352)(109,351)(102,351)
      (95,349)(90,349)(83,349)(78,347)(73,345)(66,344)(60,342)(55,342)
      (52,339)(47,337)(43,335)(38,332)(34,330)(33,328)(29,326)(26,323)
      (24,321)(21,318)(19,314)(17,311)(14,307)(12,304)(10,300)(8,297)
      (7,293)(7,288)(7,285)(5,280)(3,274)(3,269)(1,266)(1,259)(1,253)
      (1,248)(0,241)(0,234)
.TXT AT (3278,1336)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-9 81>
 T_PNTS (0,81)(0,0)(711,0)(711,104)(0,104)(0,81)(711,81)
 .STR
  "DEVICE_NUM"
.TXT AT (3326,1454)
 TXTXYOFF <-8 81>
 T_PNTS (0,81)(0,0)(545,0)(545,104)(0,104)(0,81)(545,81)
 .STR
  "LEFT_VOL"
.TXT AT (3327,1572)
 TXTXYOFF <-9 81>
 T_PNTS (0,81)(0,0)(617,0)(617,104)(0,104)(0,81)(617,81)
 .STR
  "RIGHT_VOL"
.TXT AT (18,973)
 SIZE 90000
 TXTXYOFF <-4 90>
 T_PNTS (0,90)(0,0)(711,0)(711,116)(0,116)(0,90)(711,90)
 .STR
  "studio_mixer"
.POL AT (720,1031)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(119,0)
.POL AT (839,441)
 PNTS (0,590)(0,0)
.POL AT (839,441)
 PNTS (0,0)(236,0)
.POL AT (991,399)
 PNTS (84,42)(0,0)
.POL AT (991,441)
 PNTS (84,0)(0,42)
.POL AT (839,1031)
 PNTS (0,0)(0,709)
.POL AT (839,1740)
 PNTS (0,0)(236,0)
.POL AT (991,1698)
 PNTS (84,42)(0,0)
.POL AT (991,1740)
 PNTS (84,0)(0,41)
.POL AT (1783,434)
 PNTS (0,0)(118,0)
.POL AT (1901,198)
 PNTS (0,236)(0,0)
.POL AT (1901,198)
 PNTS (0,0)(236,0)
.POL AT (2054,156)
 PNTS (83,42)(0,0)
.POL AT (2054,198)
 PNTS (83,0)(0,42)
.POL AT (1901,434)
 PNTS (0,0)(0,708)
.POL AT (1901,670)
 PNTS (0,0)(236,0)
.POL AT (2054,628)
 PNTS (83,42)(0,0)
.POL AT (2054,670)
 PNTS (83,0)(0,42)
.POL AT (1901,1142)
 PNTS (0,0)(236,0)
.POL AT (2054,1101)
 PNTS (83,41)(0,0)
.POL AT (2054,1142)
 PNTS (83,0)(0,42)
.POL AT (1783,1733)
 PNTS (0,0)(118,0)
.POL AT (1901,1497)
 PNTS (0,472)(0,0)
.POL AT (1901,1497)
 PNTS (0,0)(236,0)
.POL AT (2054,1455)
 PNTS (83,42)(0,0)
.POL AT (2054,1497)
 PNTS (83,0)(0,41)
.POL AT (1901,1969)
 PNTS (0,0)(236,0)
.POL AT (2054,1927)
 PNTS (83,42)(0,0)
.POL AT (2054,1969)
 PNTS (83,0)(0,41)
.POL AT (2846,191)
 PNTS (0,0)(118,0)
.POL AT (2964,73)
 PNTS (0,354)(0,0)
.POL AT (2964,73)
 PNTS (0,0)(236,0)
.POL AT (3116,31)
 PNTS (84,42)(0,0)
.POL AT (3116,73)
 PNTS (84,0)(0,42)
.POL AT (2964,191)
 PNTS (0,0)(236,0)
.POL AT (3116,149)
 PNTS (84,42)(0,0)
.POL AT (3116,191)
 PNTS (84,0)(0,42)
.POL AT (2964,309)
 PNTS (0,0)(236,0)
.POL AT (3116,267)
 PNTS (84,42)(0,0)
.POL AT (3116,309)
 PNTS (84,0)(0,42)
.POL AT (2964,427)
 PNTS (0,0)(236,0)
.POL AT (3116,385)
 PNTS (84,42)(0,0)
.POL AT (3116,427)
 PNTS (84,0)(0,42)
.POL AT (2846,663)
 PNTS (0,0)(118,0)
.POL AT (2964,665)
 PNTS (0,0)(0,116)
.POL AT (2964,663)
 PNTS (0,0)(236,0)
.POL AT (3116,622)
 PNTS (84,41)(0,0)
.POL AT (3116,663)
 PNTS (84,0)(0,42)
.POL AT (2964,781)
 PNTS (0,0)(236,0)
.POL AT (3116,740)
 PNTS (84,41)(0,0)
.POL AT (3116,781)
 PNTS (84,0)(0,42)
.POL AT (2846,1135)
 PNTS (0,0)(118,0)
.POL AT (2957,990)
 PNTS (0,132)(0,0)
.POL AT (2964,997)
 PNTS (0,0)(236,0)
.POL AT (3116,955)
 PNTS (84,42)(0,0)
.POL AT (3116,997)
 PNTS (84,0)(0,41)
.POL AT (2964,1135)
 PNTS (0,0)(236,0)
.POL AT (3116,1094)
 PNTS (84,41)(0,0)
.POL AT (3116,1135)
 PNTS (84,0)(0,42)
.POL AT (2846,1503)
 PNTS (0,0)(354,0)
.POL AT (3116,1462)
 PNTS (84,41)(0,0)
.POL AT (3116,1503)
 PNTS (84,0)(0,42)
.POL AT (2846,1976)
 PNTS (0,0)(354,0)
.POL AT (3116,1934)
 PNTS (84,42)(0,0)
.POL AT (3116,1976)
 PNTS (84,0)(0,41)
.TXT AT (4062,38)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 80000 MARGINS <0 0 0 0>
 TXTXYOFF <-8 80>
 T_PNTS (0,80)(0,0)(938,0)(938,82)(0,82)(0,80)(938,80)
 .STR
  "List stereo devicies"
.TXT AT (4062,156)
 T_PNTS (0,80)(0,0)(715,0)(715,82)(0,82)(0,80)(715,80)
 .STR
  "List all devices"
.TXT AT (4062,274)
 T_PNTS (0,80)(0,0)(1068,0)(1068,104)(0,104)(0,80)(1068,80)
 .STR
  "List recording devices"
.TXT AT (4062,392)
 T_PNTS (0,80)(0,0)(913,0)(913,103)(0,103)(0,80)(913,80)
 .STR
  "List output devices"
.TXT AT (4062,628)
 T_PNTS (0,80)(0,0)(1112,0)(1112,104)(0,104)(0,80)(1112,80)
 .STR
  "List all volume settings"
.TXT AT (4062,745)
 TXTXYOFF <-8 81>
 T_PNTS (0,81)(0,0)(1848,0)(1848,105)(0,105)(0,81)(1848,81)
 .STR
  "List volume settings of DEVICE_NUM"
.TXT AT (4062,983)
 TXTXYOFF <-8 80>
 T_PNTS (0,80)(0,0)(1534,0)(1534,104)(0,104)(0,80)(1534,80)
 .STR
  "List all active recording devices"
.TXT AT (4062,1100)
 TXTXYOFF <-8 81>
 T_PNTS (0,81)(0,0)(1370,0)(1370,104)(0,104)(0,81)(1370,81)
 .STR
  "List status of DEVICE_NUM"
.TXT AT (4059,1336)
 TXTXYOFF <-5 81>
 T_PNTS (0,81)(0,0)(1020,0)(1020,105)(0,105)(0,81)(1020,81)
 .STR
  "Set left&right volume"
.TXT AT (4058,1454)
 TXTXYOFF <-4 81>
 T_PNTS (0,81)(0,0)(840,0)(840,104)(0,104)(0,81)(840,81)
 .STR
  "of DEVICE_NUM"
.TXT AT (4059,1808)
 TXTXYOFF <-5 81>
 T_PNTS (0,81)(0,0)(913,0)(913,104)(0,104)(0,81)(913,81)
 .STR
  "Set DEVICE_NUM"
.TXT AT (4058,1926)
 TXTXYOFF <-4 81>
 T_PNTS (0,81)(0,0)(1318,0)(1318,105)(0,105)(0,81)(1318,81)
 .STR
  "active/inactive (ON_FLAG)"
.TXT AT (4061,2045)
 TXTXYOFF <-7 80>
 T_PNTS (0,80)(0,0)(834,0)(834,104)(0,104)(0,80)(834,80)
 .STR
  "recording device."
.POL AT (14,957)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(0,125)
.POL AT (1047,382)
 PNTS (0,0)(0,109)
.POL AT (2130,1073)
 PNTS (0,0)(0,125)
.POL AT (2122,1424)
 PNTS (0,0)(0,125)
.POL AT (2130,1898)
 PNTS (0,0)(0,125)
.POL AT (1063,1665)
 PNTS (0,0)(0,125)
.POL AT (3196,14)
 PNTS (0,493)(0,0)
.POL AT (3196,1882)
 PNTS (0,0)(0,125)
.POL AT (3188,1408)
 PNTS (0,0)(0,125)
.POL AT (2130,132)
 PNTS (0,0)(0,125)
.POL AT (2139,606)
 PNTS (0,0)(0,125)
END PICTURE

*END GRAPHICS
<end_data>
<object "object5" docType:201 appType:2>
<start_data Applix>
*BEGIN GRAPHICS VERSION=430/420 ENCODING=7BIT

COLORMAP
<"Transparent" 0 0 0 0 0 1>
<"Black" 0 0 0 0 255 0>
<"Dark gray" 0 0 0 0 127 0>
<"Dark blue" 0 128 128 0 127 0>
<"Blue" 0 255 255 0 0 0>
<"Dark cyan" 0 128 0 0 127 0>
<"Cyan" 0 255 0 0 0 0>
<"Dark green" 0 128 0 128 127 0>
<"Green" 0 255 0 255 0 0>
<"Dark magenta" 0 0 128 0 127 0>
<"Magenta" 0 0 255 0 0 0>
<"Dark red" 0 0 128 128 127 0>
<"Red" 0 0 255 255 0 0>
<"Dark yellow" 0 0 0 128 127 0>
<"Yellow" 0 0 0 255 0 0>
<"Light gray" 0 0 0 0 63 0>
<"White" 0 0 0 0 0 0>
END COLORMAP

FONT
"Arial"
END FONT

SESSION
 WIN_SIZE <7501 5501>
 STICKY ON
 VIEW <0 0 0 0 0 0>
 BACKFILL <1 0 0 8 0 0 0>
 REZ <0 2000 2000 0 284>
END SESSION

PICTURE
.RECT AT (94,7)
 BACKFILL <16 16 5 1000 0 0 0>
 LINEFILL <1 16 5 1000 0 0 0>
 SHADOW <1 0 15 15>
 THICKNESS 14 SIZE 60000 VER_JUST BASELINE
 PNTS (0,0)(3430,0)(3430,264)(0,264)(0,0)
.RECT AT (94,7)
 BACKFILL <1 16 0 10 0 0 0>
 PNTS (0,0)(680,0)(680,264)(0,264)(0,0)
.RECT AT (94,1090)
 PNTS (0,0)(680,0)(680,264)(0,264)(0,0)
.RECT AT (767,14)
 BACKFILL <1 16 0 6 0 0 0>
 THICKNESS 1
 PNTS (0,0)(2750,0)(2750,250)(0,250)(0,0)
.POL AT (434,264)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14
 PNTS (0,833)(0,0)
.POL AT (1684,264)
 PNTS (0,0)(1583,505)
.POL AT (351,1014)
 BACKFILL <1 16 5 1000 0 0 0>
 THICKNESS 1
 PNTS (83,83)(166,0)(0,0)(83,83)
.POL AT (1184,1347)
 PNTS (83,0)(167,84)(0,84)(83,0)
.POL AT (1267,1347)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14
 PNTS (0,0)(0,500)
.TXT AT (88,1368)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 80000 MARGINS <0 0 0 0>
 TXTXYOFF <-3 80>
 T_PNTS (0,80)(0,0)(290,0)(290,82)(0,82)(0,80)(290,80)
 .STR
  "TMP5"
.TXT AT (3,283)
 T_PNTS (0,80)(0,0)(274,0)(274,81)(0,81)(0,80)(274,80)
 .STR
  "TMP1"
.POL AT (517,17)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 1 STYLE 1 SIZE 60000 MARGINS <125 125 250 62>
 PNTS (0,0)(0,384)
.TXT AT (474,423)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-4 61>
 T_PNTS (0,61)(0,0)(325,0)(325,63)(0,63)(0,61)(325,61)
 .STR
  "OFFSET"
.POL AT (767,255)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14 STYLE 0 MARGINS <125 125 250 62>
 PNTS (2,92)(0,0)
.TXT AT (804,305)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 T_PNTS (0,61)(0,0)(326,0)(326,63)(0,63)(0,61)(326,61)
 .STR
  "STARTX"
.TXT AT (543,982)
 TXTXYOFF <-5 65>
 T_PNTS (0,65)(0,0)(452,0)(452,74)(0,74)(0,65)(452,65)
 .STR
  "head -c $ofs"
.POL AT (1523,19)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (1,0)(0,333)
.TXT AT (1542,308)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-7 60>
 T_PNTS (0,60)(0,0)(225,0)(225,61)(0,61)(0,60)(225,60)
 .STR
  "ENDX"
.RECT AT (760,1674)
 BACKFILL <1 16 0 21 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(1183,0)(1183,263)(0,263)(0,0)
.TXT AT (758,1950)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 80000 MARGINS <0 0 0 0>
 TXTXYOFF <-6 81>
 T_PNTS (0,81)(0,0)(630,0)(630,83)(0,83)(0,81)(630,81)
 .STR
  "CLIPBOARD"
.RECT AT (760,1090)
 BACKFILL <1 16 0 21 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(1183,0)(1183,264)(0,264)(0,0)
.TXT AT (1291,1569)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 60000 MARGINS <0 0 0 0>
 TXTXYOFF <-3 61>
 T_PNTS (0,61)(0,0)(1254,0)(1254,78)(0,78)(0,61)(1254,61)
 .STR
  V_SPACE <0 83 0>
  ITALICS ON
  "append "
 .STR
  ITALICS OFF
  "cat CLIPBOARD >>TMP5"
.RECT AT (1936,1090)
 BACKFILL <1 16 0 6 0 0 0>
 V_SPACE <0 250 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(2764,0)(2764,264)(0,264)(0,0)
.POL AT (3189,1019)
 BACKFILL <1 16 5 1000 0 0 0>
 THICKNESS 1
 PNTS (83,83)(167,0)(0,0)(83,83)
.POL AT (3267,769)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14
 PNTS (0,0)(0,333)
.POL AT (2833,2047)
 BACKFILL <1 16 5 1000 0 0 0>
 THICKNESS 1
 PNTS (84,83)(167,0)(0,0)(84,83)
.POL AT (2917,1355)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14
 PNTS (0,0)(0,758)
.RECT AT (69,2123)
 BACKFILL <1 16 0 6 0 0 0>
 PNTS (0,0)(681,0)(681,264)(0,264)(0,0)
.RECT AT (736,2123)
 PNTS (0,0)(1182,0)(1182,264)(0,264)(0,0)
.RECT AT (1911,2123)
 PNTS (0,0)(2764,0)(2764,264)(0,264)(0,0)
.TXT AT (69,2415)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 80000 MARGINS <0 0 0 0>
 TXTXYOFF <-3 80>
 T_PNTS (0,80)(0,0)(274,0)(274,81)(0,81)(0,80)(274,80)
 .STR
  "TMP1"
.TXT AT (3061,2028)
 SIZE 60000
 TXTXYOFF <-6 60>
 T_PNTS (0,60)(0,0)(581,0)(581,62)(0,62)(0,60)(581,60)
 .STR
  "mv TMP5 TMP1"
.RECT AT (767,14)
 BACKFILL <1 0 0 8 0 0 0>
 LINEFILL <1 2 5 1000 0 0 0>
 SHADOW <6 0 15 15>
 THICKNESS 1 SIZE 180000 VER_JUST TOP MARGINS <125 125 250 62>
 PNTS (0,0)(760,0)(760,250)(0,250)(0,0)
.TXT AT (1794,667)
 BACKFILL <1 16 5 1000 0 0 0>
 LINEFILL <1 16 5 1000 0 0 0>
 SHADOW <1 0 15 15>
 THICKNESS 14 SIZE 60000 HOR_JUST RIGHT VER_JUST BASELINE MARGINS <0 0 0 0>
 TXTXYOFF <-1 65>
 T_PNTS (0,65)(0,0)(956,0)(956,82)(0,82)(0,65)(956,65)
 .STR
  "tail -c +[expr 1+$ofs+$len]"
.TXT AT (2677,480)
 HOR_JUST LEFT
 TXTXYOFF <-3 59>
 T_PNTS (0,59)(0,0)(280,0)(280,76)(0,76)(0,59)(280,59)
 .STR
  ITALICS ON
  "append"
END PICTURE

*END GRAPHICS
<end_data>
<object "object4" docType:201 appType:2>
<start_data Applix>
*BEGIN GRAPHICS VERSION=430/420 ENCODING=7BIT

COLORMAP
<"Transparent" 0 0 0 0 0 1>
<"Black" 0 0 0 0 255 0>
<"Dark gray" 0 0 0 0 127 0>
<"Dark blue" 0 128 128 0 127 0>
<"Blue" 0 255 255 0 0 0>
<"Dark cyan" 0 128 0 0 127 0>
<"Cyan" 0 255 0 0 0 0>
<"Dark green" 0 128 0 128 127 0>
<"Green" 0 255 0 255 0 0>
<"Dark magenta" 0 0 128 0 127 0>
<"Magenta" 0 0 255 0 0 0>
<"Dark red" 0 0 128 128 127 0>
<"Red" 0 0 255 255 0 0>
<"Dark yellow" 0 0 0 128 127 0>
<"Yellow" 0 0 0 255 0 0>
<"Light gray" 0 0 0 0 63 0>
<"White" 0 0 0 0 0 0>
END COLORMAP

FONT
"Arial"
END FONT

SESSION
 WIN_SIZE <7501 5501>
 VIEW <0 0 0 0 0 0>
 BACKFILL <1 0 0 8 0 0 0>
 REZ <0 2000 2000 0 0>
END SESSION

PICTURE
.RECT AT (94,257)
 BACKFILL <16 16 5 1000 0 0 0>
 LINEFILL <1 16 5 1000 0 0 0>
 SHADOW <1 0 15 15>
 THICKNESS 14
 PNTS (0,0)(3430,0)(3430,264)(0,264)(0,0)
.RECT AT (94,257)
 BACKFILL <1 16 0 10 0 0 0>
 PNTS (0,0)(680,0)(680,264)(0,264)(0,0)
.RECT AT (94,1340)
 PNTS (0,0)(680,0)(680,264)(0,264)(0,0)
.RECT AT (767,264)
 BACKFILL <1 16 0 6 0 0 0>
 THICKNESS 1
 PNTS (0,0)(2750,0)(2750,250)(0,250)(0,0)
.POL AT (434,514)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14
 PNTS (0,833)(0,0)
.RECT AT (760,1340)
 BACKFILL <1 16 0 6 0 0 0>
 PNTS (0,0)(2013,0)(2013,264)(0,264)(0,0)
.RECT AT (94,2007)
 PNTS (0,0)(680,0)(680,264)(0,264)(0,0)
.RECT AT (760,2007)
 PNTS (0,0)(2013,0)(2013,264)(0,264)(0,0)
.POL AT (351,1264)
 BACKFILL <1 16 5 1000 0 0 0>
 THICKNESS 1
 PNTS (83,83)(166,0)(0,0)(83,83)
.POL AT (1184,1931)
 PNTS (83,83)(167,0)(0,0)(83,83)
.POL AT (1267,1597)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14
 PNTS (0,0)(0,417)
.TXT AT (88,1618)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 80000 VER_JUST BASELINE MARGINS <0 0 0 0>
 TXTXYOFF <-3 80>
 T_PNTS (0,80)(0,0)(290,0)(290,82)(0,82)(0,80)(290,80)
 .STR
  "TMP5"
.TXT AT (86,2293)
 T_PNTS (0,80)(0,0)(274,0)(274,81)(0,81)(0,80)(274,80)
 .STR
  "TMP1"
.TXT AT (107,555)
 T_PNTS (0,80)(0,0)(274,0)(274,81)(0,81)(0,80)(274,80)
 .STR
  "TMP1"
.POL AT (2104,1259)
 THICKNESS 1 MARGINS <125 125 250 62>
 PNTS (0,88)(209,50)(104,0)(0,88)
.POL AT (517,267)
 BACKFILL <1 16 0 0 0 0 0>
 STYLE 1 SIZE 60000
 PNTS (0,0)(0,384)
.TXT AT (474,673)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-4 61>
 T_PNTS (0,61)(0,0)(325,0)(325,63)(0,63)(0,61)(325,61)
 .STR
  "OFFSET"
.POL AT (767,505)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14 STYLE 0 MARGINS <125 125 250 62>
 PNTS (2,92)(0,0)
.TXT AT (804,555)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 T_PNTS (0,61)(0,0)(326,0)(326,63)(0,63)(0,61)(326,61)
 .STR
  "STARTX"
.TXT AT (513,981)
 TXTXYOFF <-6 65>
 T_PNTS (0,65)(0,0)(475,0)(475,75)(0,75)(0,65)(475,65)
 .STR
  "head -c $ofs "
.POL AT (1517,264)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (2,0)(0,333)
.TXT AT (1542,558)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-7 60>
 T_PNTS (0,60)(0,0)(225,0)(225,61)(0,61)(0,60)(225,60)
 .STR
  "ENDX"
.TXT AT (1386,1912)
 TXTXYOFF <-6 60>
 T_PNTS (0,60)(0,0)(581,0)(581,62)(0,62)(0,60)(581,60)
 .STR
  "mv TMP5 TMP1"
.POL AT (4434,14)
 BACKFILL <1 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,2583)(0,0)
.TXT AT (513,887)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-3 65>
 T_PNTS (0,65)(0,0)(1623,0)(1623,83)(0,83)(0,65)(1623,65)
 .STR
  "ofs=$bytes.$blocksize.(OFFSET+STARTX) "
.RECT AT (768,253)
 BACKFILL <1 0 0 8 0 0 0>
 LINEFILL <1 2 5 1000 0 0 0>
 SHADOW <6 0 15 15>
 THICKNESS 1 SIZE 180000 VER_JUST TOP MARGINS <125 125 250 62>
 PNTS (0,0)(752,0)(752,274)(0,274)(0,0)
.LINE AT (2220,1207)
 PNTS (0,86)(153,0)
.LINE AT (2373,520)
 PNTS (0,687)(0,0)
.TXT AT (2470,953)
 BACKFILL <1 16 5 1000 0 0 0>
 LINEFILL <1 16 5 1000 0 0 0>
 SHADOW <1 0 15 15>
 THICKNESS 14 SIZE 60000 VER_JUST BASELINE MARGINS <0 0 0 0>
 TXTXYOFF <-5 65>
 T_PNTS (0,65)(0,0)(1880,0)(1880,83)(0,83)(0,65)(1880,65)
 .STR
  "len=$bytes.$blocksize.(OFFSET+ENDX-STARTX) "
.TXT AT (2465,1067)
 ITALICS ON
 TXTXYOFF <-3 65>
 T_PNTS (0,65)(0,0)(1070,0)(1070,82)(0,82)(0,65)(1070,65)
 .STR
  V_SPACE <0 83 0>
  "append "
 .STR
  ITALICS OFF
  "tail -c +[1+$ofs+$len]"
END PICTURE

*END GRAPHICS
<end_data>
<object "object3" docType:201 appType:2>
<start_data Applix>
*BEGIN GRAPHICS VERSION=430/420 ENCODING=7BIT

COLORMAP
<"Transparent" 0 0 0 0 0 1>
<"Black" 0 0 0 0 255 0>
<"Dark gray" 0 0 0 0 127 0>
<"Dark blue" 0 128 128 0 127 0>
<"Blue" 0 255 255 0 0 0>
<"Dark cyan" 0 128 0 0 127 0>
<"Cyan" 0 255 0 0 0 0>
<"Dark green" 0 128 0 128 127 0>
<"Green" 0 255 0 255 0 0>
<"Dark magenta" 0 0 128 0 127 0>
<"Magenta" 0 0 255 0 0 0>
<"Dark red" 0 0 128 128 127 0>
<"Red" 0 0 255 255 0 0>
<"Dark yellow" 0 0 0 128 127 0>
<"Yellow" 0 0 0 255 0 0>
<"Light gray" 0 0 0 0 63 0>
<"White" 0 0 0 0 0 0>
END COLORMAP

FONT
"Arial"
END FONT

SESSION
 WIN_SIZE <7500 5500>
 VIEW <0 0 0 0 0 0>
 BACKFILL <1 0 0 0 0 0 0>
 LINEFILL <1 2 5 1000 0 0 0>
 SHADOW <6 0 15 15>
 SIZE 180000 VER_JUST TOP
 REZ <0 1000 1000 0 0>
END SESSION

PICTURE
.RECT AT (109,7)
 BACKFILL <16 16 5 1000 0 0 0>
 LINEFILL <1 16 5 1000 0 0 0>
 SHADOW <1 0 15 15>
 THICKNESS 14
 PNTS (0,0)(2264,0)(2264,764)(0,764)(0,0)
.RECT AT (743,7)
 BACKFILL <15 16 5 1000 0 0 0>
 PNTS (0,0)(913,0)(913,753)(0,753)(0,0)
.POL AT (415,23)
 BACKFILL <15 16 0 0 0 0 0>
 PNTS (0,262)(5,126)(5,97)(7,86)(16,97)(16,140)(17,184)(19,225)(19,262)
      (21,283)(24,312)(24,286)(26,229)(26,187)(26,151)(26,130)(28,100)
      (29,79)(29,57)(33,46)(40,46)(47,83)(47,133)(47,184)(49,232)
      (49,276)(50,319)(52,362)(52,390)(54,420)(54,441)(57,434)(59,397)
      (62,347)(62,283)(62,232)(62,204)(64,184)(64,161)(64,126)(66,104)
      (66,90)(69,107)(71,151)(73,208)(75,258)(80,309)(83,352)(85,387)
      (87,401)(90,397)(97,369)(104,333)(109,298)(116,255)(121,211)
      (123,184)(127,161)(128,194)(128,237)(128,272)(128,300)(128,322)
      (128,354)(128,376)(128,406)(130,434)(132,454)(132,434)(132,397)
      (132,354)(132,305)(132,276)(132,241)(134,229)(137,218)(141,218)
      (149,225)(151,258)(153,300)(153,352)(153,394)(153,437)(153,474)
      (156,480)(156,451)(156,408)(158,366)(160,316)(160,272)(160,229)
      (160,201)(161,178)(163,151)(165,130)(167,107)(170,97)(172,86)
      (175,86)(182,90)(182,111)(186,154)(186,191)(186,225)(186,269)
      (186,305)(186,354)(186,383)(189,390)(193,383)(193,354)(193,326)
      (193,298)(193,269)(193,258)(193,225)(194,197)(196,175)(198,161)
      (203,154)(208,147)(215,144)(217,121)(220,144)(222,178)(222,229)
      (222,286)(222,329)(222,359)(224,354)(226,333)(227,298)(229,269)
      (229,225)(236,191)(238,154)(239,119)(243,90)(245,107)(245,151)
      (245,215)(245,272)(245,343)(245,373)(245,336)(245,295)(245,244)
      (245,187)(245,121)(245,66)(245,22)(245,0)(250,39)(250,97)(250,168)
      (250,262)(250,362)(252,420)(252,441)(252,430)(252,394)(252,352)
      (252,309)(253,251)(255,201)(257,164)(260,191)(262,241)(264,298)
      (266,354)(266,406)(267,454)(271,430)(271,387)(271,336)(271,295)
      (271,244)(271,208)(272,187)(276,229)(283,286)(285,366)(292,430)
      (292,480)(293,508)(293,468)(293,420)(293,362)(293,312)(293,283)
      (293,262)(293,248)(302,251)(304,279)(311,316)(311,336)(314,366)
      (314,376)(316,362)(316,326)(316,283)(316,241)(316,218)(325,232)
      (326,269)(333,312)(342,347)(347,376)(352,369)
.POL AT (116,14)
 PNTS (299,347)(299,236)(299,252)(299,278)(299,290)(299,262)(299,212)
      (299,179)(297,175)(292,179)(292,203)(292,236)(292,278)(292,302)
      (292,326)(292,286)(292,245)(292,212)(288,215)(288,241)(287,272)
      (287,297)(287,269)(287,227)(287,196)(283,186)(282,186)(278,186)
      (276,212)(275,245)(273,278)(273,311)(273,344)(269,344)(266,293)
      (261,245)(261,196)(261,146)(259,121)(257,109)(257,104)(256,130)
      (254,179)(252,227)(252,262)(250,224)(250,200)(250,186)(247,196)
      (247,227)(245,278)(245,311)(245,297)(243,257)(242,206)(242,167)
      (240,191)(240,241)(240,297)(240,338)(240,351)(236,314)(236,257)
      (235,200)(235,175)(231,167)(226,170)(226,212)(224,262)(224,311)
      (224,335)(223,318)(223,278)(219,227)(219,170)(219,130)(219,88)
      (219,64)(217,109)(217,158)(217,233)(217,307)(217,371)(217,422)
      (216,401)(214,359)(214,318)(212,286)(210,262)(209,227)(207,220)
      (207,272)(207,314)(207,371)(207,411)(205,389)(205,363)(203,330)
      (203,359)(203,384)(198,380)(196,347)(196,359)(195,356)(193,344)
      (190,347)(188,330)(188,344)(184,338)(183,335)(183,311)(181,252)
      (177,170)(176,88)(176,22)(174,0)(174,43)(174,109)(174,182)(174,272)
      (174,371)(174,444)(176,477)(174,453)(169,396)(165,297)(162,215)
      (162,149)(162,125)(162,170)(162,227)(162,293)(162,351)(164,392)
      (164,359)(162,318)(162,269)(160,227)(160,196)(160,236)(160,293)
      (160,344)(160,384)(158,356)(158,323)(158,297)(158,347)(158,396)
      (158,437)(158,389)(158,356)(158,380)(155,371)(155,356)(153,401)
      (153,432)(151,389)(150,330)(150,266)(150,200)(148,158)(144,109)
      (144,158)(144,224)(144,297)(144,380)(144,444)(144,495)(144,519)
      (143,488)(143,411)(141,338)(141,233)(141,134)(141,61)(141,2)
      (141,52)(141,134)(141,215)(141,307)(141,380)(141,444)(139,417)
      (137,377)(137,318)(137,278)(137,245)(137,302)(137,335)(137,359)
      (136,359)(132,326)(132,286)(132,236)(132,196)(131,227)(131,269)
      (131,318)(131,377)(131,408)(131,363)(131,323)(131,272)(131,215)
      (129,149)(125,85)(120,26)(120,2)(118,64)(117,146)(117,252)(111,351)
      (111,425)(111,483)(110,458)(110,392)(108,318)(108,227)(106,146)
      (106,64)(106,40)(106,16)(104,55)(101,121)(99,186)(99,269)(99,359)
      (99,450)(99,514)(96,453)(91,389)(91,330)(91,290)(89,323)(89,380)
      (89,453)(89,503)(89,536)(87,488)(85,422)(85,363)(85,314)(85,281)
      (85,257)(84,262)(80,302)(80,344)(78,377)(78,401)(78,380)(77,347)
      (75,307)(75,272)(75,233)(75,191)(75,142)(75,116)(75,85)(73,109)
      (70,175)(65,272)(58,389)(49,488)(39,576)(39,625)(37,597)(37,533)
      (35,467)(35,401)(35,318)(35,236)(35,179)(35,137)(37,121)(35,170)
      (33,227)(33,278)(33,335)(32,359)(30,347)(30,307)(28,266)(26,286)
      (26,318)(26,359)(26,401)(25,425)(23,401)(23,368)(23,344)(21,377)
      (21,408)(19,441)(18,441)(16,408)(16,377)(16,363)(11,368)(6,392)
      (2,417)(0,441)(0,429)
.POL AT (766,68)
 PNTS (0,349)(1,238)(1,253)(1,277)(1,290)(1,262)(1,212)(1,180)(3,175)
      (8,180)(8,205)(10,238)(10,277)(10,304)(10,328)(10,286)(10,246)
      (10,212)(12,217)(12,241)(13,274)(13,298)(13,269)(13,229)(13,196)
      (17,187)(19,187)(20,187)(26,212)(26,246)(27,277)(27,310)(27,343)
      (31,343)(36,295)(40,246)(40,196)(40,147)(41,123)(41,109)(45,106)
      (45,130)(46,180)(46,229)(46,262)(50,225)(50,201)(52,187)(53,196)
      (53,229)(55,277)(55,310)(55,298)(57,258)(59,208)(59,166)(60,192)
      (62,241)(62,298)(62,340)(62,352)(64,316)(64,258)(66,201)(67,175)
      (69,166)(74,172)(74,212)(76,262)(76,310)(76,335)(78,319)(78,277)
      (81,229)(81,172)(83,130)(83,90)(83,64)(83,109)(83,159)(83,232)
      (83,307)(83,373)(83,422)(85,401)(86,361)(86,319)(88,286)(90,262)
      (92,229)(93,220)(93,274)(93,316)(93,373)(93,413)(95,389)(95,364)
      (97,331)(97,361)(97,385)(102,382)(104,349)(104,361)(105,356)
      (107,343)(111,349)(112,331)(112,343)(116,340)(118,335)(118,310)
      (119,253)(123,172)(125,90)(125,24)(126,0)(126,43)(126,109)(126,184)
      (126,274)(126,373)(126,446)(125,479)(126,455)(132,397)(135,298)
      (138,217)(140,151)(140,126)(140,172)(138,229)(138,295)(138,352)
      (137,392)(137,361)(138,319)(138,269)(140,229)(140,196)(140,238)
      (140,295)(140,343)(140,385)(142,356)(142,324)(142,298)(142,349)
      (142,397)(142,439)(142,389)(142,356)(142,382)(145,373)(145,356)
      (147,401)(147,434)(151,389)(151,331)(151,267)(151,201)(152,159)
      (156,109)(156,159)(156,225)(156,298)(156,382)(156,446)(156,494)
      (156,521)(158,488)(158,413)(159,340)(159,232)(159,135)(159,60)
      (159,3)(159,52)(159,135)(159,217)(159,307)(159,382)(159,446)
      (161,418)(163,376)(163,319)(163,277)(165,246)(165,304)(165,335)
      (165,361)(166,361)(166,328)(168,286)(168,238)(170,196)(170,229)
      (170,269)(170,319)(170,376)(170,409)(170,364)(170,324)(170,274)
      (171,217)(171,151)(175,85)(180,27)(182,3)(182,64)(182,147)(184,253)
      (187,352)(189,427)(189,484)(191,458)(191,392)(192,319)(192,229)
      (194,147)(194,64)(194,41)(194,15)(196,57)(201,123)(201,187)
      (201,269)(201,361)(201,451)(201,515)(203,455)(210,389)(210,331)
      (210,290)(211,324)(211,382)(211,455)(211,503)(211,536)(213,488)
      (215,422)(215,364)(215,316)(215,283)(215,258)(218,262)(220,304)
      (220,343)(222,376)(222,401)(224,382)(224,349)(225,307)(225,274)
      (225,232)(225,192)(225,144)(225,118)(225,85)(227,109)(230,175)
      (236,274)(243,389)(251,488)(262,578)(262,626)(263,599)(263,533)
      (265,467)(265,401)(265,319)(265,238)(265,180)(265,139)(265,123)
      (265,172)(267,229)(267,277)(267,335)(269,361)(270,349)(270,307)
      (272,267)(274,286)(274,319)(276,361)(276,401)(276,427)(277,401)
      (277,369)(277,343)(279,376)(281,409)(281,442)(284,442)(284,409)
      (284,376)(284,364)(290,369)(295,392)(298,418)(300,442)(302,430)
.POL AT (1016,186)
 PNTS (350,246)(347,382)(347,411)(345,422)(336,411)(336,368)(335,324)
      (335,281)(333,246)(331,225)(329,196)(328,220)(326,279)(326,321)
      (326,357)(326,378)(324,408)(324,429)(322,449)(319,460)(312,460)
      (305,425)(305,375)(305,324)(303,276)(303,231)(302,189)(300,146)
      (300,118)(298,88)(298,66)(295,74)(293,109)(290,159)(290,225)
      (290,276)(290,304)(288,324)(288,347)(288,382)(286,404)(286,418)
      (283,401)(281,357)(279,298)(277,250)(272,199)(269,156)(267,121)
      (265,106)(262,109)(255,139)(248,175)(243,210)(236,253)(230,297)
      (227,324)(225,347)(224,314)(224,271)(224,236)(224,208)(224,186)
      (224,153)(222,132)(222,102)(222,74)(220,54)(220,74)(220,109)
      (220,153)(220,203)(220,231)(220,267)(218,279)(215,290)(211,290)
      (204,281)(201,250)(199,208)(199,156)(199,114)(199,71)(199,34)
      (196,28)(196,57)(196,99)(194,142)(194,192)(194,236)(192,279)
      (192,307)(191,328)(189,357)(187,378)(185,401)(182,411)(180,422)
      (177,422)(170,418)(170,397)(168,354)(168,317)(166,281)(166,239)
      (166,203)(166,153)(166,125)(163,118)(159,125)(159,153)(159,180)
      (159,210)(159,239)(159,250)(159,281)(158,311)(156,331)(154,347)
      (149,354)(144,361)(137,364)(135,385)(132,364)(132,328)(132,279)
      (132,220)(132,179)(132,149)(128,153)(126,175)(126,210)(123,239)
      (123,281)(116,317)(114,354)(112,389)(109,418)(107,401)(107,357)
      (107,293)(107,236)(107,165)(107,135)(107,172)(107,213)(107,264)
      (107,321)(107,385)(107,442)(107,486)(107,507)(102,468)(102,411)
      (102,338)(102,246)(102,146)(100,88)(100,66)(100,78)(100,114)
      (100,156)(100,199)(99,257)(97,307)(95,342)(92,317)(90,267)(88,210)
      (86,153)(86,102)(85,54)(81,78)(81,121)(81,172)(81,213)(81,264)
      (81,298)(79,321)(76,279)(69,220)(67,142)(60,78)(60,28)(59,0)
      (59,38)(59,88)(59,146)(57,196)(57,225)(57,246)(57,260)(50,257)
      (48,227)(41,192)(40,172)(40,142)(38,132)(36,146)(36,180)(36,225)
      (36,267)(36,290)(27,276)(26,239)(19,196)(10,159)(5,132)(0,139)
.POL AT (1349,139)
 PNTS (0,349)(0,238)(2,253)(2,279)(2,290)(2,262)(2,213)(2,180)(3,175)
      (7,180)(9,205)(9,238)(9,279)(9,302)(9,328)(9,286)(9,246)(9,213)
      (12,215)(12,241)(14,274)(14,298)(14,269)(14,227)(14,196)(16,187)
      (19,187)(21,187)(24,213)(26,246)(26,279)(28,311)(28,344)(31,344)
      (35,293)(40,246)(40,196)(40,146)(42,121)(42,109)(43,106)(45,130)
      (47,180)(47,227)(47,262)(49,226)(50,201)(50,187)(52,196)(54,227)
      (54,279)(54,311)(55,298)(57,259)(59,208)(59,167)(61,193)(61,241)
      (61,298)(61,340)(61,352)(64,314)(64,259)(66,201)(66,175)(69,167)
      (75,172)(75,213)(75,262)(75,311)(75,335)(76,319)(78,279)(82,227)
      (82,172)(82,130)(82,88)(82,64)(83,109)(83,160)(83,233)(83,307)
      (83,373)(83,422)(85,401)(85,361)(87,319)(88,286)(90,262)(92,227)
      (94,220)(94,274)(94,314)(94,373)(94,413)(94,389)(95,364)(95,331)
      (97,361)(97,385)(101,380)(104,349)(104,361)(106,356)(106,344)
      (111,349)(113,331)(113,344)(114,340)(116,335)(118,311)(120,253)
      (123,172)(125,88)(125,24)(125,0)(125,45)(125,109)(125,184)(125,274)
      (125,373)(125,446)(125,479)(125,455)(130,397)(135,298)(139,215)
      (139,151)(139,127)(139,172)(139,227)(139,293)(137,352)(137,392)
      (137,361)(137,319)(139,269)(141,227)(141,196)(141,238)(141,293)
      (141,344)(141,385)(141,356)(142,323)(142,298)(142,349)(142,397)
      (142,439)(142,389)(142,356)(142,380)(146,373)(146,356)(147,401)
      (147,434)(149,389)(151,331)(151,267)(151,201)(151,160)(156,109)
      (156,160)(156,226)(156,298)(156,380)(156,446)(156,495)(156,521)
      (158,488)(158,413)(160,340)(160,233)(160,134)(160,61)(160,2)
      (160,52)(160,134)(160,215)(160,307)(160,380)(160,446)(161,418)
      (161,377)(161,319)(163,279)(163,246)(163,302)(163,335)(163,361)
      (165,361)(167,328)(168,286)(168,238)(168,196)(170,227)(170,269)
      (170,319)(170,377)(170,410)(170,364)(170,323)(170,274)(170,215)
      (172,151)(175,85)(180,28)(180,2)(182,64)(182,146)(184,253)(187,352)
      (187,427)(189,483)(189,458)(191,392)(193,319)(193,227)(193,146)
      (193,64)(193,40)(194,16)(196,55)(200,121)(201,187)(201,269)
      (201,361)(201,451)(201,515)(203,455)(208,389)(210,331)(210,290)
      (210,323)(212,380)(212,455)(212,503)(212,536)(213,488)(215,422)
      (215,364)(215,314)(215,281)(215,259)(217,262)(219,302)(220,344)
      (220,377)(220,401)(222,380)(224,349)(224,307)(224,274)(226,233)
      (226,193)(226,142)(226,120)(226,85)(227,109)(231,175)(236,274)
      (243,389)(252,488)(260,578)(262,627)(262,599)(264,533)(266,467)
      (266,401)(266,319)(266,238)(266,180)(266,139)(264,121)(266,172)
      (266,227)(267,279)(267,335)(267,361)(271,349)(271,307)(271,267)
      (274,286)(274,319)(274,361)(274,401)(276,427)(276,401)(278,368)
      (278,344)(279,377)(279,410)(281,443)(283,443)(283,410)(283,377)
      (283,364)(288,368)(295,392)(299,418)(300,443)(300,430)
.POL AT (1616,156)
 PNTS (299,349)(299,236)(299,252)(299,278)(299,290)(299,262)(299,212)
      (299,179)(297,176)(292,179)(292,203)(292,236)(292,278)(292,302)
      (292,327)(292,287)(292,245)(292,212)(288,217)(288,242)(287,273)
      (287,297)(287,269)(287,228)(287,196)(283,188)(282,188)(278,188)
      (276,212)(275,245)(273,278)(273,311)(273,344)(269,344)(266,294)
      (261,245)(261,196)(261,146)(259,124)(257,110)(257,104)(255,130)
      (254,179)(252,228)(252,262)(250,224)(250,200)(250,188)(247,196)
      (247,228)(245,278)(245,311)(245,297)(243,257)(242,209)(242,167)
      (240,191)(240,242)(240,297)(240,339)(240,351)(236,314)(236,257)
      (235,200)(235,176)(231,167)(226,170)(226,212)(224,262)(224,311)
      (224,335)(222,318)(222,278)(219,228)(219,170)(219,130)(219,91)
      (219,64)(217,110)(217,158)(217,231)(217,306)(217,372)(217,422)
      (216,401)(214,360)(214,318)(212,287)(210,262)(209,228)(207,221)
      (207,273)(207,314)(207,372)(207,412)(205,389)(205,363)(203,330)
      (203,360)(203,384)(198,380)(196,349)(196,360)(195,356)(193,344)
      (189,349)(188,330)(188,344)(184,339)(183,335)(183,311)(181,252)
      (177,170)(176,91)(176,23)(174,0)(174,44)(174,110)(174,184)(174,273)
      (174,372)(174,445)(176,478)(174,453)(169,396)(165,297)(162,217)
      (162,151)(162,125)(162,170)(162,228)(162,294)(162,351)(163,391)
      (163,360)(162,318)(162,269)(160,228)(160,196)(160,236)(160,294)
      (160,344)(160,384)(158,356)(158,323)(158,297)(158,349)(158,396)
      (158,438)(158,389)(158,356)(158,380)(155,372)(155,356)(153,401)
      (153,434)(151,389)(150,330)(150,266)(150,200)(148,158)(144,110)
      (144,158)(144,224)(144,297)(144,380)(144,445)(144,495)(144,519)
      (143,486)(143,412)(141,339)(141,231)(141,134)(141,61)(141,4)
      (141,51)(141,134)(141,217)(141,306)(141,380)(141,445)(139,417)
      (137,377)(137,318)(137,278)(137,245)(137,302)(137,335)(137,360)
      (136,360)(132,327)(132,287)(132,236)(132,196)(130,228)(130,269)
      (130,318)(130,377)(130,410)(130,363)(130,323)(130,273)(130,217)
      (129,151)(125,84)(120,28)(120,4)(118,64)(117,146)(117,252)(111,351)
      (111,424)(111,483)(110,459)(110,391)(108,318)(108,228)(106,146)
      (106,64)(106,40)(106,16)(104,56)(101,124)(99,188)(99,269)(99,360)
      (99,450)(99,516)(96,453)(91,389)(91,330)(91,290)(89,323)(89,380)
      (89,453)(89,504)(89,537)(87,486)(85,422)(85,363)(85,314)(85,283)
      (85,257)(84,262)(80,302)(80,344)(78,377)(78,401)(78,380)(77,349)
      (75,306)(75,273)(75,231)(75,191)(75,143)(75,118)(75,84)(73,110)
      (70,176)(64,273)(58,389)(49,486)(38,577)(38,625)(37,597)(37,533)
      (35,467)(35,401)(35,318)(35,236)(35,179)(35,137)(37,124)(35,170)
      (33,228)(33,278)(33,335)(32,360)(30,349)(30,306)(28,266)(26,287)
      (26,318)(26,360)(26,401)(25,424)(23,401)(23,368)(23,344)(21,377)
      (21,410)(19,443)(18,443)(16,410)(16,377)(16,363)(11,368)(5,391)
      (2,417)(0,443)(0,431)
.POL AT (1930,184)
 PNTS (436,248)(431,384)(431,413)(431,425)(421,413)(417,372)(417,326)
      (415,283)(415,248)(412,227)(408,196)(408,222)(407,281)(407,323)
      (405,359)(405,382)(405,410)(403,431)(403,453)(398,464)(389,464)
      (381,427)(379,378)(379,326)(379,278)(377,233)(377,191)(375,148)
      (372,120)(372,89)(370,68)(367,76)(363,111)(362,161)(362,227)
      (360,278)(360,306)(360,326)(358,349)(358,384)(355,406)(355,420)
      (353,403)(349,359)(349,302)(348,250)(339,201)(336,158)(334,122)
      (330,108)(325,111)(316,141)(310,175)(303,212)(296,255)(287,299)
      (285,326)(282,349)(278,316)(278,273)(278,238)(278,208)(278,188)
      (278,155)(278,132)(278,104)(277,76)(273,54)(273,76)(273,111)
      (273,155)(273,205)(273,233)(273,269)(271,281)(268,292)(263,292)
      (254,283)(250,250)(250,208)(250,158)(250,116)(250,73)(250,36)
      (244,30)(244,57)(244,101)(244,144)(242,194)(242,238)(240,281)
      (240,309)(240,330)(237,359)(233,382)(231,403)(226,413)(224,425)
      (221,425)(214,420)(212,399)(209,356)(209,319)(207,283)(207,240)
      (207,205)(207,155)(207,127)(205,120)(200,127)(198,155)(198,184)
      (198,212)(198,240)(198,250)(198,283)(198,313)(195,335)(191,349)
      (188,356)(179,363)(172,366)(169,387)(167,366)(164,330)(164,281)
      (164,222)(164,179)(164,151)(160,155)(160,175)(158,212)(155,240)
      (153,283)(146,319)(143,356)(141,392)(136,420)(134,403)(134,359)
      (134,295)(134,238)(134,165)(134,137)(134,174)(134,217)(134,266)
      (134,323)(134,387)(134,444)(134,488)(134,510)(131,470)(127,413)
      (127,340)(127,248)(127,148)(127,89)(127,68)(127,78)(127,116)
      (125,158)(125,201)(124,260)(122,309)(122,345)(115,319)(113,269)
      (110,212)(110,155)(110,104)(108,54)(105,78)(105,122)(105,174)
      (101,217)(101,266)(101,302)(101,323)(96,281)(89,222)(86,144)
      (79,78)(79,30)(77,0)(77,40)(77,89)(77,148)(73,196)(73,227)(73,248)
      (73,262)(65,260)(63,229)(54,194)(53,174)(51,144)(51,132)(46,148)
      (46,184)(46,227)(46,269)(46,292)(37,278)(33,240)(27,196)(16,161)
      (7,132)(0,141)
.POL AT (116,764)
 PNTS (0,0)(0,83)
.POL AT (2366,764)
 PNTS (0,0)(0,83)
.TXT AT (5,791)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 80000 VER_JUST BASELINE MARGINS <0 0 0 0>
 TXTXYOFF <-5 80>
 T_PNTS (0,80)(0,0)(52,0)(52,82)(0,82)(0,80)(52,80)
 .STR
  "0"
.TXT AT (2372,850)
 TXTXYOFF <-6 81>
 T_PNTS (0,81)(0,0)(868,0)(868,104)(0,104)(0,81)(868,81)
 .STR
  "CANVAS_WIDTH"
.TXT AT (828,808)
 HOR_JUST RIGHT
 TXTXYOFF <-8 81>
 T_PNTS (0,81)(0,0)(537,0)(537,104)(0,104)(0,81)(537,81)
 .STR
  "Mark_Start"
.POL AT (783,764)
 BACKFILL <15 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(0,83)
.POL AT (1533,764)
 PNTS (0,0)(0,83)
.RECT AT (1193,1590)
 BACKFILL <16 16 5 1000 0 0 0>
 PNTS (0,0)(2263,0)(2263,764)(0,764)(0,0)
.POL AT (1200,2347)
 BACKFILL <16 16 0 0 0 0 0>
 PNTS (0,0)(0,83)
.POL AT (3450,2347)
 PNTS (0,0)(0,83)
.TXT AT (1095,2409)
 BACKFILL <1 16 5 1000 0 0 0>
 HOR_JUST LEFT MARGINS <0 0 0 0>
 TXTXYOFF <-5 80>
 T_PNTS (0,80)(0,0)(52,0)(52,82)(0,82)(0,80)(52,80)
 .STR
  "0"
.TXT AT (3456,2516)
 TXTXYOFF <-6 81>
 T_PNTS (0,81)(0,0)(868,0)(868,104)(0,104)(0,81)(868,81)
 .STR
  "CANVAS_WIDTH"
.POL AT (783,847)
 BACKFILL <16 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(417,750)
.POL AT (1033,1347)
 BACKFILL <1 16 5 1000 0 0 0>
 THICKNESS 1
 PNTS (167,250)(83,0)(0,84)(167,250)
.POL AT (1533,847)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14
 PNTS (0,0)(1917,750)
.POL AT (3116,1431)
 BACKFILL <1 16 5 1000 0 0 0>
 THICKNESS 1
 PNTS (334,166)(84,0)(0,83)(334,166)
.TXT AT (1690,818)
 MARGINS <0 0 0 0>
 TXTXYOFF <-8 80>
 T_PNTS (0,80)(0,0)(493,0)(493,103)(0,103)(0,80)(493,80)
 .STR
  "Mark_End"
.TXT AT (1303,1143)
 SIZE 120000 ITALICS ON
 TXTXYOFF <-4 121>
 T_PNTS (0,121)(0,0)(504,0)(504,124)(0,124)(0,121)(504,121)
 .STR
  "ZOOM"
.POL AT (1207,1609)
 BACKFILL <1 16 0 0 0 0 0>
 THICKNESS 14 MARGINS <125 125 250 62>
 PNTS (0,353)(3,240)(3,256)(3,280)(3,292)(3,264)(3,214)(3,183)(8,177)
      (20,183)(20,207)(26,240)(26,280)(26,306)(26,330)(26,289)(26,249)
      (26,214)(29,219)(29,243)(34,276)(34,301)(34,271)(34,231)(34,198)
      (43,190)(46,190)(52,190)(64,214)(64,249)(69,280)(69,313)(69,346)
      (78,346)(92,297)(100,249)(100,198)(100,148)(104,124)(104,110)
      (112,106)(112,131)(118,183)(118,231)(118,264)(126,228)(126,203)
      (130,190)(135,198)(135,231)(138,280)(138,313)(138,301)(144,261)
      (147,210)(147,167)(152,195)(158,243)(158,301)(158,342)(158,356)
      (161,318)(161,261)(166,203)(170,177)(175,167)(187,172)(187,214)
      (192,264)(192,313)(192,337)(196,321)(196,280)(204,231)(204,172)
      (210,131)(210,91)(210,65)(210,110)(210,160)(210,235)(210,309)
      (210,377)(210,426)(213,405)(218,365)(218,321)(222,289)(227,264)
      (232,231)(236,223)(236,276)(236,318)(236,377)(236,417)(241,393)
      (241,368)(244,334)(244,365)(244,389)(258,386)(262,353)(262,365)
      (267,360)(270,346)(279,353)(284,334)(284,346)(293,342)(296,337)
      (296,313)(302,256)(310,172)(316,91)(316,25)(319,0)(319,44)(319,110)
      (319,186)(319,276)(319,377)(319,450)(316,483)(319,459)(333,401)
      (342,301)(350,219)(354,151)(354,127)(354,172)(350,231)(350,297)
      (350,356)(345,396)(345,365)(350,321)(350,271)(354,231)(354,198)
      (354,240)(354,297)(354,346)(354,389)(359,360)(359,327)(359,301)
      (359,353)(359,401)(359,443)(359,393)(359,360)(359,386)(368,377)
      (368,360)(371,405)(371,438)(380,393)(380,334)(380,269)(380,203)
      (385,160)(394,110)(394,160)(394,228)(394,301)(394,386)(394,450)
      (394,499)(394,525)(399,492)(399,417)(402,342)(402,235)(402,136)
      (402,61)(402,4)(402,52)(402,136)(402,219)(402,309)(402,386)
      (402,450)(408,422)(411,381)(411,321)(411,280)(416,249)(416,306)
      (416,337)(416,365)(420,365)(420,330)(425,289)(425,240)(428,198)
      (428,231)(428,271)(428,321)(428,381)(428,413)(428,368)(428,327)
      (428,276)(434,219)(434,151)(442,85)(454,28)(460,4)(460,65)(460,148)
      (465,256)(473,356)(477,431)(477,488)(482,462)(482,396)(486,321)
      (486,231)(491,148)(491,65)(491,42)(491,16)(494,58)(508,124)
      (508,190)(508,271)(508,365)(508,455)(508,519)(512,459)(529,393)
      (529,334)(529,292)(534,327)(534,386)(534,459)(534,507)(534,542)
      (538,492)(543,426)(543,368)(543,318)(543,285)(543,261)(552,264)
      (557,306)(557,346)(560,381)(560,405)(566,386)(566,353)(569,309)
      (569,276)(569,235)(569,195)(569,144)(569,118)(569,85)(574,110)
      (583,177)(595,276)(612,393)(635,492)(661,584)(661,632)(666,604)
      (666,538)(670,471)(670,405)(670,321)(670,240)(670,183)(670,139)
      (670,124)(670,172)(675,231)(675,280)(675,337)(678,365)(684,353)
      (684,309)(687,269)(692,289)(692,321)(697,365)(697,405)(697,431)
      (701,405)(701,374)(701,346)(706,381)(710,413)(710,446)(718,446)
      (718,413)(718,381)(718,368)(732,374)(744,396)(753,422)(758,446)
      (762,434)
.POL AT (1838,1727)
 PNTS (886,250)(877,386)(877,417)(874,427)(851,417)(851,372)(848,328)
      (848,285)(842,250)(839,228)(834,198)(829,223)(825,283)(825,325)
      (825,361)(825,382)(820,414)(820,434)(816,455)(808,466)(790,466)
      (773,431)(773,379)(773,328)(768,280)(768,235)(764,191)(759,148)
      (759,120)(754,91)(754,68)(745,77)(742,111)(733,162)(733,228)
      (733,280)(733,308)(728,328)(728,351)(728,386)(724,408)(724,424)
      (716,405)(711,361)(707,302)(702,254)(690,202)(681,158)(676,124)
      (671,108)(662,111)(645,141)(627,177)(615,212)(596,257)(584,301)
      (575,328)(570,351)(566,318)(566,275)(566,240)(566,210)(566,188)
      (566,155)(561,134)(561,105)(561,77)(558,54)(558,77)(558,111)
      (558,155)(558,205)(558,235)(558,271)(553,283)(544,294)(535,294)
      (518,285)(509,254)(504,210)(504,158)(504,117)(504,73)(504,35)
      (495,28)(495,59)(495,101)(492,144)(492,195)(492,240)(487,283)
      (487,311)(483,332)(478,361)(474,382)(469,405)(461,417)(457,427)
      (447,427)(429,424)(429,401)(426,358)(426,322)(421,285)(421,243)
      (421,205)(421,155)(421,127)(412,120)(403,127)(403,155)(403,183)
      (403,212)(403,243)(403,254)(403,285)(400,315)(395,335)(391,351)
      (377,358)(363,365)(346,368)(342,389)(334,368)(334,332)(334,283)
      (334,223)(334,181)(334,151)(325,155)(320,177)(320,212)(311,243)
      (311,285)(294,322)(289,358)(285,393)(277,424)(271,405)(271,361)
      (271,297)(271,240)(271,167)(271,138)(271,174)(271,216)(271,268)
      (271,325)(271,389)(271,448)(271,492)(271,513)(259,474)(259,417)
      (259,342)(259,250)(259,148)(254,91)(254,68)(254,80)(254,117)
      (254,158)(254,202)(250,261)(245,311)(242,346)(233,322)(228,271)
      (224,212)(219,155)(219,105)(214,54)(205,80)(205,124)(205,174)
      (205,216)(205,268)(205,302)(202,325)(193,283)(176,223)(171,144)
      (153,80)(153,28)(150,0)(150,39)(150,91)(150,148)(145,198)(145,228)
      (145,250)(145,264)(127,261)(122,230)(105,195)(101,174)(101,144)
      (96,134)(92,148)(92,183)(92,228)(92,271)(92,294)(70,280)(66,243)
      (47,198)(27,162)(13,134)(0,141)
.POL AT (2680,1680)
 PNTS (0,353)(0,240)(6,256)(6,283)(6,294)(6,264)(6,216)(6,183)(9,178)
      (18,183)(23,207)(23,240)(23,283)(23,306)(23,332)(23,290)(23,249)
      (23,216)(32,218)(32,244)(35,277)(35,303)(35,271)(35,230)(35,198)
      (40,190)(49,190)(53,190)(61,216)(66,249)(66,283)(72,315)(72,348)
      (80,348)(89,297)(101,249)(101,198)(101,148)(106,124)(106,112)
      (110,108)(115,132)(119,183)(119,230)(119,264)(124,228)(127,204)
      (127,190)(132,198)(136,230)(136,283)(136,315)(141,303)(145,261)
      (150,211)(150,169)(155,195)(155,244)(155,303)(155,344)(155,356)
      (164,318)(164,261)(167,204)(167,178)(176,169)(190,174)(190,216)
      (190,264)(190,315)(190,339)(193,323)(198,283)(207,230)(207,174)
      (207,132)(207,89)(207,65)(211,112)(211,162)(211,235)(211,311)
      (211,377)(211,426)(216,405)(216,365)(219,323)(224,290)(230,264)
      (233,230)(238,223)(238,277)(238,318)(238,377)(238,417)(238,393)
      (242,369)(242,336)(247,365)(247,389)(256,384)(264,353)(264,365)
      (268,360)(268,348)(282,353)(285,336)(285,348)(290,344)(294,339)
      (299,315)(304,256)(313,174)(316,89)(316,25)(316,0)(316,46)(316,112)
      (316,186)(316,277)(316,377)(316,450)(316,485)(316,461)(330,402)
      (342,303)(351,218)(351,153)(351,129)(351,174)(351,230)(351,297)
      (348,356)(348,396)(348,365)(348,323)(351,271)(356,230)(356,198)
      (356,240)(356,297)(356,348)(356,389)(356,360)(360,327)(360,303)
      (360,353)(360,402)(360,443)(360,393)(360,360)(360,384)(369,377)
      (369,360)(374,405)(374,438)(379,393)(382,336)(382,270)(382,204)
      (382,162)(396,112)(396,162)(396,228)(396,303)(396,384)(396,450)
      (396,500)(396,527)(400,494)(400,417)(405,344)(405,235)(405,136)
      (405,61)(405,2)(405,53)(405,136)(405,218)(405,311)(405,384)
      (405,450)(408,422)(408,381)(408,323)(414,283)(414,249)(414,306)
      (414,339)(414,365)(417,365)(422,332)(426,290)(426,240)(426,198)
      (431,230)(431,271)(431,323)(431,381)(431,414)(431,369)(431,327)
      (431,277)(431,218)(434,153)(443,86)(457,28)(457,2)(462,65)(462,148)
      (466,256)(474,356)(474,431)(480,488)(480,464)(483,396)(488,323)
      (488,230)(488,148)(488,65)(488,40)(492,16)(497,56)(506,124)
      (509,190)(509,271)(509,365)(509,455)(509,521)(514,461)(526,393)
      (532,336)(532,294)(532,327)(537,384)(537,461)(537,509)(537,542)
      (540,494)(546,426)(546,369)(546,318)(546,285)(546,261)(549,264)
      (554,306)(558,348)(558,381)(558,405)(563,384)(566,353)(566,311)
      (566,277)(572,235)(572,195)(572,145)(572,122)(572,86)(575,112)
      (584,178)(598,277)(615,393)(638,494)(658,584)(664,634)(664,605)
      (667,539)(672,473)(672,405)(672,323)(672,240)(672,183)(672,141)
      (667,124)(672,174)(672,230)(676,283)(676,339)(676,365)(684,353)
      (684,311)(684,270)(695,290)(695,323)(695,365)(695,405)(698,431)
      (698,405)(704,372)(704,348)(707,381)(707,414)(712,447)(716,447)
      (716,414)(716,381)(716,369)(730,372)(747,396)(756,422)(759,447)
      (759,435)
.POL AT (116,847)
 THICKNESS 1 STYLE 2
 PNTS (0,0)(0,1583)
.POL AT (783,764)
 PNTS (0,1632)(0,0)
.TXT AT (138,2376)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 80000 ITALICS OFF MARGINS <0 0 0 0>
 TXTXYOFF <-6 81>
 T_PNTS (0,81)(0,0)(619,0)(619,83)(0,83)(0,81)(619,81)
 .STR
  "< OFFSET >"
.TXT AT (179,2558)
 TXTXYOFF <-5 81>
 T_PNTS (0,81)(0,0)(1709,0)(1709,104)(0,104)(0,81)(1709,81)
 .STR
  "OFFSET=BLOCKSIZE*Mark_Start"
END PICTURE

*END GRAPHICS
<end_data>
<object "object2" docType:201 appType:2>
<start_data Applix>
*BEGIN GRAPHICS VERSION=430/420 ENCODING=7BIT

COLORMAP
<"Transparent" 0 0 0 0 0 1>
<"Black" 0 0 0 0 255 0>
<"Dark gray" 0 0 0 0 127 0>
<"Dark blue" 0 128 128 0 127 0>
<"Blue" 0 255 255 0 0 0>
<"Dark cyan" 0 128 0 0 127 0>
<"Cyan" 0 255 0 0 0 0>
<"Dark green" 0 128 0 128 127 0>
<"Green" 0 255 0 255 0 0>
<"Dark magenta" 0 0 128 0 127 0>
<"Magenta" 0 0 255 0 0 0>
<"Dark red" 0 0 128 128 127 0>
<"Red" 0 0 255 255 0 0>
<"Dark yellow" 0 0 0 128 127 0>
<"Yellow" 0 0 0 255 0 0>
<"Light gray" 0 0 0 0 63 0>
<"White" 0 0 0 0 0 0>
END COLORMAP

FONT
"Arial"
END FONT

SESSION
 WIN_SIZE <7500 5500>
 VIEW <0 0 0 0 0 0>
 BACKFILL <1 0 0 0 0 0 0>
 LINEFILL <1 2 5 1000 0 0 0>
 SHADOW <6 0 15 15>
 SIZE 180000 VER_JUST TOP
 REZ <0 1000 1000 0 0>
END SESSION

PICTURE
.POL AT (333,208)
 BACKFILL <1 16 0 0 0 0 0>
 LINEFILL <1 16 5 1000 0 0 0>
 SHADOW <1 0 15 15>
 THICKNESS 14
 PNTS (0,0)(0,1000)
.POL AT (333,208)
 PNTS (0,0)(1334,0)
.POL AT (1500,208)
 BACKFILL <1 16 5 1000 0 0 0>
 THICKNESS 1
 PNTS (167,0)(0,84)(0,0)(167,0)
.POL AT (1500,125)
 PNTS (167,83)(0,0)(0,83)(167,83)
.POL AT (250,1125)
 PNTS (83,167)(0,0)(83,0)(83,167)
.POL AT (333,1125)
 PNTS (0,167)(84,0)(0,0)(0,167)
.TXT AT (173,25)
 SIZE 100000 VER_JUST BASELINE MARGINS <0 0 0 0>
 TXTXYOFF <-6 100>
 T_PNTS (0,100)(0,0)(182,0)(182,121)(0,121)(0,100)(182,100)
 .STR
  "0,0"
.TXT AT (6,1025)
 T_PNTS (0,100)(0,0)(179,0)(179,121)(0,121)(0,100)(179,100)
 .STR
  "0,n"
.TXT AT (1259,25)
 TXTXYOFF <-9 100>
 T_PNTS (0,100)(0,0)(179,0)(179,121)(0,121)(0,100)(179,100)
 .STR
  "n,0"
.TXT AT (1168,359)
 TXTXYOFF <-1 99>
 T_PNTS (0,99)(0,0)(730,0)(730,129)(0,129)(0,99)(730,99)
 .STR
  "x increasing"
.TXT AT (419,943)
 TXTXYOFF <-2 99>
 T_PNTS (0,99)(0,0)(775,0)(775,129)(0,129)(0,99)(775,99)
 .STR
  "y decreasing"
END PICTURE

*END GRAPHICS
<end_data>
<object "object1" docType:201 appType:2>
<start_data Applix>
*BEGIN GRAPHICS VERSION=430/420 ENCODING=7BIT

COLORMAP
<"Transparent" 0 0 0 0 0 1>
<"Black" 0 0 0 0 255 0>
<"Dark gray" 0 0 0 0 127 0>
<"Dark blue" 0 128 128 0 127 0>
<"Blue" 0 255 255 0 0 0>
<"Dark cyan" 0 128 0 0 127 0>
<"Cyan" 0 255 0 0 0 0>
<"Dark green" 0 128 0 128 127 0>
<"Green" 0 255 0 255 0 0>
<"Dark magenta" 0 0 128 0 127 0>
<"Magenta" 0 0 255 0 0 0>
<"Dark red" 0 0 128 128 127 0>
<"Red" 0 0 255 255 0 0>
<"Dark yellow" 0 0 0 128 127 0>
<"Yellow" 0 0 0 255 0 0>
<"Light gray" 0 0 0 0 63 0>
<"White" 0 0 0 0 0 0>
END COLORMAP

FONT
"Arial"
END FONT

SESSION
 WIN_SIZE <7502 5502>
 VIEW <0 0 0 0 0 0>
 L_ARROW 1
 REZ <0 1000 1000 0 0>
END SESSION

PICTURE
.POL AT (2422,2189)
 BACKFILL <1 16 0 0 0 0 0>
 LINEFILL <1 16 5 1000 0 0 0>
 SHADOW <1 0 15 15>
 STYLE 2 R_ARROW 1
 PNTS (0,0)(167,834)
.POL AT (589,606)
 THICKNESS 14 STYLE 0 L_ARROW 0 R_ARROW 0
 PNTS (0,583)(0,0)
.RECT AT (7,66)
 BACKFILL <16 16 5 1000 0 0 0>
 PNTS (0,0)(1180,0)(1180,597)(0,597)(0,0)
.RECT AT (1582,1099)
 XYRAD <250 250>
 PNTS (0,0)(1595,0)(1595,1095)(0,1095)(0,0)
.POL AT (1887,14)
 BACKFILL <16 16 0 0 0 0 0>
 PNTS (0,0)(1083,0)
.POL AT (1887,347)
 PNTS (0,0)(1083,0)
.POL AT (2422,356)
 L_ARROW 1 R_ARROW 1
 PNTS (0,0)(0,750)
.RECT AT (5224,182)
 BACKFILL <16 16 5 1000 0 0 0>
 L_ARROW 0 R_ARROW 0
 XYRAD <0 0>
 PNTS (0,0)(1180,0)(1180,598)(0,598)(0,0)
.TXT AT (1929,88)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 100000 VER_JUST BASELINE MARGINS <0 0 0 0>
 TXTXYOFF <-7 101>
 T_PNTS (0,101)(0,0)(960,0)(960,104)(0,104)(0,101)(960,101)
 .STR
  "GLOBAL DATA"
.TXT AT (1928,1255)
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(824,0)(824,104)(0,104)(0,101)(824,101)
 .STR
  "Sound Studio"
.TXT AT (2098,1547)
 SIZE 140000 BOLD ON
 TXTXYOFF <-9 142>
 T_PNTS (0,142)(0,0)(324,0)(324,145)(0,145)(0,142)(324,142)
 .STR
  "GUI"
.TXT AT (5320,338)
 SIZE 100000 BOLD OFF
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(885,0)(885,104)(0,104)(0,101)(885,101)
 .STR
  "SAMPLE FILE"
.POL AT (5363,2097)
 BACKFILL <16 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(1083,0)
.POL AT (5363,2431)
 PNTS (0,0)(1083,0)
.POL AT (3172,1404)
 THICKNESS 1 STYLE 2 R_ARROW 1
 PNTS (0,202)(982,0)
.RECT AT (3974,2599)
 BACKFILL <16 16 5 1000 0 0 0>
 THICKNESS 14 STYLE 0 R_ARROW 0
 PNTS (0,0)(1180,0)(1180,597)(0,597)(0,0)
.TXT AT (4320,2838)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 T_PNTS (0,101)(0,0)(396,0)(396,104)(0,104)(0,101)(396,101)
 .STR
  "Stdout"
.ELL AT (4141,849)
 BACKFILL <16 16 5 1000 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(1180,0)(1180,1180)(0,1180)(0,0)
.TXT AT (4487,1005)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 T_PNTS (0,101)(0,0)(461,0)(461,130)(0,130)(0,101)(461,101)
 .STR
  "Sample"
.TXT AT (4408,1174)
 TXTXYOFF <-10 99>
 T_PNTS (0,99)(0,0)(769,0)(769,128)(0,128)(0,99)(769,99)
 .STR
  "Manipulation"
.TXT AT (4398,1340)
 TXTXYOFF <0 99>
 T_PNTS (0,99)(0,0)(744,0)(744,128)(0,128)(0,99)(744,99)
 .STR
  "Applications"
.TXT AT (4324,1588)
 ITALICS ON
 TXTXYOFF <-10 101>
 T_PNTS (0,101)(0,0)(790,0)(790,122)(0,122)(0,101)(790,101)
 .STR
  "Sox, maxmin"
.TXT AT (3218,1113)
 SIZE 80000 ITALICS OFF
 TXTXYOFF <-4 81>
 T_PNTS (0,81)(0,0)(877,0)(877,104)(0,104)(0,81)(877,81)
 .STR
  "effect/file_request"
.POL AT (4731,523)
 BACKFILL <16 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (167,0)(0,333)
.POL AT (4898,523)
 PNTS (0,0)(333,0)
.POL AT (4814,571)
 PNTS (7,57)(0,0)
.POL AT (4830,613)
 PNTS (0,40)(73,0)
.POL AT (5231,1773)
 L_ARROW 1
 PNTS (0,0)(500,166)
.POL AT (5731,1939)
 L_ARROW 0 R_ARROW 1
 PNTS (0,0)(0,167)
.TXT AT (5261,2213)
 BACKFILL <1 16 5 1000 0 0 0>
 R_ARROW 0 SIZE 100000 MARGINS <0 0 0 0>
 TXTXYOFF <-3 101>
 T_PNTS (0,101)(0,0)(1297,0)(1297,104)(0,104)(0,101)(1297,101)
 .STR
  "TEMPORARY FILES"
.TXT AT (4065,358)
 SIZE 80000
 TXTXYOFF <-1 81>
 T_PNTS (0,81)(0,0)(857,0)(857,104)(0,104)(0,81)(857,81)
 .STR
  "formated_sample"
.TXT AT (5488,1693)
 TXTXYOFF <-7 80>
 T_PNTS (0,80)(0,0)(447,0)(447,103)(0,103)(0,80)(447,80)
 .STR
  "raw_data"
.TXT AT (2508,693)
 TXTXYOFF <-3 80>
 T_PNTS (0,80)(0,0)(629,0)(629,103)(0,103)(0,80)(629,80)
 .STR
  "state_control"
.POL AT (4564,2023)
 BACKFILL <16 16 0 0 0 0 0>
 R_ARROW 1 MARGINS <125 125 250 62>
 PNTS (84,0)(0,583)
.POL AT (3089,2773)
 R_ARROW 0
 PNTS (0,0)(881,0)
.POL AT (2922,2189)
 R_ARROW 1
 PNTS (167,584)(0,0)
.ELL AT (2231,2941)
 BACKFILL <16 16 5 1000 0 0 0>
 R_ARROW 0
 PNTS (0,0)(1180,0)(1180,1180)(0,1180)(0,0)
.TXT AT (2465,3157)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 100000 MARGINS <0 0 0 0>
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(725,0)(725,104)(0,104)(0,101)(725,101)
 .STR
  "Sound Card"
.TXT AT (2458,3299)
 TXTXYOFF <0 99>
 T_PNTS (0,99)(0,0)(744,0)(744,128)(0,128)(0,99)(744,99)
 .STR
  "Applications"
.TXT AT (2499,3499)
 ITALICS ON
 TXTXYOFF <-5 99>
 T_PNTS (0,99)(0,0)(673,0)(673,128)(0,128)(0,99)(673,99)
 .STR
  "studio_tool"
.TXT AT (2442,3652)
 T_PNTS (0,99)(0,0)(794,0)(794,128)(0,128)(0,99)(794,99)
 .STR
  "studio_mixer"
.ELL AT (415,2433)
 BACKFILL <16 16 5 1000 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(1180,0)(1180,1180)(0,1180)(0,0)
.TXT AT (832,2573)
 BACKFILL <1 16 5 1000 0 0 0>
 ITALICS OFF MARGINS <0 0 0 0>
 TXTXYOFF <-11 99>
 T_PNTS (0,99)(0,0)(321,0)(321,102)(0,102)(0,99)(321,99)
 .STR
  "UNIX"
.TXT AT (768,2739)
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(449,0)(449,131)(0,131)(0,101)(449,101)
 .STR
  "System"
.TXT AT (686,2897)
 TXTXYOFF <-7 101>
 T_PNTS (0,101)(0,0)(698,0)(698,104)(0,104)(0,101)(698,101)
 .STR
  "Commands"
.TXT AT (632,3099)
 ITALICS ON
 TXTXYOFF <-4 99>
 T_PNTS (0,99)(0,0)(741,0)(741,129)(0,129)(0,99)(741,99)
 .STR
  "grep,awk,cp"
.TXT AT (640,3285)
 TXTXYOFF <-5 99>
 T_PNTS (0,99)(0,0)(751,0)(751,120)(0,120)(0,99)(751,99)
 .STR
  "mv,head,tail"
.TXT AT (341,271)
 ITALICS OFF
 TXTXYOFF <-11 101>
 T_PNTS (0,101)(0,0)(374,0)(374,104)(0,104)(0,101)(374,101)
 .STR
  "USER"
.POL AT (589,1189)
 BACKFILL <16 16 0 0 0 0 0>
 R_ARROW 1 MARGINS <125 125 250 62>
 PNTS (0,0)(1000,167)
.TXT AT (762,1028)
 BACKFILL <1 16 5 1000 0 0 0>
 R_ARROW 0 SIZE 80000 MARGINS <0 0 0 0>
 TXTXYOFF <-7 78>
 T_PNTS (0,78)(0,0)(752,0)(752,101)(0,101)(0,78)(752,78)
 .STR
  "request_events"
.POL AT (1821,2189)
 BACKFILL <16 16 0 0 0 0 0>
 THICKNESS 1 STYLE 2 MARGINS <125 125 250 62>
 PNTS (0,0)(0,125)
.POL AT (1422,2314)
 R_ARROW 1
 PNTS (399,0)(0,283)
.POL AT (3396,3550)
 THICKNESS 14 STYLE 0 R_ARROW 0
 PNTS (0,0)(809,0)
.POL AT (4205,3200)
 PNTS (0,0)(0,350)
.POL AT (5137,2441)
 R_ARROW 1
 PNTS (0,2175)(625,0)
.POL AT (1830,4616)
 R_ARROW 0
 PNTS (0,0)(3291,0)
.POL AT (1304,3516)
 PNTS (526,1100)(0,0)
.RECT AT (3542,3856)
 BACKFILL <16 16 5 1000 0 0 0>
 PNTS (0,0)(1180,0)(1180,597)(0,597)(0,0)
.TXT AT (3708,3984)
 BACKFILL <1 16 5 1000 0 0 0>
 SIZE 100000 MARGINS <0 0 0 0>
 TXTXYOFF <-1 101>
 T_PNTS (0,101)(0,0)(698,0)(698,104)(0,104)(0,101)(698,101)
 .STR
  "VoxWare &"
.TXT AT (3695,4175)
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(725,0)(725,104)(0,104)(0,101)(725,101)
 .STR
  "Sound Card"
.POL AT (3154,4017)
 BACKFILL <16 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (0,0)(101,209)
.POL AT (3255,4233)
 PNTS (0,0)(299,8)
.POL AT (2804,4116)
 R_ARROW 1
 PNTS (0,0)(0,500)
.POL AT (3154,4542)
 R_ARROW 0
 PNTS (0,0)(84,74)
.POL AT (3137,4616)
 PNTS (101,0)(0,68)
.POL AT (3389,4175)
 PNTS (0,0)(66,51)
.POL AT (3387,4250)
 PNTS (0,50)(77,0)
.POL AT (3187,4108)
 PNTS (9,0)(0,109)
.POL AT (3205,4101)
 PNTS (0,0)(74,33)
.POL AT (5262,2441)
 R_ARROW 1
 PNTS (642,0)(0,2309)
.POL AT (1729,4750)
 R_ARROW 0
 PNTS (3517,0)(0,0)
.POL AT (1179,3592)
 R_ARROW 1
 PNTS (550,1158)(0,0)
.TXT AT (2227,4319)
 BACKFILL <1 16 5 1000 0 0 0>
 R_ARROW 0 SIZE 80000 MARGINS <0 0 0 0>
 TXTXYOFF <-7 80>
 T_PNTS (0,80)(0,0)(423,0)(423,103)(0,103)(0,80)(423,80)
 .STR
  "rec_data"
.TXT AT (1435,3703)
 TXTXYOFF <-4 80>
 T_PNTS (0,80)(0,0)(575,0)(575,103)(0,103)(0,80)(575,80)
 .STR
  "edited_data"
.TXT AT (5453,4451)
 TXTXYOFF <-7 80>
 T_PNTS (0,80)(0,0)(447,0)(447,103)(0,103)(0,80)(447,80)
 .STR
  "raw_data"
.POL AT (3762,3550)
 BACKFILL <16 16 0 0 0 0 0>
 MARGINS <125 125 250 62>
 PNTS (77,0)(0,51)
.POL AT (3755,3483)
 PNTS (90,76)(0,0)
.TXT AT (3742,3619)
 BACKFILL <1 16 5 1000 0 0 0>
 MARGINS <0 0 0 0>
 TXTXYOFF <-4 81>
 T_PNTS (0,81)(0,0)(452,0)(452,104)(0,104)(0,81)(452,81)
 .STR
  "card_info"
.TXT AT (3167,2577)
 T_PNTS (0,81)(0,0)(669,0)(669,104)(0,104)(0,81)(669,81)
 .STR
  "app_data/info"
.TXT AT (1093,2236)
 TXTXYOFF <-4 80>
 T_PNTS (0,80)(0,0)(610,0)(610,103)(0,103)(0,80)(610,80)
 .STR
  "edit_request"
.TXT AT (1969,2702)
 TXTXYOFF <-7 81>
 T_PNTS (0,81)(0,0)(979,0)(979,105)(0,105)(0,81)(979,81)
 .STR
  "play/record_request"
.TXT AT (3073,4285)
 TXTXYOFF <-7 80>
 T_PNTS (0,80)(0,0)(447,0)(447,103)(0,103)(0,80)(447,80)
 .STR
  "raw_data"
.ELL AT (2937,4725)
 MARGINS <125 125 250 62>
 PNTS (0,0)(60,0)(60,56)(0,56)(0,0)
.POL AT (2967,4108)
 BACKFILL <1 16 0 0 0 0 0>
 R_ARROW 1
 PNTS (0,621)(0,0)
.POL AT (5029,512)
 R_ARROW 0
 PNTS (84,0)(0,57)
.POL AT (5029,462)
 PNTS (0,0)(66,50)
.ELL AT (2772,4581)
 BACKFILL <1 16 5 1000 0 0 0>
 PNTS (0,0)(60,0)(60,56)(0,56)(0,0)
.TXT AT (4058,2114)
 MARGINS <0 0 0 0>
 TXTXYOFF <-4 80>
 T_PNTS (0,80)(0,0)(457,0)(457,103)(0,103)(0,80)(457,80)
 .STR
  "app_data"
.TXT AT (2015,2420)
 TXTXYOFF <0 80>
 T_PNTS (0,80)(0,0)(839,0)(839,104)(0,104)(0,80)(839,80)
 .STR
  "VU and play_line"
.TXT AT (2526,3792)
 SIZE 100000 ITALICS ON
 TXTXYOFF <-6 101>
 T_PNTS (0,101)(0,0)(599,0)(599,122)(0,122)(0,101)(599,101)
 .STR
  "fader, etc."
END PICTURE

*END GRAPHICS
<end_data>
<start_footnote "3">
<S_F fieldType:5>
<T "footnote_body">
<FV>
<T "1" position:3 >
<E_F>
<T "When compiling ">
<T "fader.c" face:"Courier" >
<T " \"by hand\", be absolutely sure to use the compiler options listed in the \
 install file.  The program is forced to do type conversions between the sample\
  types and double-precision floating point, its own internal working type.  It\
  is very important that, in particular, ">
<T "char" face:"Courier" >
<T " types are ">
<T "signed" italic >
<T " be default.  Use the following command for compilation with " no-italic >
<T "gcc" no-italic face:"Courier" >
<T ":" no-italic >
<P "Normal" lineSpacing:0 >
<T "gcc -O2 -fsigned-char fader.c -o fader -lm" face:"Courier" >
<P "Normal" justifyCenter lineSpacing:0  face:"Courier" >
<end_footnote>
<start_vars>
<V "PathName$" "/home/nick/Projects/lib/SoundStudio/DOCS/Applix/prog-guide.aw">
<V "DocName$" "prog-guide.aw">
<V "Language@" 1>
<V "ReadOnly@" 0>
<V "PathNameImport$" "/home/een6njb/Tcl/StudioDoc/SS_PROG.DOC">
<V "Creator$" "Applix Words">
<V "Created$" "Release 4.30 (build 807.65.0.22) #17">
<V "AxCreationCodeVersion$" "Thu Mar 13 12:31:34 1997">
<V "CreationDate$" "Thu Jan 15 10:23:26 1998">
<V "CreationUser$" "???">
<V "Updated$" "Release 4.42 (build 1021.500) #17">
<V "AxUpdateCodeVersion$" "Thu Jul  8 01:01:49 1999">
<V "UpdateDate$" "Tue May 22 15:31:06 2001">
<V "UpdateUser$" "nick">
<V "DocWindowAttrType@" 1>
<V "DocWindowSize$" < 8507 7400>>
<end_vars>
<end_document>
*END WORDS
